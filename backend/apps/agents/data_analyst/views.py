"""Phase 9D — Data Analyst Agent V1 read-only API.
...
def _serialize_snapshot(snapshot: DataAnalystSnapshot) -> dict[str, Any]:
    return {
        ...
        "leadCount30d": snapshot.lead_count_30d,
        "callCount30d": snapshot.call_count_30d,
        "confirmedOrderCount30d": snapshot.confirmed_order_count_30d,
        "deliveredOrderCount30d": snapshot.delivered_order_count_30d,
        "reorderCount30d": snapshot.reorder_count_30d,
        "leadToCallRate": snapshot.lead_to_call_rate,
        "callToConfirmedRate": snapshot.call_to_confirmed_rate,
        "confirmedToDeliveredRate": snapshot.confirmed_to_delivered_rate,
        "deliveredToReorderRate": snapshot.delivered_to_reorder_rate,
        "topStates": list(snapshot.top_states or []),
        "dayOfWeekCounts": dict(snapshot.day_of_week_counts or {}),
...
class DataAnalystSnapshotsListView(APIView):
    """``GET /api/v1/data-analyst/snapshots/?page=&page_size=``."""
    ...
    def get(self, request):
        ...
        qs = DataAnalystSnapshot.objects.all().order_by("-snapshot_at")
        total = qs.count()
...
class DataAnalystSnapshotLatestView(APIView):
    """``GET /api/v1/data-analyst/snapshots/latest/`` convenience."""
    ...
    def get(self, request):
        ...
        last_run = (
            AgentRun.objects.filter(agent=AgentRun.Agent.DATA_ANALYST)
            .order_by("-created_at")
            .first()
        )
        return Response(
...
class DataAnalystSnapshotDetailView(APIView):
    """``GET /api/v1/data-analyst/snapshots/<id>/``."""

    permission_classes = [_AdminPermission]

    def get(self, request, pk: int):
        snapshot = DataAnalystSnapshot.objects.filter(pk=pk).first()
        if snapshot is None:

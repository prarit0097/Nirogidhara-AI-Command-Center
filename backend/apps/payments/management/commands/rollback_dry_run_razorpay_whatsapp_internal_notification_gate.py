...
Phase 7E - rollback-dry-run rehearsal. Re-validates invariants AFTER
the dry-run pass to prove no row leaked. NEVER calls a provider;
NEVER sends or queues WhatsApp; NEVER mutates business rows. Reason
text required.
"""
...
class Command(BaseCommand):
    help = (
        "Phase 7E - rollback-dry-run a WhatsApp internal notification "
        "gate (no provider call; no WhatsApp send / queue; no "
    ...
    def handle(self, *args, **options) -> None:
        ...
        if not reason:
            raise CommandError(
                "--reason must be a non-empty string for Phase 7E rollback dry-run"
            )
        report = rollback_dry_run_phase7e_gate(gate_id, reason=reason)
        if options.get("json"):

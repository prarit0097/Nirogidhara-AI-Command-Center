"""``python manage.py archive_phase8a_payment_order_mutation_sandbox --gate-id N --reason "..." --json``."""
...
class Command(BaseCommand):
    help = (
        "Phase 8A — archive a sandbox gate. Never calls a provider; "
    ...
    def handle(self, *args, **options) -> None:
        ...
        report = archive_phase8a_payment_order_mutation_sandbox(
        ...
        if report.get("ok"):
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Phase 8A gate archived gate_id={gate['id']} "

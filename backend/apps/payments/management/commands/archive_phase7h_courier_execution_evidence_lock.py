"""``python manage.py archive_phase7h_courier_execution_evidence_lock --lock-id N --reason "..." --json``."""
...
class Command(BaseCommand):
    help = (
        "Phase 7H - archive an evidence lock. Lock-only; never calls "
    ...
    def handle(self, *args, **options) -> None:
        ...
        report = archive_phase7h_evidence_lock(
        ...
        if report.get("ok"):
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Phase 7H evidence lock archived lock_id="

"""``python manage.py reject_razorpay_payment_dispatch_readiness_gate \\
    --readiness-id <ID> --reason "..." --json``.

Phase 6R — reject a dispatch readiness gate. Audit-only.
"""
...
class Command(BaseCommand):
    help = (
        "Phase 6R — reject a payment dispatch readiness gate review. "
        "No WhatsApp send, no courier call, no real business mutation, "
        "no provider call."
    )
    ...
    def handle(self, *args, **options) -> None:
        ...
        report = reject_phase6r_payment_dispatch_readiness_gate(
            readiness_id,
            reviewed_by=None,

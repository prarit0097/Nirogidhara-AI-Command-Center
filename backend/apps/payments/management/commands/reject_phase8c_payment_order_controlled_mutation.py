"""``python manage.py reject_phase8c_payment_order_controlled_mutation --gate-id N --reason "..." --json``."""
...
class Command(BaseCommand):
    help = (
        "Phase 8C — reject a controlled-mutation gate (only valid "
        "from draft / pending_manual_review / dry_run_passed / "
        "approved_for_one_shot_controlled_mutation / blocked). "
        "Never executes a mutation; never calls a provider."
    )
    ...
    def handle(self, *args, **options) -> None:
        ...
        report = reject_phase8c_payment_order_controlled_mutation(
        ...
        if report.get("ok"):
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"Phase 8C gate rejected gate_id={gate['id']} "

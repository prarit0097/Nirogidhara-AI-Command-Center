"""``python manage.py approve_phase8a_payment_order_mutation_sandbox --gate-id N --reason "..." --json``."""
...
class Command(BaseCommand):
    help = (
        "Phase 8A — approve a sandbox gate (state transition only; "
        "moves status to approved_for_future_phase8b_review). "
        "Requires at least one passed dry-run. Never enables any "
        "real mutation; never calls a provider; never mutates "
        "business rows."
    )
    ...
    def handle(self, *args, **options) -> None:
        ...
        report = approve_phase8a_payment_order_mutation_sandbox(
        ...
        if report.get("ok"):
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Phase 8A gate approved gate_id={gate['id']} "

"""``python manage.py reject_phase8a_payment_order_mutation_sandbox --gate-id N --reason "..." --json``."""
...
class Command(BaseCommand):
    help = (
        "Phase 8A — reject a sandbox gate (only valid from draft / "
    ...
    def handle(self, *args, **options) -> None:
        ...
        report = reject_phase8a_payment_order_mutation_sandbox(
        ...
        if report.get("ok"):
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"Phase 8A gate rejected gate_id={gate['id']} "

"""``python manage.py reject_phase8b_payment_order_mutation_review_gate --gate-id N --reason "..." --json``."""
...
class Command(BaseCommand):
    help = (
        "Phase 8B — reject a review gate (only valid from draft / "
        "pending_manual_review / dry_run_passed / blocked). Never "
    ...
    def handle(self, *args, **options) -> None:
        ...
        report = reject_phase8b_payment_order_mutation_review_gate(
        ...
        if report.get("ok"):
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"Phase 8B gate rejected gate_id={gate['id']} "

"""``python manage.py archive_phase8c_payment_order_controlled_mutation --gate-id N --reason "..." --json``."""
...
class Command(BaseCommand):
    help = (
        "Phase 8C — archive a controlled-mutation gate. Never calls "
        "a provider; never mutates business rows."
    )
    ...
    def handle(self, *args, **options) -> None:
        ...
        report = archive_phase8c_payment_order_controlled_mutation(
        ...
        if report.get("ok"):
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Phase 8C gate archived gate_id={gate['id']} "

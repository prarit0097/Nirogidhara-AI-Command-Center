"""``python manage.py archive_phase7i_final_audit_lock --lock-id N --reason "..." --json``."""
...
class Command(BaseCommand):
    help = (
        "Phase 7I — archive a Final Phase 7 audit-lock row. Lock-"
        "only; never calls a provider; never mutates business rows."
    )
    ...
    def handle(self, *args, **options) -> None:
        ...
        report = archive_phase7i_final_audit_lock(
        ...
        if report.get("ok"):
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Phase 7I final audit archived lock_id="

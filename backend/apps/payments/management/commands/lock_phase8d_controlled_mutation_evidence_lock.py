"""``python manage.py lock_phase8d_controlled_mutation_evidence_lock --lock-id N --reason "..." --json``."""
...
class Command(BaseCommand):
    help = (
        "Phase 8D — lock an evidence row (state transition only; "
        "moves status to `locked` and freezes the composite "
        "snapshot). Never executes Phase 8C again; never calls a "
    ...
    def handle(self, *args, **options) -> None:
        ...
        report = lock_phase8d_controlled_mutation_evidence_lock(
        ...
        if report.get("ok"):
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Phase 8D evidence locked lock_id={lock['id']} "

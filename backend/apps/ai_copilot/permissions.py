"""Phase 16I — DRF permissions for the AI Copilot surfaces.

Every endpoint requires authentication. Reads are open to any authenticated
user (read-only viewers may view suggestions + status). Mutations (generate a
suggestion, review one) require director/admin/superuser — a copilot suggestion
never executes a business action, but generating + approving one is a Director
governance act.
"""

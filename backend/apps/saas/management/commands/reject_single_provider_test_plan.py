"""``python manage.py reject_single_provider_test_plan --plan-id <id> ...``."""
...
class Command(BaseCommand):
    help = "Reject a provider test plan. Phase 6J planning only."
    ...
    def handle(self, *args, **options) -> None:
        ...
        report = {
            "passed": plan.status == RuntimeProviderTestPlan.Status.REJECTED,
            **serialize_provider_test_plan(plan),
        }
        if options.get("json"):
            self.stdout.write(_json.dumps(report, default=str))
            return
        self.stdout.write(self.style.WARNING("Provider test plan rejected"))

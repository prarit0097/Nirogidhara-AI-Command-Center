"""``python manage.py archive_single_provider_test_plan --plan-id <id> ...``."""
...
class Command(BaseCommand):
    help = "Archive a provider test plan. No external side effects."
    ...
    def handle(self, *args, **options) -> None:
        ...
        report = {
            "passed": plan.status == RuntimeProviderTestPlan.Status.ARCHIVED,
            **serialize_provider_test_plan(plan),
        }
        if options.get("json"):
            self.stdout.write(_json.dumps(report, default=str))
            return
        self.stdout.write(self.style.MIGRATE_HEADING("Provider test plan archived"))

...
Status: Phase 1 + 2A + 2B + 2C + 2D + 2E + 3A + 3B + 3C + 3D + 3E + 4A + 4B + 4C + 4D + 4E complete (CRM data layer, write APIs, all four gateway integrations, AgentRun + 7 per-agent runtime services, Celery beat at 09:00 + 18:00 IST, OpenAI → Anthropic fallback, model-wise USD cost tracking, sandbox toggle + versioned prompts with rollback + per-agent USD budgets, Scheduler + Governance frontend pages, Phase 3E business config: catalog admin app + discount policy (10/20% bands) + ₹499 fixed advance + reward/penalty deterministic scoring + approval matrix table + WhatsApp design scaffold, Phase 4B reward/penalty engine wiring: AI-agents-only scoring + CEO AI net accountability + /api/rewards/{events,summary,sweep}/ endpoints + Rewards page leaderboard, **Phase 4C approval-matrix middleware enforcement: ApprovalRequest + ApprovalDecisionLog models, apps.ai_governance.approval_engine (evaluate / enforce_or_queue / approve / reject / AgentRun bridge), 5 admin/director endpoints, live enforcement on custom-amount payment links + prompt activation + sandbox-disable, Governance page approval queue with Approve / Reject**). Phase 4D adds the **Approved Action Execution Layer**: `ApprovalExecutionLog` model (one-executed-per-request constraint), `apps.ai_governance.approval_execution` engine + 3-action allow-listed registry (payment.link.advance_499, payment.link.custom_amount, ai.prompt_version.activate), `POST /api/ai/approvals/{id}/execute/` endpoint, Governance page Execute button + execution-status column. Unmapped actions → HTTP 400 + `ai.approval.execution_skipped`. CAIO blocked at engine + AgentRun bridge + execute layer. Phase 4A adds **real-time AuditEvent WebSockets**: Django Channels + channels_redis + daphne, `ws://<host>/ws/audit/events/` carries the full stored `AuditEvent.payload`, Dashboard activity feed and Governance approval queue refresh live, existing polling endpoints remain as fallback. Phase 4E **expands the execution registry** with `discount.up_to_10`, `discount.11_to_20`, and `ai.sandbox.disable` (Director-only). New `apply_order_discount` service mutates only `Order.discount_pct` via the locked Phase 3E policy. `discount.above_20` + ad-budget + refund + WhatsApp + production-live-mode-switch stay unmapped → HTTP 400 + `ai.approval.execution_skipped`. **351 backend tests + 13 frontend tests**, all green. **Phase 5A-0 (audit-only, doc-only):** the [`prarit0097/Whatsapp-sales-dashboard`](https://github.com/prarit0097/Whatsapp-sales-dashboard) reference repo was audited (SHA `273b57a3`); the integration plan lives in `docs/WHATSAPP_INTEGRATION_PLAN.md`. The reference repo's Meta Cloud provider is stubbed and unusable — Nirogidhara builds Meta Cloud from scratch in Phase 5A. Baileys is dev/demo only, off by default in any non-DEBUG environment. **Phase 5A-1 (addendum, doc-only):** WhatsApp module direction widened to **inbound-first AI Chat Sales Agent + lifecycle messaging** mirroring the AI Calling Agent. Locked greeting rule, address collection in chat, category detection, chat-to-call handoff, **discount discipline (no upfront discount; rescue-only)**, and **50% total discount hard cap across all AI workflows**. Sections S–GG of the integration plan. Next: Phase 5A WhatsApp Live Sender Foundation.

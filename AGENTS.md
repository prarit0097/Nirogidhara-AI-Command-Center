...
13. **Phase 7F is gate-only.** Phase 7F (`apps.payments.razorpay_courier_readiness`) NEVER calls the Delhivery API, NEVER creates a `Shipment` / `WorkflowStep` / `RescueAttempt` row, NEVER creates an AWB, NEVER books a pickup, NEVER generates a courier label, NEVER sends or queues WhatsApp, NEVER calls Meta Cloud / Razorpay / Vapi, NEVER mutates real business rows, NEVER edits any `.env*` file. The service module is asserted (by static-file scan tests) to **never** import `apps.shipments.integrations.delhivery_client.create_awb` / `_create_via_sdk`, `apps.shipments.services.create_shipment` / `create_rescue_attempt` / `update_rescue_outcome`, `apps.whatsapp.services.send_freeform_text_message` / `send_queued_message` / `queue_template_message`, `apps.whatsapp.integrations.whatsapp.meta_cloud_client`, `apps.payments.integrations.razorpay_client`, or `dotenv`. Read-only count diagnostics on `apps.shipments.models.{Shipment,WorkflowStep,RescueAttempt}` and `apps.whatsapp.models.{WhatsAppMessage,WhatsAppLifecycleEvent,WhatsAppHandoffToCall}` are allowed — never `.create()` / `.update()` / `.save()`. Phase 7F approval flips status to `approved_for_future_phase7g_or_courier_execution_review` only; it does NOT enable any provider call. Phase 7G (live courier execution) requires a separate, dated, written Director directive AND a future "execute-window guard for Delhivery" extension reusing `apps.saas.utc_window.validate_within_director_window` (15-minute cap, mirrors Phase 7D-Hotfix-1).

12. **Provider-touching execute commands MUST validate `now ∈ [BEGIN_UTC, END_UTC]` from a structured Director sign-off.** The Phase 7D execute-once incident on 2026-05-07 (`order_SmThqpK6sc6Dhs`, attempt id 1, executed at `12:42:46Z`, ~134s before the approved `12:45:00Z → 13:00:00Z` window) shipped because `execute_razorpay_controlled_pilot_test_order` accepted free-text sign-off without parsing structured markers. **No business or customer impact** (every locked-False boolean stayed False, the attempt was rolled back), but the time invariant was violated. **Phase 7D-Hotfix-1 (SHIPPED)** fixes this — both `execute_razorpay_controlled_pilot_test_order` and `execute_single_razorpay_test_order` now call `apps.saas.utc_window.parse_director_signoff_window` + `validate_within_director_window` (alias `validate_execution_window`, default `max_window_seconds=900`) and refuse on missing markers, malformed timestamps, `now < window_start`, `now > window_end`, window length > 15 min, or stale window (`window_start < now - 24h`). No free-text honour-system windows. Phase 7E review windows use `validate_review_window` (max 24h); execute windows use `validate_within_director_window` / `validate_execution_window` (max 15 min). Historical Phase 7D attempt id 1 stays `NULL` on the new `recorded_signoff_window_*` fields and remains the canonical legacy free-text row that Phase 7E acknowledges via `--acknowledge-source-phase7d-window-violation`. Any future Phase 7E-Live / Phase 7F send / courier command must reuse the same predicate; do not roll a fresh window check.

**Current baseline (Phase 9B RTO Prevention agent):** Phase 9B RTO Prevention Agent V1 is shipped as a **recommendations-only** deterministic daily Celery sweep over in-flight orders (stages `Confirmed`, `Dispatched`, `Out for Delivery`; explicitly excludes terminal `Delivered`, `RTO`, `Cancelled`). It adds `agents.RtoRiskSnapshot`, migration `agents.0003_rto_risk_snapshot`, a new `AgentRun.Agent.RTO_PREVENTION` enum value + migration `ai_governance.0008_phase9b_add_rto_prevention_agent`, service helpers in `apps.agents.rto_prevention.service`, Celery task `apps.agents.rto_prevention.tasks.run_rto_prevention_agent_daily` scheduled at 09:00 IST, and read-only endpoints under `/api/v1/rto-prevention/` (`snapshots/`, `snapshots/<id>/`, `cohorts/`) plus a read-only `/saas-admin` card. The agent computes a deterministic 0–100 risk score, classifies into one of four risk tiers (`low` / `medium` / `high` / `critical`), assigns a lifecycle stage (`pre_dispatch` / `in_transit` / `delivery_at_risk`), populates risk reason codes, and emits one of four recommendation kinds (`monitor_only` / `send_confirmation_reminder` / `send_pre_delivery_call_request` / `escalate_to_team_lead`). The agent **NEVER** triggers calls, WhatsApp sends, discount creation, shipment mutation, or payment mutation; all outbound continues to require the established gates (Phase 5D lifecycle, Phase 5E rescue discount, Phase 7E-Live-B, Phase 7G-Live). Kill switch uses the Phase 7E-Live-B Hotfix-1 Postgres-safe pattern. Sandbox mode propagates to both the snapshot and the linked AgentRun. Verification baseline is **2272 backend tests + 82 frontend tests**.

**Previous baseline (Phase 9A Customer Success agent):** Phase 9A Customer Success / Reorder Agent V1 is shipped as a **recommendations-only** deterministic daily Celery sweep. It adds `agents.CustomerSuccessSnapshot`, migration `agents.0002_customer_success_snapshot`, a new `AgentRun.Agent.CUSTOMER_SUCCESS` enum value + migration `ai_governance.0007_phase9a_add_customer_success_agent`, service helpers in `apps.agents.customer_success.service`, Celery task `apps.agents.customer_success.tasks.run_customer_success_agent_daily` scheduled at 08:00 IST, and read-only endpoints under `/api/v1/customer-success/` (`snapshots/`, `snapshots/<id>/`, `cohorts/`) plus a read-only `/saas-admin` card. The agent computes a deterministic 0–100 score, lifecycle stage, reorder-candidate and at-risk flags, and one of four recommendation kinds. The agent **NEVER** sends WhatsApp, makes a call, creates a payment link, dispatches a shipment, or mutates any business row; all outbound continues to require the established gates (Phase 5D lifecycle, Phase 7E-Live-B, Phase 7G-Live). Kill switch uses the Phase 7E-Live-B Hotfix-1 Postgres-safe pattern. Sandbox mode propagates to both the snapshot and the linked AgentRun. Verification baseline is **2245 backend tests + 82 frontend tests**.

**Previous baseline (Phase 7G-Live gate):** Phase 7G-Live real-customer Delhivery one-shot dispatch gate is shipped as CLI-only governance. It adds `shipments.Phase7GLiveRealCustomerDispatchGate`, migration `shipments.0005_phase7g_live_real_customer_dispatch_gate`, service helper `apps.shipments.phase7g_live_real_customer_dispatch`, five commands (`inspect_phase7g_live_real_customer_gate`, `prepare_phase7g_live_real_customer_gate`, `approve_phase7g_live_real_customer_gate`, `execute_phase7g_live_real_customer_dispatch`, `rollback_phase7g_live_real_customer_dispatch`), a new `cancel_awb` helper on `apps.shipments.integrations.delhivery_client`, read-only `/api/v1/saas/phase7g-live/gates/`, and a read-only `/saas-admin` card. No frontend approve / execute / rollback buttons exist. The execute path is one gate / one confirmed real customer order / one Delhivery AWB only, requires runtime `PHASE7G_LIVE_REAL_CUSTOMER_DISPATCH_ENABLED=true` AND `DELHIVERY_MODE=live` via env prefix, structured Director UTC window, and explicit confirmation, then calls `apps.shipments.services.create_shipment(order=...)` once inside `transaction.atomic()`. Locked-False flags `payment_mutation_made`, `order_payment_status_changed`, `whatsapp_sent`, `razorpay_called` stay False. The kill switch helper uses the Phase 7E-Live-B Hotfix-1 fixed pattern: an explicit `scope="global", enabled=False` row always wins over the seeded enabled default, ordered by `-pk` for determinism. Rollback attempts the Delhivery cancellation API for the gate's AWB and records the provider result honestly — Delhivery may refuse if the shipment is already in transit; the gate transitions to `rollback_recorded` either way. `.env.production` is not edited. No live customer dispatch was run by this change. Phase 7E-Live-B gate code remains shipped but no live customer send has been performed. Previous Phase 8F Reading 1 remains complete as executed + rolled back, with Order `NRG-20435` back to Partial and Payment `PAY-30125` back to Pending. Expected verification baseline is **2217 backend tests + 82 frontend tests**.

**Previous baseline (Phase 7E-Live-B gate):** Phase 7E-Live-B real-customer WhatsApp one-shot controlled send gate is shipped as CLI-only governance. It adds `whatsapp.Phase7ELiveBRealCustomerSendGate`, migration `whatsapp.0007_phase7e_live_b_real_customer_send_gate`, five commands (`inspect_phase7e_live_b_real_customer_gate`, `prepare_phase7e_live_b_real_customer_gate`, `approve_phase7e_live_b_real_customer_gate`, `execute_phase7e_live_b_real_customer_send`, `cancel_phase7e_live_b_real_customer_gate`), read-only `/api/v1/saas/phase7e-live-b/gates/`, and a read-only `/saas-admin` card. No frontend approve/execute/cancel buttons exist. The execute path requires runtime `PHASE7E_LIVE_B_REAL_CUSTOMER_SEND_ENABLED=true` via env prefix, structured Director UTC window, and explicit confirmation, then calls `queue_template_message(..., override_limited_test_mode=True)` for that one CLI execution. `.env.production` is not edited. No live customer send was run by that change. The Phase 7E-Live-B Hotfix-1 (commit `0d9c975`) made `_kill_switch_state()` conservative on Postgres: any `scope="global", enabled=False` row wins, ordered by `-pk`. Phase 7G-Live (above) reuses this exact pattern.
...
backend/apps/whatsapp/phase7e_live_b_real_customer_send.py ← Phase 7E-Live-B CLI-only real-customer WhatsApp one-shot gate helpers (one approved template / one customer / no rollback; runtime env prefix only)
backend/apps/whatsapp/management/commands/*phase7e_live_b_real_customer*.py ← Phase 7E-Live-B inspect/prepare/approve/execute/cancel commands; no frontend execute surface
backend/apps/shipments/phase7g_live_real_customer_dispatch.py ← Phase 7G-Live CLI-only real-customer Delhivery one-shot dispatch gate helpers (one confirmed order / one AWB / rollback attempts cancellation API and records honestly; uses Phase 7E-Live-B Hotfix-1 kill-switch helper pattern; runtime env prefix only)
backend/apps/shipments/management/commands/*phase7g_live_real_customer*.py ← Phase 7G-Live inspect/prepare/approve/execute/rollback commands; no frontend execute surface
backend/apps/agents/customer_success/ ← Phase 9A Customer Success / Reorder Agent V1 (recommendations-only). Service helpers + Celery task + read-only API live here; the agent NEVER triggers outbound action. Snapshots are immutable; every run writes a new row + AgentRun + AuditEvent. Kill switch uses Phase 7E-Live-B Hotfix-1 Postgres-safe pattern.
backend/apps/agents/rto_prevention/ ← Phase 9B RTO Prevention Agent V1 (recommendations-only). Same shape as Phase 9A but scoped to in-flight orders (Confirmed / Dispatched / Out for Delivery; excludes terminal stages). Daily Celery sweep at 09:00 IST. Tier mapping: 0–39 low, 40–59 medium, 60–79 high, 80–100 critical. NEVER triggers calls / WhatsApp / discount / shipment / payment mutation.
...
frontend/src/pages/SaasAdmin.tsx          ← Phase 6E read-only `/saas-admin`; Phase 6G adds Controlled Runtime Routing Dry Run + AI Provider Routing; Phase 6H adds Controlled Runtime Live Audit Gate; Phase 6I adds Single Internal Live Gate Simulation; Phase 7+ / Phase 8+ sections render read-only through Phase 8F, including Phase 8E candidate pool + Phase 8F target snapshot; no send/run-live/execute controls
...
backend/apps/saas/razorpay_audit_review.py ← Phase 6L — read-only audit-review + webhook-readiness service (review_razorpay_test_execution_audit / inspect_razorpay_webhook_readiness / plan_razorpay_webhook_readiness); never calls Razorpay; never returns raw secrets / raw provider response
...
backend/apps/payments/phase8f_real_customer_controlled_mutation.py ← Phase 8F controlled real-customer payment → order mutation service; CLI-only one-shot execute path, no provider calls, no WhatsApp/courier/customer notification, never mutates `Order.state`
backend/apps/payments/management/commands/recover_phase8f_attempt_to_approved.py ← Phase 8F-Hotfix-3 governance-only blocked-attempt recovery; status `blocked` → `approved_for_one_shot_real_mutation`, audit marker only, no Order/Payment/provider/WhatsApp/env touch
backend/apps/payments/migrations/0025_phase8f_hotfix_rename_indexes.py ← Phase 8F-Hotfix-1 stable explicit index names (`RenameIndex` ×8 only)
backend/tests/test_phase8f_real_customer_controlled_mutation.py ← Phase 8F + Hotfix-1/-2 regression coverage; Postgres-safe dynamic Phase 8E gate id handling
backend/tests/test_phase8f_hotfix3_attempt_recovery.py ← Phase 8F-Hotfix-3 recovery command regression coverage
backend/tests/conftest.py          ← Test Hygiene Hotfix-1 pins integration modes to mock for full-suite test sessions while leaving production runtime unchanged
...
backend/apps/whatsapp/order_booking.py    ← Phase 5C book_order_from_decision: validates draft, calls apps.orders.services.create_order, optionally creates ₹499 advance link via apps.payments.services.create_payment_link; never touches shipments
backend/apps/whatsapp/tasks.py            ← Phase 5C adds run_whatsapp_ai_agent_for_conversation Celery task wired from inbound webhook (eager-mode safe)
...
23 frontend pages: Dashboard, Leads CRM, Customer 360, AI Calling, Orders Pipeline, Confirmation Queue, Payments, Delhivery Tracking, RTO Rescue, AI Agents Center, CEO AI Briefing, CAIO Audit, AI Scheduler & Cost (Phase 3C), AI Governance (Phase 3D), Reward & Penalty, Human Call Learning, Claim Vault, Analytics, WhatsApp Inbox (Phase 5B), WhatsApp Templates (Phase 5A), WhatsApp Monitoring (Phase 5F-Gate), SaaS Admin (Phase 6E → 8F), Settings.
...
- **WhatsApp AI Chat Sales Agent (Phase 5C, locked)**: orchestration lives in `apps.whatsapp.ai_orchestration.run_whatsapp_ai_agent`. The flow is: language detection → greeting fast-path (locked Hindi/English UTILITY templates only) → LLM dispatch via `apps.integrations.ai.dispatch.dispatch_messages` → JSON schema validation (`apps.whatsapp.ai_schema.parse_decision`) → safety + Claim Vault + discount discipline + rate limits → freeform send (`services.send_freeform_text_message`) OR order booking (`order_booking.book_order_from_decision`) OR handoff. **Auto-reply defaults to OFF** (`WHATSAPP_AI_AUTO_REPLY_ENABLED=false`); production opts in after verification. Hard stops the agent must respect: (a) no medical-emergency replies — set `safety.medicalEmergency=true` → handoff; (b) no freeform claims outside `apps.compliance.Claim.approved`; (c) blocked phrases (`100% cure`, `permanent solution`, etc.) refuse-to-send via `reply_contains_blocked_phrase`; (d) never offer a discount on the first ask; (e) total cumulative discount across all stages NEVER exceeds 50% (`validate_total_discount_cap`); (f) order booking requires explicit customer confirmation in the latest inbound; (g) NEVER touch `apps.shipments` from chat — order is created at `Order.Stage.ORDER_PUNCHED` and Phase 5D moves it to `Order.Stage.CONFIRMATION_PENDING` via `apps.orders.services.move_to_confirmation`; (h) CAIO can never originate a customer-facing send (refused at `services.send_freeform_text_message` + `queue_template_message` + matrix). New endpoints under `/api/whatsapp/ai/...` and per-conversation `ai-mode / run-ai / ai-runs / handoff / resume-ai` — viewer read-only, operations+ for writes.
- **Phase 5E-Hotfix — Migration drift gate (locked)**: hand-rolling Django index names is a footgun. Phase 5D + 5E migrations used short manual index names (e.g. `whatsapp_wh_convers_h0_idx`) but Django's auto-namer wants suffix-hash style (e.g. `whatsapp_wh_convers_ae1708_idx`); the VPS first-deploy migrate logged "models in app(s) 'orders', 'whatsapp' have changes that are not yet reflected in a migration" until Phase 5E-Hotfix added two `RenameIndex` migrations (`apps/orders/migrations/0004_rename_orders_disc_*` + `apps/whatsapp/migrations/0004_rename_whatsapp_wh_*`). **Release checklist now includes `python manage.py makemigrations --check --dry-run` — it MUST report "No changes detected" before every commit.** When you write a new migration with custom index names, generate them via `makemigrations` (or copy the auto-suffix names verbatim from a `--dry-run -v 3`); never hand-roll a name like `whatsapp_wh_status_h0_idx` again.
- **Phase 5E — Rescue Discount + Day-20 Reorder + Default Claims (locked)**: rescue discount math is owned by `apps.orders.rescue_discount`; never hand-roll cap math elsewhere. Cumulative cap is **50% absolute hard cap** across confirmation / delivery / RTO / reorder stages — `get_current_total_discount_pct` reads `Order.discount_pct` AND `WhatsAppConversation.metadata.ai.totalDiscountPct` and uses the larger of the two. The per-stage rescue ladder (confirmation 5/10/15, delivery 5/10, RTO 10/15/20 + high-risk step-up, reorder 5) is conservative-first; do not jump steps. Anything above the 0–10 auto band, above the 50% cap, or with no headroom **must** create an `ApprovalRequest` via `discount.rescue.ceo_review` (CEO AI / admin) or `discount.above_safe_auto_band` (director-only) — the rescue creator does this automatically through `apps.ai_governance.approval_engine.enforce_or_queue`. Customer acceptance applies via `apps.orders.services.apply_order_discount` only; no module mutates `Order.discount_pct` directly. `apps.orders.DiscountOfferLog` is append-only — every offer attempt (including blocked / skipped / CAIO refusal / over-cap) writes a row; the audit ledger fires `discount.offer.{created,sent,accepted,rejected,blocked,needs_ceo_review}`. Lifecycle service routes confirmation / delivery / RTO refusals + Day-20 reorder through Phase 5A's `queue_template_message`; new lifecycle audit kinds (`whatsapp.lifecycle.rescue_discount_{queued,sent}`, `whatsapp.lifecycle.reorder_day20_{queued,sent}`) make filtering rescue / reorder activity easy on the operator surface. Day-20 reorder sweep (`apps.whatsapp.reorder.run_day20_reorder_sweep`, `python manage.py run_reorder_day20_sweep`, Celery `run_reorder_day20_sweep_task`) covers delivered orders 20–27 days old, idempotent on `lifecycle:whatsapp.reorder_day20_reminder:order:{id}:day20`. **Defaults are SAFE**: `WHATSAPP_RESCUE_DISCOUNT_ENABLED=false`, `WHATSAPP_RTO_RESCUE_DISCOUNT_ENABLED=false`, `WHATSAPP_REORDER_DAY20_ENABLED=false`, `DEFAULT_CLAIMS_SEED_DEMO_ONLY=true`. Default Claim Vault seed (`python manage.py seed_default_claims`) is idempotent + protects real admin-added rows; demo seeds are flagged `version="demo-v1"` and surface in coverage as `risk=demo_ok` with a "replace before live rollout" note. Hard stops: never exceed 50% cumulative; never offer upfront on reorder Day 20 (only opens on customer objection); CAIO never originates an offer or applies a discount; **no campaigns / no refunds / no ad-budget execution / no shipment from chat**.
- **Phase 5D — Chat-to-Call Handoff + Lifecycle Automation (locked)**: handoff service at `apps.whatsapp.call_handoff.trigger_vapi_call_from_whatsapp` is the ONLY path that may dial Vapi from WhatsApp; never call `apps.calls.integrations.vapi_client` directly from a view. The service is idempotent on `(conversation, inbound_message, reason)`. Safety reasons (medical_emergency / side_effect_complaint / legal_threat / refund_threat) record a `WhatsAppHandoffToCall` row in `status=skipped` with `error_message=non_auto_reason` and never auto-dial sales — a human / doctor / admin picks up the call manually. Operator manual trigger: `POST /api/whatsapp/conversations/{id}/handoff-to-call/` (operations+ only). AI-booked orders move directly to the confirmation queue via `apps.orders.services.move_to_confirmation` from `book_order_from_decision`; failure is non-fatal but logged via `whatsapp.ai.order_moved_to_confirmation` audit + `confirmationMoveFailed` metadata flag. Lifecycle automation in `apps.whatsapp.lifecycle.queue_lifecycle_message` is idempotent on `lifecycle:{action}:{type}:{id}:{event}`; signal receivers in `apps.whatsapp.signals` listen on Order/Payment/Shipment `post_save` and dispatch via Celery (`send_whatsapp_lifecycle_message_task`) on commit. Lifecycle templates that touch product (`whatsapp.usage_explanation`) fail closed when the Phase 5D Claim Vault coverage report (`apps.compliance.coverage`) shows the product is `missing` / `weak`. **Defaults are SAFE**: `WHATSAPP_CALL_HANDOFF_ENABLED=false`, `WHATSAPP_LIFECYCLE_AUTOMATION_ENABLED=false`, `WHATSAPP_LIVE_META_LIMITED_TEST_MODE=true`. Production rollout sequence: mock → OpenAI test → limited live Meta with one number → flip auto-reply → flip handoff → flip lifecycle. Hard stops still apply: no shipment from chat, no campaigns, no refunds, no ad-budget execution.
- **Phase 5F-Gate Internal Allowed-Number Cohort Tooling (locked)**: three management commands enable safe expansion of the controlled live test from one allowed number to a tiny internal cohort of 2–3 staff numbers. Hard rules: (1) **`prepare_whatsapp_internal_test_number`** must REFUSE outright if the phone is not on `WHATSAPP_LIVE_META_ALLOWED_TEST_NUMBERS`. The allow-list lives in `.env.production` only — no DB-driven allow-list, no cookie-cutter expansion via the UI. (2) **No automation flag flips** during cohort expansion: `WHATSAPP_AI_AUTO_REPLY_ENABLED` / `WHATSAPP_CALL_HANDOFF_ENABLED` / `WHATSAPP_LIFECYCLE_AUTOMATION_ENABLED` / `WHATSAPP_RESCUE_DISCOUNT_ENABLED` / `WHATSAPP_RTO_RESCUE_DISCOUNT_ENABLED` / `WHATSAPP_REORDER_DAY20_ENABLED` stay default OFF. The inspector explicitly surfaces these in its JSON and flips `nextAction=keep_global_auto_reply_off` when any are on. (3) **Phone numbers are masked** to last-4 by default in the JSON output and audit payloads. `--show-full-numbers` is operator-only with a "do not paste publicly" warning. The audit row carries `phone_suffix` only; full E.164 must NEVER appear in audit payloads or committed docs. (4) **No mutation** of `Order` / `Payment` / `Shipment` / `DiscountOfferLog` from any cohort path (asserted in tests with before/after counts). `prepare_whatsapp_internal_test_number` only creates / reuses `Customer` + grants `WhatsAppConsent`. The inspector and dry-run loop never write a row. (5) **Send path unchanged** — after registering a number, the operator runs the existing `run_controlled_ai_auto_reply_test --phone +91… --send --json` per number. The existing safety stack, intent classifier, deterministic grounded / objection-aware fallbacks, final-send limited-mode guard, and audit ledger all stay in force. (6) Adding a new cohort number is a **two-step** procedure: first edit `.env.production` and recreate the backend container; then run `prepare_whatsapp_internal_test_number` to grant consent. Skipping either step → the controlled-test command refuses. (7) Cohort starts at 2–3 numbers and stays small until Director sign-off. Phase 5F (broadcast campaigns / growth automation) remains LOCKED.
- **Phase 5F-Gate Objection & Handoff Reason Refinement (locked)**: deterministic intent classifier + objection-aware reply builder + typed human-request handoff for the controlled AI auto-reply test command. `apps.whatsapp.grounded_reply_builder.classify_inbound_intent(text)` returns `IntentResult.primary ∈ {unsafe, human_request, discount_objection, product_info, unknown}` with detector outputs for `discount_objection` (with `objection_type ∈ {discount, price}`), `purchase_intent`, `human_request`, and `unsafe`. Hard rules: (1) **Priority order** in the controlled-test command: safety blockers → human request → unknown / no-claim-vault → discount objection → normal product-info → fail closed. (2) **Human-request short-circuit** — when the inbound matches the human-request vocabulary AND is not unsafe, the controlled-test command emits `whatsapp.ai.human_request_detected` + `whatsapp.ai.handoff_required` (`reason=human_advisor_requested`) WITHOUT invoking the LLM orchestrator. The handoff_required audit row never carries `reason=claim_vault_not_used` for human requests. (3) **Vapi remains gated** by `WHATSAPP_CALL_HANDOFF_ENABLED=false`; no call is triggered by the human-request path during the gate phase. (4) **Objection-aware fallback** runs only when discount-objection signal is present AND inbound is not unsafe AND backend grounding is valid; the reply NEVER promises a discount (`validate_objection_reply` rejects `discount confirmed` / `guaranteed discount` / `50% discount` framing); reply NEVER mutates `DiscountOfferLog` / `Order` / `Payment` / `Shipment`. (5) **`replyPolicy`** block on the controlled-test JSON output declares `upfrontDiscountOffered=false`, `discountMutationCreated=false`, `businessMutationCreated=false` for objection / human-request paths. (6) Adding new objection vocabulary requires updating both `DISCOUNT_OBJECTION_KEYWORDS` and (if a separate type) the type-resolution lists inside `detect_discount_objection`. Adding new human-request vocabulary requires updating `HUMAN_REQUEST_KEYWORDS`. Four new audit kinds (`whatsapp.ai.objection_detected` / `whatsapp.ai.objection_reply_used` / `whatsapp.ai.objection_reply_blocked` / `whatsapp.ai.human_request_detected`); payloads NEVER carry tokens / verify token / app secret. Phase 5F (broadcast campaigns / growth automation) remains LOCKED.
- **Phase 5F-Gate Deterministic Grounded Reply Builder (locked)**: backend Claim-Vault-grounded reply fallback used **only** by the controlled AI auto-reply test command. Lives in `apps.whatsapp.grounded_reply_builder` (`is_normal_product_info_inquiry`, `can_build_grounded_product_reply`, `build_grounded_product_reply`, `validate_reply_uses_claim_vault`). Hard rules: (1) Fallback fires ONLY when the LLM blocked with a soft non-safety reason (`claim_vault_not_used` / `low_confidence` / `ai_handoff_requested` / `auto_reply_disabled` / `no_action`) AND backend grounding is valid (claim row + ≥1 approved phrase) AND inbound is a normal product-info inquiry. Safety blockers (`medical_emergency` / `side_effect_complaint` / `legal_threat` / `blocked_phrase` / `limited_test_number_not_allowed`) NEVER trigger fallback. (2) Reply text is built ONLY from `Claim.approved` phrases + locked business facts (₹3000 / 30 capsules / ₹499 advance / conservative usage / doctor-escalation). Never adds invented benefits, never offers discount, never emits cure / guarantee / 100% / permanent. (3) `claimVaultUsed=true` flips ONLY because the validator confirmed the reply text literally embeds at least one approved phrase. (4) The send still flows through `services.send_freeform_text_message` so the limited-mode allow-list guard, consent, CAIO, idempotency, and audit ledger gates all stay in force. (5) The fallback is **controlled-test-only**; `run_whatsapp_ai_agent` for production webhook-driven runs stays strict — never invoke this builder from the orchestrator's webhook path. (6) Adding new business facts requires updating the builder's locked constants AND the `_build_context.settings.businessFactsAllowed` whitelist. Two new audit kinds (`whatsapp.ai.deterministic_grounded_reply_used` / `whatsapp.ai.deterministic_grounded_reply_blocked`); payloads NEVER carry tokens / verify token / app secret. Phase 5F (broadcast campaigns / growth automation) remains LOCKED.
- **Phase 5F-Gate Controlled Reply Confidence Fix (locked)**: prompt + action discipline + diagnostics cleanup. The system prompt carries an explicit `BUSINESS FACTS YOU MAY STATE FREELY` section (₹3000 / 30 capsules, ₹499 advance, conservative usage guidance, doctor-escalation rule for pregnancy / serious illness / allergies) AND an `ACTION SELECTION DECISION TREE` (cases A–E mapping every common inbound to the canonical action) AND a `FINAL CHECK` paragraph stating: "if the claims block has at least one APPROVED phrase AND every safety flag is false AND the inbound is a normal product / price / quantity / use-guidance question, then action MUST be `send_reply` with confidence ≥ 0.85 and `safety.claimVaultUsed=true`, and replyText MUST literally include one of the APPROVED phrases — defaulting to handoff on a grounded inquiry is a defect." Hard rules: (1) Business facts are routing labels and price/quantity facts; medical / product-benefit content still comes only from `Claim.approved`. (2) The confidence threshold env (`WHATSAPP_AI_AUTO_REPLY_CONFIDENCE_THRESHOLD=0.75`) is **never** lowered globally to fix a confidence problem — fix the prompt first. (3) `claimVaultUsed=false` still blocks the send; the prompt grounds the LLM but does not weaken the safety contract. (4) Adding new business facts requires updating the prompt's `BUSINESS FACTS` block AND the `_build_context` `settings.businessFactsAllowed` whitelist. (5) Diagnostics cleanup: the ambiguous `claim_count` is split into `claim_row_count` / `approved_claim_count` / `disallowed_phrase_count` across all four orchestrator audit kinds (`category_detected` / `reply_blocked` / `handoff_required` / `controlled_test.blocked`) and the controlled-test JSON; `claim_count` is preserved as a backward-compat alias for `approved_claim_count`. Audit payloads NEVER carry tokens / verify token / app secret. Phase 5F (broadcast campaigns / growth automation) remains LOCKED.
- **Phase 5F-Gate Claim Vault Grounding Fix (locked)**: category-slug → `Claim.product` mapping is the **only** sanctioned bridge between the AI's `weight-management`-style detection slug and the Claim Vault's human-label rows (`Weight Management`, …). Helper: `apps.whatsapp.claim_mapping.category_to_claim_product` (eight canonical slugs + ~25 aliases). Hard rules: (1) `_claims_for_category` runs `Claim.product__iexact=normalized_product` — NEVER `__icontains` against a slug, NEVER `__icontains=""` (empty-string fallback was the bug — it returned every Claim row). (2) Unknown / empty category fails closed (returns `[]`). (3) Adding a new category requires a new entry in `CATEGORY_SLUG_TO_PRODUCT` — do NOT add fuzzy substring matchers; every key is explicit. (4) `claimVaultUsed` stays false → block stays in force, regardless of how many claims the prompt could see. (5) Disallowed phrases continue to flow into the prompt avoid list. The controlled-test command's `--json` output now carries a full diagnostics block (`detectedCategory / normalizedClaimProduct / claimCount / confidence / action / replyPreview / safetyFlags / groundingStatus`); the orchestrator's `whatsapp.ai.{category_detected,reply_blocked,handoff_required}` audits gain `category / normalized_claim_product / claim_count / confidence`. Audit payloads NEVER carry tokens / verify token / app secret. Phase 5F (broadcast campaigns / growth automation) remains LOCKED.
- **Phase 5F-Gate Controlled AI Auto-Reply Test Harness (locked)**: scoped one-shot live AI reply path for the allowed test number only — without flipping the global `WHATSAPP_AI_AUTO_REPLY_ENABLED` env. Two contracts to honour. (1) **Final-send limited-mode guard** in `apps.whatsapp.services._limited_test_mode_blocks_send` — runs inside `send_freeform_text_message` AND `queue_template_message`. When `WHATSAPP_PROVIDER=meta_cloud` AND `WHATSAPP_LIVE_META_LIMITED_TEST_MODE=true`, every customer-facing send must target a phone on `WHATSAPP_LIVE_META_ALLOWED_TEST_NUMBERS` or it raises `WhatsAppServiceError(block_reason="limited_test_number_not_allowed")` + writes `whatsapp.send.blocked` audit. **Never bypass this guard from the orchestrator, lifecycle service, or any future caller — it is the last-line defence under limited mode.** (2) **`force_auto_reply: bool = False`** kwarg on `run_whatsapp_ai_agent` — only the new CLI command may pass it `True`, and only after every gate above has passed. Webhook-driven runs never set it. The new `python manage.py run_controlled_ai_auto_reply_test --phone +91… --message "…" [--dry-run|--send] [--json]` is the **only** sanctioned path that may produce a real AI auto-reply during the gate phase; default `--dry-run`, `--send` required for a real dispatch, refuses if any of the 6 automation flags (including `WHATSAPP_AI_AUTO_REPLY_ENABLED`) is on. Five new audit kinds (`whatsapp.ai.controlled_test.{started,dry_run_passed,sent,blocked,completed}`); payloads carry phone last-4 only, body 120-char preview, no tokens. **Do NOT introduce broadcast / campaign / MARKETING-tier behaviour to enable wider auto-reply** — Phase 5F remains LOCKED until a clean 24-hour soak under this harness has been observed.
- **Phase 5F-Gate Hardening Hotfix (locked)**: post-live-pass diagnostics layer. The limited live one-number test passed end-to-end on the VPS (`WAM-100003` outbound + `WAM-100004` inbound), but three gaps surfaced during the run and are now closed: (1) the duplicate-idempotency unique-constraint crash now returns clean JSON with `duplicateIdempotencyKey=true`, `existingMessageId`, `alreadyQueued`/`alreadySent`, `nextAction=inspect_existing_message` + a `whatsapp.meta_test.duplicate_idempotency` audit row; (2) `--check-webhook-config` now also reports `wabaSubscriptionActive` / `wabaSubscribedAppCount` from `GET /{WABA}/subscribed_apps` and emits a `whatsapp.meta_test.webhook_subscription_checked` audit row; (3) new strictly-read-only `python manage.py inspect_whatsapp_live_test --phone +91XXXXXXXXXX --json` command surfaces customer / consent / conversation / latest messages / webhook envelopes / status events / WABA subscription / latest `whatsapp.*` audit rows + typed `nextAction`. Inspector **NEVER** writes audit rows, sends messages, or mutates the DB; it gracefully handles missing Meta credentials and never prints tokens / verify token / app secret. `whatsapp.meta_test.duplicate_idempotency` and `whatsapp.meta_test.webhook_subscription_checked` are the only two new audit kinds — Phase 5F (broadcast campaigns / growth automation) remains LOCKED until the inspector reports `gate_hardened_ready_for_limited_ai_auto_reply_plan` on a real VPS run.
- **Phase 5F-Gate — Limited Live Meta WhatsApp One-Number Test (locked)**: real Meta Cloud sends are gated behind `python manage.py run_meta_one_number_test`. The command stacks every safety check before a single byte goes to `graph.facebook.com` — provider must be `meta_cloud`, `WHATSAPP_LIVE_META_LIMITED_TEST_MODE=true`, destination must be in `WHATSAPP_LIVE_META_ALLOWED_TEST_NUMBERS`, the template must be APPROVED + active + UTILITY/AUTHENTICATION (MARKETING tier refused), and every automation flag (`WHATSAPP_AI_AUTO_REPLY_ENABLED`, `WHATSAPP_CALL_HANDOFF_ENABLED`, `WHATSAPP_LIFECYCLE_AUTOMATION_ENABLED`, `WHATSAPP_RESCUE_DISCOUNT_ENABLED`, `WHATSAPP_RTO_RESCUE_DISCOUNT_ENABLED`, `WHATSAPP_REORDER_DAY20_ENABLED`) must remain off. Defaults to `--dry-run`; `--send` is required for a real dispatch. The number guard `apps.whatsapp.meta_one_number_test.is_number_allowed_for_live_meta_test` is the only allow-list checker — never roll your own. Eight new audit kinds (`whatsapp.meta_test.{started,config_ok,config_failed,blocked_number,template_missing,sent,failed,completed}`); audit payloads NEVER carry tokens. **Phase 5F (broadcast campaigns / growth automation) remains LOCKED until this gate passes on a live test number.** Do not start broadcast / MARKETING template work, do not enable any of the 6 automation flags, and do not extend the number allow-list to a customer cohort before the gate is green.
- **Production deployment is LIVE at `https://ai.nirogidhara.com`** (Phase 5B-Deploy + post-deploy hotfix sync). VPS folder: `/opt/nirogidhara-command`. Stack: isolated Docker Compose with 6 containers (`nirogidhara-db / -redis / -backend / -worker / -beat / -nginx`) — project name `nirogidhara-command`, network `nirogidhara_network`, host port `18020:80`. Operational runbook: `nd.md` §17 + `docs/DEPLOYMENT_VPS.md`. After ANY deployment-shape change (compose, Dockerfile, entrypoint, env example, nginx config) you MUST update both `nd.md` §17 and `docs/DEPLOYMENT_VPS.md`. **Never commit `.env.production`** (gitignored at the repo root). All integration `*_MODE` env vars stay `mock` until Prarit confirms each integration's live credentials. The VPS already runs Postzyo / OpenClaw — never run `docker system prune -a`, never reuse their container names, never point the new stack at their volumes, never share their Redis. **Never change Docker container / volume / network names or the `18020` host port without explicit user approval** — the host Nginx config and SSL cert paths assume that exact shape.

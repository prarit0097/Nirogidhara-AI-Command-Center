...
**Current baseline (Phase 8F-Hotfix-3):** Phase 8F-Hotfix-1 (migration `payments.0025_phase8f_hotfix_rename_indexes` + narrow recovery approval path), Phase 8F-Hotfix-2 (Postgres-safe Phase 8F tests), Test Hygiene Hotfix-1 (test-session integration modes pinned to mock in `backend/tests/conftest.py`), and Phase 8F-Hotfix-3 (attempt-level governance recovery command) are shipped. The VPS Phase 8F gate id=1 was recovered/approved on 2026-05-14; after a placeholder execute signoff blocked attempt id=1, Hotfix-3 recovers that attempt back to `approved_for_one_shot_real_mutation` with every locked-False flag still False. **Phase 8F execute was NOT run**; Order `NRG-20435` remains Partial and Payment `PAY-30125` remains Pending. Phase 8F live execute, Phase 7E-Live-B, and Phase 7G-Live remain **NOT approved**. Verification baseline: **2200 backend tests + 82 frontend tests**, green on local SQLite and VPS Postgres baseline plus Hotfix-3 local coverage.

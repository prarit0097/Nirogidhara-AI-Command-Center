...
**Current baseline (Phase 8F Live Execute Reading 1):** Phase 8F is complete as executed + rolled back on the VPS on 2026-05-14. Execute used attempt id=1, gate id=1, source Phase 8E gate id=1, target Order `NRG-20435`, target Payment `PAY-30125`, operator `Prarit Sidana`, and Director signoff window `BEGIN_UTC=2026-05-14T09:32:29Z` to `END_UTC=2026-05-14T09:45:29Z`; result `ok=True`, `status=executed`, Order/Payment temporarily moved to Paid/Paid. Rollback Reading 1 returned `ok=True`, `status=rollback_recorded`, `rollbackId=1`; psql confirmed system state back to Order Partial / Payment Pending, and health returned `{"status": "ok"}`. All locked-False flags stayed False; no provider call, WhatsApp send, customer notification, shipment/AWB, or `Order.state` mutation occurred. The three `PHASE8F_*` flags were passed via runtime env prefix only; `.env.production` was NOT edited. Phase 7E-Live-B and Phase 7G-Live remain **NOT approved**. Verification baseline remains **2200 backend tests + 82 frontend tests**; this docs record is doc-only.

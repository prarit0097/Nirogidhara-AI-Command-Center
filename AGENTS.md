...
12. **Provider-touching execute commands MUST validate `now ∈ [BEGIN_UTC, END_UTC]` from a structured Director sign-off.** The Phase 7D execute-once incident on 2026-05-07 (`order_SmThqpK6sc6Dhs`, attempt id 1, executed at `12:42:46Z`, ~134s before the approved `12:45:00Z → 13:00:00Z` window) shipped because `execute_razorpay_controlled_pilot_test_order` accepted free-text sign-off without parsing structured markers. **No business or customer impact** (every locked-False boolean stayed False, the attempt was rolled back), but the time invariant was violated. Phase 7D-Hotfix-1 fixes this — any future `execute_razorpay_controlled_pilot_test_order` / `execute_single_razorpay_test_order` / future Phase 7E-Live / Phase 7F send / courier command MUST call `apps.saas.utc_window.parse_director_signoff_window` + `validate_execution_window(max_window_seconds=900)` and refuse on missing markers, malformed timestamps, `now < window_start`, `now > window_end`, window length > 15 min, or stale window (`window_start < now - 24h`). No free-text honour-system windows. Phase 7E review windows use `validate_review_window` (max 24h); execute windows use `validate_execution_window` (max 15 min).

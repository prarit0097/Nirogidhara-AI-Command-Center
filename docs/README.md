...
> **Current operational baseline: Phase 15M — Foundation Release Freeze.** Safety shell is frozen at code commit `eefd8b3`; docs / sign-off at `c85a32e`. The canonical operational source of truth is [`../nd.md`](../nd.md) + [`PHASE_15M_DIRECTOR_SIGNOFF_PACK.md`](PHASE_15M_DIRECTOR_SIGNOFF_PACK.md). Next planned work is **Phase 16A — Business MVP Gap Audit** (planning-only, awaiting Director directive). The roadmap docs below cover phases up to the Phase 12D-era baseline plus the Phase 14D → 15M chrome; see `nd.md` head-of-file for current truth.

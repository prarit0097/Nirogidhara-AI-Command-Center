...
> **Current operational UI baseline is Phase 16E — Payment / Logistics Integration Hardening (implemented + pushed; VPS browser verification pending).** New page **`/operations/payment-logistics`** (sidebar "Operations", Wallet icon): a no-side-effect safety banner ("Hardening mode only — does NOT create live payment links, capture/refund, or book Delhivery shipments…"); safety chips (AI Paused / Sandbox OFF / live provider actions Locked / Phase 16E hardening); payment readiness cards for **Razorpay** + **PayU** and a logistics card for **Delhivery** (each showing mode / status pill / configured / live-gate / blocked reasons / safe actions); order-workflow gate rows (payment gate, shipment gate — both live-blocked); a recent-internal-events section; and **no live action button** ("Live actions disabled — Director live gate required."). Loading / error / empty states render; responsive, no horizontal overflow; secrets never shown (presence only). The Phase 16D pages are below.
>
...
>
> **Both pages are internal-only and trigger NO provider / WhatsApp / payment / courier / Vapi / AI call.** The Phase 16B browser-verified surfaces (Leads CRM phone-only duplicate UX + `S.N.` column, Customer 360 hydrated tabs, Orders Pipeline responsive wrapped layout) are unchanged. The Phase 15 safety shell remains **frozen** at code commit `eefd8b3` and untouched. Use [`nd.md`](../nd.md) head-of-file for current truth. The "Historical frontend audit snapshot" below is preserved as the Phase 12D-era reference; any "current" wording inside it is historical, not current. Next planned work: Phase 16E — Payment / Logistics Integration Hardening (separate Director directive required).

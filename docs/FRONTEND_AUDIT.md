...
**Phase 6P baseline.** `/saas-admin` now renders every Phase 6 read-only
section through Phase 6P:
...
Vitest tests | 54 tests today. Phase 6J/6K/6L/6M-0/6M/6N/6O/**6P** assertions in `frontend/src/test/saas-admin.test.tsx` cover render of every new section, absence of forbidden buttons (Execute / Capture / Go Live / Activate / Replay / Disable Kill Switch / Apply Mutation / Mark Paid / Refund / Create Payment Link / Mutate Order / Run MCP Tool / Execute Payment / Apply Payment / Execute Sandbox / Rollback Sandbox), no raw env-var names like `RAZORPAY_KEY_SECRET` in body text, no full Indian phone numbers (`+91XXXXXXXXXX` regex check), and no raw secrets in any rendered preview. Phase 6O specifically asserts the per-row buttons are labelled exactly "Approve Review Only" / "Reject Review" / "Archive Review". Phase 6P specifically asserts the "CLI-only" + "Sandbox ledger only" reminders render and that no execute/rollback buttons exist on the page.

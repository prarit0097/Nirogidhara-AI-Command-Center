...
> **Operational baseline note (Phase 16L — PRODUCTION VERIFIED on the VPS and CLOSED at commit `9d144f5`):** This roadmap contains historical shipped/planned backend gates through the Phase 12D Tier-4 calling baseline. **Current operational baseline is Phase 16L — Scoped Team Member Work Permissions + My Work Queue** (extends `apps.ai_copilot` with 1 additive model `AiWorkboardDepartmentMember` + 7 new endpoints `workboard/my/` + `workboard/my/summary/` + `workboard/my-permissions/` + `workboard/department-members/` GET+POST + `.../<id>/activate/` + `.../deactivate/`; the 6 Phase 16K scoped transitions now run `IsAuthenticated` + service scoped checks; Director/Admin keep full control, a non-admin claims only an unclaimed action with an active dept membership and works only its own assigned actions, assign/reassign + membership-management stay Director/Admin-only; every path is DB-only and never calls a provider / never broadens operations users to admin; backend 24 Phase 16L + 58 regression = 82 passed targeted, frontend 398/398, lint 0, build green, `makemigrations --check` + `manage.py check` clean; **SHIPPED + VPS-verified (2026-06-05)** — HEAD `9d144f5`; migration `ai_copilot.0004_phase16l_workboard_department_member` applied OK on the VPS; `makemigrations --check` / `manage.py check` clean; 16L + 16K/16J/16I tests `[100%]`; healthz 200 OK; Director-led browser validation of `/operations/ai-copilot` + `/operations/pilot-workbench` passed; PRODUCTION VERIFIED + CLOSED at `9d144f5`), on top of **Phase 16K — Department Action Workboard + Ownership / SLA Execution Layer** (PRODUCTION VERIFIED at `efea751`) (extends `apps.ai_copilot` — `AiApprovedAction` gains 8 additive workboard fields + a new `AiActionWorkEvent` model; 12 new endpoints `workboard/` + `workboard/summary/` + `workboard/director-attention/` + `actions/<id>/{assign,claim,start,block,unblock,complete-internal,reassign,notes}/`; `sla_status` computed at read time; every transition is DB-only and never calls a provider; all workboard mutations require director/admin/superuser; backend 23 Phase 16K + 102 regression = 125 passed targeted, frontend 385/385, lint 0, build green, `makemigrations --check` + `manage.py check` clean; **SHIPPED + VPS-verified** — browser validation of the Department action workboard on `/operations/ai-copilot` passed [Assign / Claim / Block (Director attention updated) / Unblock / Complete internal / Add note; external/provider action flags false; no live side effect], `pytest tests/test_phase16k_action_workboard.py` → 23 passed, healthz OK; ADDS migration `ai_copilot.0003_phase16k_action_workboard` → a fresh deploy requires `migrate`), on top of **Phase 16J — AI-Approved Internal Action Queue + Work Execution Bridge** (PRODUCTION VERIFIED at `aa8cf13`) (extends `apps.ai_copilot` with `AiApprovedAction` / `AiApprovedActionEvent` + 7 `/api/v1/ai-copilot/actions...` endpoints; converts an approved Phase 16I suggestion into an internal-only work item a human applies — only an approved suggestion can become an action [else `409`]; applying is DB-only [may create an internal `PilotTask` via the Phase 16H safe service, else records a `result_payload`] and never calls a provider; locked `provider_action_attempted` / `provider_action_taken` / `external_action_allowed` / `external_action_taken` = false; backend 16 new + regression 102 passed targeted, frontend 374/374, lint 0, build green, `makemigrations --check` + `manage.py check` clean; **SHIPPED + VPS-verified** — browser validation of the Approved action queue on `/operations/ai-copilot` passed [approved suggestion → create internal action → PENDING INTERNAL ACTION → Apply → APPLIED INTERNAL; Reject → REJECTED; safety flags false; no live provider action], `pytest tests/test_phase16j_ai_action_queue.py` → 16 passed, healthz OK; ADDS migration `ai_copilot.0002_phase16j_ai_action_queue` → a fresh deploy requires `migrate`), on top of **Phase 16I — AI Copilot Enablement + Human Approval Workflow** (PRODUCTION VERIFIED at `0f91f6b`) (new app `apps.ai_copilot` with `AiCopilotSuggestion` / `AiCopilotReviewEvent` + `/api/v1/ai-copilot/` endpoints; internal/DB-only deterministic AI suggestions with human approval, no live AI/LLM call, locked `provider_call_made` / `external_action_allowed` / `external_action_taken` = false; SHIPPED + VPS-verified — browser validation of `/operations/ai-copilot` passed [suggestion generated → pending review → approved + rejected internally, external-action flags false], targeted suite 19 passed `[100%]`, regression 67 passed `[100%]`, healthz OK; ADDS migration `ai_copilot.0001_phase16i_ai_copilot` → a fresh deploy requires `migrate`), on top of **Phase 16H — Internal Pilot Execution Workbench + Role-Based Task Queues** (extends `apps.pilot` with `PilotTask` / `PilotTaskEvent` + `/api/v1/pilot/` task endpoints; internal/DB-only role-based task queues with `provider_actions_allowed` locked false at every task status; SHIPPED + VPS-verified — browser validation of `/operations/pilot-workbench` passed [pilot plan created/approved, 14 tasks generated, full task lifecycle exercised], targeted suite 19 passed `[100%]`, `makemigrations --check` + `manage.py check` clean, healthz OK; ADDS migration `pilot.0003_phase16h_pilot_execution` → a fresh deploy requires `migrate`), on top of **Phase 16G — Internal Pilot Control Center / Pilot Execution Dashboard** (extends `apps.pilot` with `PilotPlan` / `PilotPlanEvent` / `PilotPlanReview` + `/api/v1/pilot/` plan endpoints; internal/DB-only pilot management with `provider_actions_allowed` locked false at every state; SHIPPED + VPS-verified — browser validation of `/operations/pilot-control` passed, targeted suite 19 passed `[100%]`, `makemigrations --check` + `manage.py check` clean, healthz OK; ADDS migration `pilot.0002_phase16g_pilot_control` → a fresh deploy requires `migrate`), on top of **Phase 16F — Controlled Internal Pilot Readiness + End-to-End Dry Run** (PRODUCTION VERIFIED at `967ed3d`) (new app `apps.pilot`, DB-only `PilotDryRun` / `PilotDecision` + `/api/v1/pilot/` readiness + dry-run endpoints reusing Phase 16E readiness; internal/DB-only, no live provider call; **SHIPPED + VPS-verified** — browser validation of `/operations/pilot-readiness` passed with the gate matrix's payment/shipment/Vapi-AI gates BLOCKED and a BLOCKED dry-run recorded, `migrate --noinput` → "No migrations to apply.", targeted + regression suites `[100%]`, healthz OK), on top of Phase 16E — Payment / Logistics Integration Hardening (PRODUCTION VERIFIED at `36395f6`), Phase 16D — Uploaded Customer Data Campaigns + Calling Lifecycle (verified at `c0be74a`), Phase 16C — Director Daily Briefing + Team Roles UI (verified at `687ef41`), Phase 16B — Customer Lifecycle UI Backbone (verified at `00c3295`), and the Phase 15M Foundation Release Freeze (safety shell frozen at `eefd8b3`). Phase 16N is PRODUCTION VERIFIED on the VPS and CLOSED at commit `8c168db`; Phase 16O is implemented and pushed to origin/main (VPS verification pending); the next planned step is **Phase 16P** (NOT started; separate Director directive required to start). **Phase 16M — Workboard Analytics + SLA Throughput Dashboard** has since shipped read-only/internal-only on top of Phase 16L — it extends `apps.ai_copilot` with NO new model and NO migration (1 read-only `GET /api/v1/ai-copilot/workboard/analytics/` endpoint deriving summary / per-department / per-member / SLA / blocker / throughput-trend analytics purely from existing workboard fields; never calls a provider, mutates a business row, or touches `RuntimeKillSwitch` / `SandboxState`; locked provider/external flags false; backend 15 Phase 16M + 82 regression = 97 passed targeted, frontend 410/410, lint 0, build green, `makemigrations --check` → "No changes detected") and is **PRODUCTION VERIFIED on the VPS and CLOSED at commit `a992207`**. Any Phase 8F / 7E-Live-B / 7G-Live live-execution references remain **historical and locked unless separately approved** in writing by the Director. For current truth read [`../nd.md`](../nd.md) head-of-file.

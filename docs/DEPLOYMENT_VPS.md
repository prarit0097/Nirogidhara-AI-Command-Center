...
> **Current operational baseline: Phase 16G — Internal Pilot Control Center / Pilot Execution Dashboard, IMPLEMENTED + PUSHED to `origin/main` (VPS production verification pending).** Route: `/operations/pilot-control`. Phase 16G extends the additive `apps.pilot` app with 3 new models (`PilotPlan` / `PilotPlanEvent` / `PilotPlanReview`) + migration `pilot.0002_phase16g_pilot_control` (pure CreateModel; no existing table altered) + a DB-only pilot-management layer. **Phase 16G ADDS a migration, so the VPS deploy REQUIRES `migrate` after a pre-deploy backup:**
>
> ```bash

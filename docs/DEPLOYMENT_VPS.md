...
> **Current operational baseline: Phase 16H — Internal Pilot Execution Workbench + Role-Based Task Queues, IMPLEMENTED + PUSHED to `origin/main` (VPS production verification pending).** Route: `/operations/pilot-workbench`. Phase 16H extends the additive `apps.pilot` app with 2 new models (`PilotTask` / `PilotTaskEvent`) + migration `pilot.0003_phase16h_pilot_execution` (pure CreateModel; no existing table altered) + a DB-only role-based task-queue execution layer (`provider_actions_allowed` locked false at every task status including `in_progress`/`done`). **Phase 16H ADDS a migration, so the VPS deploy REQUIRES `migrate` after a pre-deploy backup:**
>
> ```bash

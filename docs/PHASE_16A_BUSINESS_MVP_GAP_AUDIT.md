...
>
> **Phase 16L follow-up status (2026-06-05):** **Phase 16L — Scoped Team Member Work Permissions + My Work Queue is PRODUCTION VERIFIED on the VPS and CLOSED at commit `9d144f5`.** It lets real internal team members safely work their own assigned internal actions (+ claim eligible department work) without broad Director/Admin power — it extends `apps.ai_copilot` with 1 additive model `AiWorkboardDepartmentMember` (`user` / `department` [the 9 Phase 16K teams] / `is_active` / `can_claim` / `can_work` / `can_complete` / `created_by` / timestamps; migration `ai_copilot.0004_phase16l_workboard_department_member`, pure additive) + 7 new endpoints (`workboard/my/`, `workboard/my/summary/`, `workboard/my-permissions/`, `workboard/department-members/` GET+POST, `.../<id>/activate/` + `.../deactivate/`) + a My work queue section on `/operations/ai-copilot`. The 6 Phase 16K scoped transitions now run `IsAuthenticated` + service-level scoped checks; Director/Admin keep full control, a non-admin claims only an unclaimed action with an active dept membership and works only its own assigned actions, assign/reassign + membership-management stay Director/Admin-only, viewers read-only. Every path is internal/DB-only — never calls a provider, never broadens operations users to admin; `provider_action_*` / `external_action_*` stay false (asserted by a defensive test). Backend 24 Phase 16L + 58 regression = 82 passed targeted; frontend +13 (398); lint 0; build green; `makemigrations --check` + `manage.py check` clean. ADDS migration `ai_copilot.0004_phase16l_workboard_department_member` → the VPS deploy requires `migrate`. This lets the workboard move from "Director/Admin does every action" to "scoped team members execute their own internal work safely." VPS production verification PASSED (2026-06-05): migration `ai_copilot.0004_phase16l_workboard_department_member` applied on the VPS; 16L + 16K/16J/16I tests `[100%]`; healthz 200 OK; Director browser validation of `/operations/ai-copilot` + `/operations/pilot-workbench` passed with the safety shell unchanged. Phase 16N is PRODUCTION VERIFIED on the VPS and CLOSED at commit `8c168db`; Phase 16O is implemented and pushed to origin/main (VPS verification pending); the next planned work: **Phase 16P** (NOT started; separate Director directive required). Phase 16M (Workboard Analytics + SLA Throughput Dashboard) is PRODUCTION VERIFIED + CLOSED at `a992207`.
>
> **Phase 16K follow-up status (2026-06-04):** **Phase 16K — Department Action Workboard + Ownership / SLA Execution Layer is PRODUCTION VERIFIED on the VPS and CLOSED at commit `efea751`.** It makes the Phase 16J AI-approved internal actions operational for real internal departments — it extends the existing `apps.ai_copilot` app (`AiApprovedAction` gains 8 additive workboard fields [`department` (9 internal teams), `assignee_user`, `work_status`, `due_at`, `blocker_reason`, `completed_by`, `completed_at`, `last_activity_at`] + a new `AiActionWorkEvent` model; migration `ai_copilot.0003_phase16k_action_workboard`, pure additive) + 12 new endpoints under `/api/v1/ai-copilot/` (`workboard/`, `workboard/summary/`, `workboard/director-attention/`, `actions/<id>/{assign,claim,start,block,unblock,complete-internal,reassign,notes}/`) + a Department action workboard section on `/operations/ai-copilot` (summary cards, filters, Director-attention panel, per-row Assign / Claim / Start / Block / Unblock / Complete internal / Reassign / Add note). `sla_status` is computed at read time. Every transition is internal/DB-only — it never calls a provider; `provider_action_*` / `external_action_*` stay false (asserted by a defensive test). All workboard mutations require director/admin/superuser; reads = auth. Backend 23 Phase 16K + 102 regression = 125 passed targeted; frontend +11 (385); lint 0; build green; `makemigrations --check` + `manage.py check` clean. VPS browser-validated at `/operations/ai-copilot` (Department action workboard + summary cards; Assign / Claim / Block [Director attention updated] / Unblock / Complete internal / Add note all PASSED; external/provider action flags false; no live side effect; healthz OK). ADDS migration `ai_copilot.0003_phase16k_action_workboard` → a fresh VPS deploy requires `migrate` (already applied on the VPS). This closes the loop from "an approved AI suggestion → a safe internal work item" to "teams can own, track, and close that internal work item safely." (Phase 16L has since shipped on top — PRODUCTION VERIFIED + CLOSED at `9d144f5`; the current next-planned work is **Phase 16M**.)
>
> **Phase 16J follow-up status (2026-06-03):** **Phase 16J — AI-Approved Internal Action Queue + Work Execution Bridge is PRODUCTION VERIFIED on the VPS and CLOSED at commit `aa8cf13`.** It extends the existing `apps.ai_copilot` app (2 additive models `AiApprovedAction` / `AiApprovedActionEvent`; migration `ai_copilot.0002_phase16j_ai_action_queue`) + 7 endpoints under `/api/v1/ai-copilot/actions...` + an Approved action queue section on `/operations/ai-copilot` — converting an **approved** Phase 16I suggestion into a tracked internal-only work item (calling follow-up / QA review / pilot task / customer-or-order note / callback / RTO review / payment follow-up / dispatch review / Director review item) a human applies. Only an approved suggestion can become an action (else HTTP 409); applying is DB-only (may create an internal `PilotTask` via the Phase 16H safe service, else records a `result_payload`) and never calls a provider — every action keeps `provider_action_attempted` / `provider_action_taken` / `external_action_allowed` / `external_action_taken` = false (asserted by a defensive test). Backend 16 new + regression 102 passed targeted; frontend +7 (374); lint 0; build green; `makemigrations --check` + `manage.py check` clean. VPS browser-validated at `/operations/ai-copilot` (approved suggestion → create internal action → PENDING INTERNAL ACTION → Apply → APPLIED INTERNAL; Reject → REJECTED; safety flags false; no live provider action; healthz OK). ADDS migration `ai_copilot.0002_phase16j_ai_action_queue` → a fresh VPS deploy requires `migrate` (already applied on the VPS). This bridges approved AI decisions to safe internal execution without enabling any customer-facing/live-provider action. (Phase 16K has since shipped on top — PRODUCTION VERIFIED + CLOSED at `efea751`; the current next-planned work is **Phase 16L**.)
>
> **Phase 16I follow-up status (2026-06-02):** **Phase 16I — AI Copilot Enablement + Human Approval Workflow is PRODUCTION VERIFIED on the VPS and CLOSED at `0f91f6b`** (internal/DB-only — no live AI/LLM call, no live provider action, Phase 15 safety shell untouched; VPS browser validation of `/operations/ai-copilot` passed — suggestion generated → pending review → approved + rejected internally, external-action flags false, targeted suite 19 passed `[100%]`, regression 67 passed `[100%]`, healthz OK). New app `apps.ai_copilot` adds an internal AI copilot / decision-support layer (`/operations/ai-copilot`, 5 endpoints under `/api/v1/ai-copilot/`) that analyzes / drafts / recommends / scores / summarises (lead summary, call priority, call script, objection handling, compliance risk, pilot/director recommendation, WhatsApp/payment/RTO draft) — a human approves before any business action; AI is NOT a live autonomous operator. AI mode is deterministic ("mock"/"sandbox") by default; a live LLM/AI provider is never called this phase. Every suggestion is sanitized + stored with `provider_call_made=false` / `external_action_allowed=false` / `external_action_taken=false`. Backend 19 new tests (155 regression); frontend +10 (367); lint 0; build green; `makemigrations --check` + `manage.py check` clean. ADDS migration `ai_copilot.0001_phase16i_ai_copilot` → VPS deploy requires `migrate`. This adds the decision-support layer above the pilot execution stack without enabling live autonomous AI.
>
> **Phase 16H follow-up status (2026-06-01):** **Phase 16H — Internal Pilot Execution Workbench + Role-Based Task Queues is PRODUCTION VERIFIED on the VPS and CLOSED at `d733cf0`** (internal/DB-only — no live provider call, Phase 15 safety shell untouched; VPS browser validation of `/operations/pilot-workbench` passed — pilot plan created/approved, 14 internal tasks generated, full task lifecycle [Start/Complete/Block/Unblock/Skip] exercised, targeted suite 19 passed `[100%]`, healthz OK). It extends `apps.pilot` with `PilotTask` / `PilotTaskEvent` + 7 endpoints under `/api/v1/pilot/` + the page `/operations/pilot-workbench`, giving the Director the internal execution layer: convert an approved pilot plan into role-based internal task queues (Calling / Confirmation / Dispatch-Warehouse / Delivery-RTO / QA-Compliance / Finance), track task status (`todo → in_progress → done`, plus `blocked`/`unblock`/`skip`/`cancel`), and view per-team execution progress. A transition changes only the internal task record — `provider_actions_allowed` stays false at every status including `in_progress`/`done`. Backend 19 new tests (136 regression); frontend +10 (357); lint 0; build green; `makemigrations --check` + `manage.py check` clean. ADDS migration `pilot.0003_phase16h_pilot_execution` → VPS deploy requires `migrate`. This closes the "no role-based internal execution layer" gap above the Phase 16G control center, without activating any provider.
>
> **Phase 16G follow-up status (2026-06-01):** **Phase 16G — Internal Pilot Control Center / Pilot Execution Dashboard is PRODUCTION VERIFIED on the VPS and CLOSED at `38e8dc8`** (internal/DB-only — no live provider call, Phase 15 safety shell untouched; VPS browser validation of `/operations/pilot-control` passed, targeted suite 19 passed `[100%]`, healthz OK). It extends `apps.pilot` with `PilotPlan` / `PilotPlanEvent` / `PilotPlanReview` + 6 endpoints under `/api/v1/pilot/` + the page `/operations/pilot-control`, giving the Director a pilot-management + observability layer: create / configure / internally approve / start / pause / resume / complete / cancel / review a controlled pilot plan via the status flow `draft → ready_for_review → approved_internal → running_internal → paused → completed | cancelled`. A transition changes only the internal record — `provider_actions_allowed` stays false at every state including `running_internal`. It reuses the Phase 16F readiness gate matrix + Phase 16E payment/logistics readiness for the per-plan gate checklist. Backend 19 new tests (117 regression); frontend +11 (347); lint 0; build green; `makemigrations --check` + `manage.py check` clean. ADDS migration `pilot.0002_phase16g_pilot_control` → VPS deploy requires `migrate`. This closes the "no operational pilot-control/observability layer" gap above the Phase 16F readiness rehearsal, without activating any provider.
>
> **Phase 16F follow-up status (2026-05-30):** **Phase 16F — Controlled Internal Pilot Readiness + End-to-End Dry Run is PRODUCTION VERIFIED on the VPS and CLOSED at `967ed3d`** (internal/DB-only — no live provider call, Phase 15 safety shell untouched). New app `apps.pilot` adds an internal pilot/dry-run command center (`/operations/pilot-readiness`) that reuses the Phase 16E readiness + Claim Vault coverage to build a 12-gate matrix; the Director runs an internal dry-run (recorded BLOCKED on the VPS — correct, because live provider actions remain locked) and a sign-off review (live-provider gate force-locked NOT approved). VPS browser validation passed; `migrate --noinput` → "No migrations to apply."; targeted + regression suites `[100%]`; healthz OK. An observed gate-matrix warning ("WhatsApp live automation blocked — WARNING" / "WhatsApp automation appears enabled — review before pilot.") is a risk to review before any future pilot, NOT a blocker. This is the rehearsal surface for the future Director-gated live pilot; it resolves the "no end-to-end rehearsal" gap without activating any provider.
>

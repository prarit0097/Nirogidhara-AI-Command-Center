...
> **Phase 16O — Director Briefing Snapshot History + Acknowledgement Trail is implemented and pushed to origin/main; VPS production verification pending.** INTERNAL-ONLY history / acknowledgement layer over the Phase 16N Director AI briefing — save / view / open / acknowledge / mark-needs-follow-up / archive / annotate briefings + a review-event trail. Extends `apps.ai_copilot` with 2 additive models (`AiDirectorBriefingSnapshot` + `AiDirectorBriefingSnapshotEvent`) + migration `ai_copilot.0005_phase16o_director_briefing_snapshot` (pure additive). New `apps.ai_copilot.briefing_snapshots` service reuses the Phase 16N briefing service. 8 new endpoints under `/api/v1/ai-copilot/director-briefing/snapshots/` (GET+POST, summary, detail, acknowledge, needs-follow-up, archive, notes, safe-text); read = any authenticated user, save / acknowledge / needs-follow-up / archive / notes = Director/Admin only; unsupported methods → 405. New **Director Briefing History** section on `/operations/ai-copilot` (read-only safety banner, Save-snapshot, status filter, Refresh, summary cards, snapshot list + per-row View details / Acknowledge / Mark needs follow-up / Archive / Add note + inline detail with event history + safe empty state) — no send / payment / courier / call / run-live-AI / resume-AI / auto-execute / send-briefing button. Internal-only / DB-only: saving or transitioning a snapshot reuses ONLY the Phase 16N briefing service; never calls a provider, mutates a business / workboard row, enqueues a Celery job, or touches `RuntimeKillSwitch` / `SandboxState` (asserted by a defensive test); every snapshot preserves the locked flags + `phase15ShellFrozenCommit=eefd8b3`. Verification: backend 19 Phase 16O + 114 regression = 133 passed targeted, frontend 437/437, lint 0, build green; `makemigrations --check` → "No changes detected" (migration `0005` present). **Phase 16O ADDS migration `ai_copilot.0005_phase16o_director_briefing_snapshot` — a fresh VPS deploy requires `migrate`. Phase 16O is implemented and pushed, VPS production verification pending — do NOT mark production verified until a VPS deploy + Director browser validation pass. Phase 16P is NOT started.** Phase 16N (`8c168db`) is the previous verified baseline.
>
> **Phase 16N — Director AI Daily Briefing Real Data Wiring + Safe Recommendation Pack is PRODUCTION VERIFIED on the VPS and CLOSED at commit `8c168db`.** Read-only / internal-only decision layer over the existing Phase 16I–16M workboard data — extends `apps.ai_copilot` with **NO new model and NO migration**: a new `apps.ai_copilot.briefing` module + 3 read-only endpoints `GET /api/v1/ai-copilot/director-briefing/` + `.../summary/` + `.../recommendations/` (`AuthenticatedReadAdminWrite`, GET-only; POST/PATCH/DELETE → 405). Composes the Phase 16M analytics + Phase 16K director-attention queue + pending suggestion/action counts into a deterministic AI briefing (executive summary, attention items, department/member focus, safe internal-only recommendations with `permittedAction` ∈ {internal_review, assign_internal, create_internal_action, review_blocker, no_external_action}, blocked-live-actions, safety snapshot). New **Director AI Briefing** section on `/operations/ai-copilot` (read-only, no live-action button). Never calls a provider, mutates a business/workboard row, enqueues a Celery job, or touches `RuntimeKillSwitch` / `SandboxState`. Verification: backend 17 Phase 16N + 97 regression = 114 passed targeted, frontend 423/423, lint 0, build green; `makemigrations --check` → "No changes detected" (NO migration) + `manage.py check` clean. **Phase 16N is PRODUCTION VERIFIED on the VPS and CLOSED at commit `8c168db`. Phase 16O is implemented and pushed to origin/main (VPS verification pending); Phase 16P is NOT started.** Phase 16M (`a992207`) is the previous verified baseline.
>
> **Previous verified baseline: Phase 16M — Workboard Analytics + SLA Throughput Dashboard, PRODUCTION VERIFIED on the VPS and CLOSED at commit `a992207`.** Read-only / internal-only analytics over the existing Phase 16J/16K/16L workboard — extends `apps.ai_copilot` with **NO new model and NO migration**: 1 read-only service `get_workboard_analytics(window_days=14)` + 1 read-only endpoint `GET /api/v1/ai-copilot/workboard/analytics/` (`AuthenticatedReadAdminWrite`, GET-only; POST/PATCH/DELETE → 405). Derives summary / per-department / per-member / SLA / blocker / daily-throughput-trend analytics purely from existing fields; SLA reuses Phase 16K `compute_sla_status` at read time. New **Workboard Analytics + SLA Throughput** section on `/operations/ai-copilot` — read-only, no mutation/provider button. Never calls a provider, mutates a business row, enqueues a business Celery job, or touches `RuntimeKillSwitch` / `SandboxState`. Verification: backend 15 Phase 16M + 82 regression = 97 passed (targeted); `makemigrations --check` → "No changes detected" (**NO migration**) + `manage.py check` clean; frontend 410/410; lint 0 errors; build green. **Phase 16M is PRODUCTION VERIFIED on the VPS and CLOSED at commit `a992207` (a fresh deploy needs no `migrate` for Phase 16M). Phase 16N is PRODUCTION VERIFIED on the VPS and CLOSED at commit `8c168db`; Phase 16O is implemented and pushed to origin/main (VPS verification pending); Phase 16P is NOT started.** Phase 16L (`9d144f5`) is the previous verified baseline.
>
> **Phase 16L — Scoped Team Member Work Permissions + My Work Queue, PRODUCTION VERIFIED on the VPS and CLOSED at commit `9d144f5`.** Phase 16L lets internal team members safely work their own assigned internal actions (+ claim eligible department work) without broad Director/Admin power: it extends `apps.ai_copilot` with 1 additive model `AiWorkboardDepartmentMember` (`user` / `department` [the 9 Phase 16K teams] / `is_active` / `can_claim` / `can_work` / `can_complete` / `created_by` / timestamps); migration `ai_copilot.0004_phase16l_workboard_department_member` (pure additive). Scoped rules via `services.can_user_work_action` / `can_user_claim_action`: Director/Admin/Superuser keep full control; a non-admin claims an unclaimed non-terminal action only with an active dept membership (`can_claim=true`); the assigned user can start/block/unblock/complete-internal/add-note their own action (per-dept `can_work` / `can_complete=false` further restricts); assign/reassign + membership-management stay Director/Admin-only; viewers read-only. 7 new endpoints (`workboard/my/`, `workboard/my/summary/`, `workboard/my-permissions/`, `workboard/department-members/` GET+POST, `.../<id>/activate/` + `.../deactivate/`); the 6 Phase 16K scoped transitions now run `IsAuthenticated` + service scoped checks; workboard responses carry per-action `permissions` booleans; new **My work queue** section on `/operations/ai-copilot`. Every path is internal/DB-only and never calls a provider, never broadens operations users to admin; `provider_action_*` / `external_action_*` stay false (asserted by a defensive test). Verification: backend 24 Phase 16L + 58 regression = 82 passed (targeted); `makemigrations --check` + `manage.py check` clean; frontend 398/398; lint 0 errors; build green. **It ADDS migration `ai_copilot.0004_phase16l_workboard_department_member` (applied on the VPS). VPS production verification PASSED (2026-06-05): HEAD `9d144f5`; migration applied OK; `makemigrations --check` / `manage.py check` clean; 16L + 16K/16J/16I tests `[100%]`; healthz 200 OK; Director browser validation of `/operations/ai-copilot` (My work queue + workboard) + `/operations/pilot-workbench` passed with the safety shell unchanged. See the Scoped Work Permissions + My Work Queue (Phase 16L) section below. Phase 16L is CLOSED. Next planned work is Phase 16P — NOT started; requires a separate written Director directive.** Phase 16K (`efea751`) is the previous verified baseline.
>
> **Phase 16K — Department Action Workboard + Ownership / SLA Execution Layer, PRODUCTION VERIFIED on the VPS and CLOSED at commit `efea751`.** Phase 16K makes the Phase 16J AI-approved internal actions operational for real internal departments: it extends the existing `apps.ai_copilot` app (no second action queue) — `AiApprovedAction` gains 8 additive workboard fields (`department` [9 internal teams], `assignee_user`, `work_status`, `due_at`, `blocker_reason`, `completed_by`, `completed_at`, `last_activity_at`) + a new model `AiActionWorkEvent`; migration `ai_copilot.0003_phase16k_action_workboard` (pure additive); `sla_status` is computed at read time. 12 new endpoints (`workboard/`, `workboard/summary/`, `workboard/director-attention/`, `actions/<id>/{assign,claim,start,block,unblock,complete-internal,reassign,notes}/`) + a Department action workboard section on `/operations/ai-copilot`. Every transition is internal/DB-only and never calls a provider; `provider_action_*` / `external_action_*` stay false (asserted by a defensive test). All workboard mutations require director/admin/superuser; reads = auth. Verification: backend 23 Phase 16K + 102 regression = 125 passed (targeted); `makemigrations --check` + `manage.py check` clean; frontend 385/385; lint 0 errors; build green. **VPS production verification PASSED:** browser validation of the Department action workboard on `/operations/ai-copilot` — Assign / Claim / Block (Director attention updated) / Unblock / Complete internal / Add note all PASSED; external action taken=false / provider action taken=false; safety shell unchanged; no live provider / customer-facing action; `pytest tests/test_phase16k_action_workboard.py` → 23 passed; healthz OK. It ADDS migration `ai_copilot.0003_phase16k_action_workboard`, so a fresh VPS deploy requires `migrate` (already applied on the VPS). See the AI-Approved Internal Action Queue + Department Workboard (Phase 16J + 16K) section below. **Phase 16K is CLOSED. (Historical close-state: Phase 16L was next at 16K close — it has since shipped and is PRODUCTION VERIFIED + CLOSED at `9d144f5`; the current next-planned work is Phase 16P.)** Phase 16J (`aa8cf13`) is the previous verified baseline.
>
> **Phase 16J — AI-Approved Internal Action Queue + Work Execution Bridge, PRODUCTION VERIFIED on the VPS and CLOSED at commit `aa8cf13`.** Phase 16J extends the existing `apps.ai_copilot` app (2 additive models `AiApprovedAction` / `AiApprovedActionEvent`, migration `ai_copilot.0002_phase16j_ai_action_queue`, 7 endpoints under `/api/v1/ai-copilot/actions...`, Approved action queue section on `/operations/ai-copilot`) to convert an **approved** Phase 16I AI suggestion into an internal-only work item that a human applies. Only an approved suggestion can become an action (else HTTP 409); applying is DB-only (may create an internal `PilotTask` via the Phase 16H safe service, else records a `result_payload`) and never calls a provider — every action keeps `provider_action_attempted` / `provider_action_taken` / `external_action_allowed` / `external_action_taken` = false (asserted by a defensive test). Verification: backend 16 Phase 16J + regression 102 passed (targeted); `makemigrations --check` + `manage.py check` clean; frontend 374/374; lint 0 errors; build green. **VPS production verification PASSED:** browser validation of the Approved action queue on `/operations/ai-copilot` — an approved suggestion created an internal action (PENDING INTERNAL ACTION → Apply → APPLIED INTERNAL; Reject → REJECTED); safety flags stayed false; safety shell unchanged; no live provider / customer-facing action; `migrate --noinput` → "No migrations to apply"; `pytest tests/test_phase16j_ai_action_queue.py` → 16 passed; `GET /api/healthz/` OK. It ADDS migration `ai_copilot.0002_phase16j_ai_action_queue`, so a fresh VPS deploy requires `migrate` (already applied on the VPS). See the AI Copilot Center (Phase 16I + 16J) section below. **Phase 16J is CLOSED. (Historical close-state: Phase 16K was next at 16J close — it has since shipped and is PRODUCTION VERIFIED + CLOSED at `efea751`; the current next-planned work is Phase 16P.)** Phase 16I (`0f91f6b`) is the previous verified baseline.
>
> **Phase 16I — AI Copilot Enablement + Human Approval Workflow, PRODUCTION VERIFIED on the VPS and CLOSED (commit `0f91f6b`).** Phase 16I adds an internal AI copilot / decision-support layer (new app `apps.ai_copilot`, page `/operations/ai-copilot`) that analyzes / drafts / recommends / scores / summarises — a human approves before any business action; AI is NOT a live autonomous operator. AI mode is deterministic ("mock"/"sandbox") by default; a live LLM/AI provider is NEVER called this phase. Every suggestion carries `provider_call_made=false` / `external_action_allowed=false` / `external_action_taken=false`. **VPS proof:** browser validation of `/operations/ai-copilot` (AI Copilot Center; sidebar AI Copilot; safety shell AI Paused / Sandbox OFF / Sync Live; AI Mode mock / Live Autonomous Locked / Live Provider unavailable / Provider disabled / Human Approval Required / Provider Call None; suggestion generated → pending review → approved internally + another rejected; external action allowed/taken + provider call false; no live side effect); `pytest tests/test_phase16i_ai_copilot.py` → 19 passed `[100%]`; regression (16H/16G/16F/16E) → 67 passed `[100%]`; `makemigrations --check` + `manage.py check` clean; `GET /api/healthz/` → `{"status": "ok", "service": "nirogidhara-backend"}`. It ADDS migration `ai_copilot.0001_phase16i_ai_copilot`, so a fresh VPS deploy requires `migrate` (already applied on the VPS). See the **AI Copilot Center (Phase 16I)** section below. Phase 16H (`d733cf0`) is the previous verified baseline.
>
> **Phase 16H — Internal Pilot Execution Workbench + Role-Based Task Queues, PRODUCTION VERIFIED on the VPS and CLOSED (commit `d733cf0`).** Phase 16H extends the existing `apps.pilot` app with a Director/Admin execution workbench (`/operations/pilot-workbench`) — convert an approved internal pilot plan into role-based internal task queues (Calling / Confirmation / Dispatch-Warehouse / Delivery-RTO / QA-Compliance / Finance), track task status (`todo → in_progress → done`, plus `blocked`/`unblock`/`skip`/`cancel`), and view per-team execution progress, all DB-only with `provider_actions_allowed` locked false at every task status including `in_progress`/`done`. **VPS proof:** browser validation of `/operations/pilot-workbench` (title Internal Pilot Execution Workbench; sidebar Pilot Workbench; safety shell AI Paused / Sandbox OFF / Sync Live / Live Provider Actions Locked; pilot plan created + approved; 14 internal tasks generated across the six team queues [0/14 done]; task lifecycle Start→IN PROGRESS / Complete→DONE / Block→BLOCKED / Unblock→IN PROGRESS / Skip→SKIPPED; no live side effect); `makemigrations --check` → "No changes detected"; `manage.py check` → "0 issues"; `pytest tests/test_phase16h_pilot_execution.py --tb=no -q` → 19 passed `[100%]`; `GET /api/healthz/` → `{"status": "ok", "service": "nirogidhara-backend"}`. It ADDS migration `pilot.0003_phase16h_pilot_execution`, so a fresh VPS deploy requires `migrate` (already applied on the VPS). See the **Internal Pilot Execution Workbench (Phase 16H)** section below. Phase 16G (`38e8dc8`) is the previous verified baseline.
>
...
### AI Copilot Center (Phase 16I — PRODUCTION VERIFIED on the VPS and CLOSED at `0f91f6b`)

**VPS production verification (PASSED — Phase 16I CLOSED at `0f91f6b`):** Director-led browser validation of `/operations/ai-copilot` showed the **AI Copilot Center** page, the sidebar item **AI Copilot**, and the safety shell visible + unchanged (**AI Paused / Sandbox OFF / Sync Live**) with **AI Mode mock / Live Autonomous Locked / Live Provider unavailable / Provider disabled / Human Approval Required / Provider Call None**. An AI suggestion was generated internally and appeared as **awaiting review**, was **approved** internally, and another was **rejected** internally — the external-action flags stayed false throughout (**external action allowed: false, external action taken: false, provider call: false**); **no live WhatsApp / Razorpay / PayU / Delhivery / Vapi / live-AI-provider action was triggered.** Terminal: `python manage.py makemigrations --check --dry-run` → "No changes detected"; `python manage.py check` → "System check identified no issues (0 silenced)"; `python -m pytest tests/test_phase16i_ai_copilot.py --tb=no -q` → 19 passed `[100%]`; regression (`tests/test_phase16h_pilot_execution.py tests/test_phase16g_pilot_control.py tests/test_phase16f_pilot_readiness.py tests/test_phase16e_payment_logistics.py`) → 67 passed `[100%]`; `GET https://ai.nirogidhara.com/api/healthz/` → `{"status": "ok", "service": "nirogidhara-backend"}`.

Phase 16I adds an internal **AI copilot / decision-support** layer: the AI can analyze, draft, recommend, score, and summarise — but **a human approves before any business mutation or customer-facing action.** It is NOT a live autonomous operator. New additive app `apps.ai_copilot` (2 models `AiCopilotSuggestion` / `AiCopilotReviewEvent`; migration `ai_copilot.0001_phase16i_ai_copilot`) + 5 endpoints under `/api/v1/ai-copilot/` + the page `/operations/ai-copilot`. **AI mode is SAFE/deterministic by default** — suggestions are generated by deterministic ("mock", or "sandbox" when sandbox is ON) generators; **a live LLM/AI provider is NEVER called this phase** (`AI_PROVIDER` defaults `disabled`; a configured provider is reported `live_gated` but never invoked — live AI is a future, separately-approved Director gate). **Every Phase 16I path is internal/DB-only — it NEVER sends WhatsApp/Meta Cloud, places a Vapi call, calls Razorpay/PayU live or creates a payment link, books a live Delhivery AWB, calls any AI/LLM provider, auto-confirms/cancels an order, auto-applies a discount, enqueues a business Celery job, or mutates `RuntimeKillSwitch` / `SandboxState`** (asserted by a defensive test). Every suggestion is stored sanitized (phones masked to last-4, no full address, no raw payload) with `provider_call_made=false` / `external_action_allowed=false` / `external_action_taken=false`.

**Open the AI Copilot Center — `/operations/ai-copilot` (sidebar → Operations, Bot icon):**
- A no-side-effect safety banner: "Internal copilot only — no live autonomous execution."
- AI safety status chips: AI Paused / Sandbox OFF / AI mode (mock/sandbox) / Live autonomous Locked / Live provider (live_gated|unavailable, never invoked) / Provider / Human approval Required / Provider call None.
- A **Generate AI suggestion** form (type + source + optional source id + optional compliance text) and a **suggestions queue** with risk-flag pills.

**How to generate a safe AI suggestion:** pick a suggestion type (`lead_summary` / `call_priority` / `call_script` / `objection_handling` / `compliance_risk` / `pilot_recommendation` / `director_briefing` / `whatsapp_draft` / `payment_followup_draft` / `rto_rescue_draft`), a source (`lead` / `customer` / `order` / `imported_queue_item` / `pilot_plan` / `pilot_task` / `manual`) + optional source id, then **Generate suggestion** → `POST /api/v1/ai-copilot/suggestions/generate/`. The suggestion is generated deterministically and lands in the queue with status `pending_review`.

**How to approve / reject a suggestion (internal only):** on a `pending_review` card use **Approve (internal)** / **Reject** / **Mark applied (internal only)** → `POST /api/v1/ai-copilot/suggestions/<id>/review/` with `{action: approve|reject|comment|apply_internal}`. `apply_internal` is an internal acknowledgement only — it NEVER authorises an external action; the locked contract holds after every review.

**How to verify no external side effects:** the entire flow hits only `/api/v1/ai-copilot/` endpoints (DB-only). The defensive test `tests/test_phase16i_ai_copilot.py` patches `queue_template_message` / `send_freeform_text_message` / `trigger_call_for_lead` / `razorpay_client.create_payment_link` / `delhivery_client.create_awb` and asserts all stay `assert_not_called` over all 11 suggestion types across generate → approve → apply_internal, with `Lead`/`Customer`/`Order`/`Payment` counts + sandbox + kill-switch unchanged, and every stored suggestion keeping the locked contract.

**Permissions:** generate + review require director / admin / superuser; status + list + detail reads require auth (read-only viewers may view). Non-admin generate/review → 403.

**Deploy (Phase 16I — ADDS migration `ai_copilot.0001_phase16i_ai_copilot`, so `migrate` is required):**
```bash
cd /opt/nirogidhara-command
...
docker compose -f docker-compose.prod.yml up -d --build && sleep 20
...
docker compose -f docker-compose.prod.yml exec backend python -m pytest tests/test_phase16i_ai_copilot.py --tb=no -q
docker compose -f docker-compose.prod.yml restart nginx
curl -sS https://ai.nirogidhara.com/api/healthz/
```

**Validation checklist (Phase 16I):**
1. Hard refresh; safety shell unchanged (AI Paused / Sandbox OFF / Sync Live).
2. `/operations/ai-copilot` loads; safety banner + AI status chips render; live autonomous execution shown Locked.
3. Generate a safe suggestion (e.g. director_briefing or compliance_risk); it appears in the queue with `provider call: None` and `external action allowed/taken: false`.
4. Approve one suggestion; reject another; status updates internally.
5. No live send/call/payment/courier button exists anywhere.
6. No WhatsApp / payment / courier / Vapi / AI-provider side effect observed.
7. Phase 16H / 16G / 16E pages still work (Pilot Workbench, Pilot Control, Payment & Logistics).

**Rollback (Phase 16I):** additive new app. For code rollback, `git revert` the Phase 16I commit + redeploy; the `ai_copilot.0001_phase16i_ai_copilot` migration creates two new tables that are unused after revert — leave them (no FK from existing tables points into them) or drop manually; no existing table is altered. Prefer a forward-fix migration over manually dropping production tables. For a docs-only finalization commit, rollback is `git revert` of that docs commit (no runtime/migration impact).

### Scoped Work Permissions + My Work Queue (Phase 16L — PRODUCTION VERIFIED on the VPS and CLOSED at `9d144f5`)

Phase 16L lets **real internal team members** safely work their own assigned internal actions (and claim eligible department work) **without broad Director/Admin power**. It **extends `apps.ai_copilot`** with 1 additive model `AiWorkboardDepartmentMember` (migration `ai_copilot.0004_phase16l_workboard_department_member`, pure `CreateModel`). Department choices are the same 9 Phase 16K internal teams. Per-membership flags: `can_claim` / `can_work` / `can_complete`; partial-unique active membership per `(user, department)`.

**Permission model (decided centrally in `services.can_user_work_action` / `can_user_claim_action`):**
- Director / Admin / Superuser → full control (claim/start/block/unblock/complete/note/assign/reassign).
- A non-admin may **claim** an action only when it is non-terminal, has a department, is currently unclaimed, and the user holds an **active** membership for that department with `can_claim=true`.
- The directly **assigned** user may `start` / `block` (reason required) / `unblock` / `complete-internal` / `add note` their own action; a per-department `can_work=false` blocks start/block/unblock/note, and `can_complete=false` blocks completion.
- **Assign / reassign + department-membership management stay Director/Admin-only.**
- Viewers / read-only users cannot mutate anything; a non-assignee / non-member → `403 work_permission_denied`.
- Terminal queue actions (Phase 16J `status` rejected/cancelled) and closed work items (completed/rejected/cancelled) stay protected exactly as Phase 16K.

**My Work surface — `/operations/ai-copilot` (My work queue section):**
- Safety copy: "Team members can only update internal workboard records they are assigned to or allowed to claim by department membership. These actions never send WhatsApp, create payment links, book shipments, call customers, invoke Vapi, or call a live AI provider."
- Summary cards: My total / Assigned / In progress / Blocked / Due soon / Overdue / Completed.
- Per-row, permission-gated **Start / Block (reason) / Unblock / Complete internal / Add note** (only buttons the API says the user may use are shown). Assign/reassign are hidden for non-admins.

**Endpoints** (under `/api/v1/ai-copilot/`): `GET workboard/my/`, `GET workboard/my/summary/`, `GET workboard/my-permissions/`, `GET + POST workboard/department-members/`, `POST workboard/department-members/<id>/activate/`, `POST workboard/department-members/<id>/deactivate/`. The 6 Phase 16K scoped transitions (`actions/<id>/{claim,start,block,unblock,complete-internal,notes}/`) now accept any authenticated user and enforce scope in the service. Membership-management endpoints are Director/Admin-only (even on GET).

**Deploy (Phase 16L — ADDS migration `ai_copilot.0004_phase16l_workboard_department_member`, so `migrate` is required):**
```bash
cd /opt/nirogidhara-command
...
git pull origin main
docker compose -f docker-compose.prod.yml up -d --build && sleep 25
...
docker compose -f docker-compose.prod.yml exec backend python manage.py makemigrations --check --dry-run   # → No changes detected
docker compose -f docker-compose.prod.yml exec backend python manage.py check
docker compose -f docker-compose.prod.yml exec backend python -m pytest tests/test_phase16l_workboard_permissions.py --tb=no -q
docker compose -f docker-compose.prod.yml restart nginx
curl -sS https://ai.nirogidhara.com/api/healthz/
```

**Validation checklist (Phase 16L — PASSED on the VPS 2026-06-05; rerun after any redeploy):**
1. Hard refresh; safety shell unchanged (AI Paused / Sandbox OFF / Sync Live / AI Mode mock / Live Autonomous Locked).
2. `/operations/ai-copilot` loads; Approved action queue + Department action workboard + My work queue all render.
3. As Director/Admin: add a team member to a department (`POST workboard/department-members/`), assign a workboard item to a department/user.
4. As that scoped team member (if a login is available): view My Work; claim eligible work; start; block with reason; unblock; complete internal; add note.
5. Confirm a non-assigned / non-member user cannot mutate (403); confirm viewer cannot mutate.
6. Confirm permission-based buttons hide/disable correctly (no assign/reassign for non-admins).
7. Confirm `provider_action_taken=false` / `external_action_taken=false`; no WhatsApp / payment / courier / Vapi / live-AI side effect.
8. Phase 16K Department workboard + Phase 16H Pilot Workbench still work.

**Rollback (Phase 16L):** additive (1 new table). `git revert` the Phase 16L commit + redeploy; the `ai_copilot.0004_phase16l_workboard_department_member` migration adds a new table that is unused after revert (no existing FK points into it) — leave it or prefer a forward-fix migration. Reverting also restores the Phase 16K admin-only permission on the scoped transition endpoints. Do NOT manually drop production tables without a verified backup + Director approval.

### Department Action Workboard (Phase 16K — PRODUCTION VERIFIED on the VPS and CLOSED at `efea751`)

Phase 16K makes the Phase 16J AI-approved internal actions **operational for real internal departments**. It **extends `apps.ai_copilot`** (no second action queue): `AiApprovedAction` gains 8 additive workboard fields + a new `AiActionWorkEvent` model; migration `ai_copilot.0003_phase16k_action_workboard` (pure `CreateModel` + `AddField` + `AddIndex`). Departments (9 internal teams): `calling` / `confirmation` / `qa_compliance` / `finance_accounts` / `dispatch_warehouse` / `delivery_rto` / `director_office` / `data_ops` / `ai_governance`. Work status flow: `unassigned → assigned → in_progress → blocked → completed_internal` (plus `rejected` / `cancelled`). `sla_status` (`no_due_date` / `on_track` / `due_soon` / `overdue`) is computed at read time from `due_at` (no DB column).

**Open the Department action workboard — `/operations/ai-copilot` (Department action workboard section):**
- A no-side-effect safety banner: "Completing or updating a workboard action never sends WhatsApp, creates payment links, books shipments, calls customers, invokes Vapi, or calls a live AI provider. This is an internal execution tracker only."
- **Summary cards**: Total / Unassigned / Assigned / In progress / Blocked / Overdue / Completed / Director attention.
- **Filters**: department, work status, priority, SLA status, search.
- **Director attention panel**: blocked + overdue + unassigned-high/urgent-priority actions, each with a reason.
- Per-row controls: **Assign** (pick department) / **Claim** / **Start** / **Block** (requires reason) / **Unblock** / **Complete internal** / **Reassign** / **Add note**.

**Endpoints** (under `/api/v1/ai-copilot/`): `GET workboard/`, `GET workboard/summary/`, `GET workboard/director-attention/`, `POST actions/<id>/assign/`, `POST actions/<id>/claim/`, `POST actions/<id>/start/`, `POST actions/<id>/block/`, `POST actions/<id>/unblock/`, `POST actions/<id>/complete-internal/`, `POST actions/<id>/reassign/`, `POST actions/<id>/notes/`.

**Guards / safety:** a queue-terminal action (Phase 16J `status` rejected/cancelled) cannot be worked; a closed work item (completed/rejected/cancelled) cannot be re-opened; blocking requires a reason; every transition re-asserts `provider_action_*` / `external_action_*` = false and writes an `AiActionWorkEvent`. **Permissions:** all workboard mutations require director/admin/superuser; reads require auth (read-only viewers may view).

**Deploy (Phase 16K — ADDS migration `ai_copilot.0003_phase16k_action_workboard`, so `migrate` is required):**
```bash
cd /opt/nirogidhara-command
...
docker compose -f docker-compose.prod.yml exec backend python manage.py makemigrations --check --dry-run   # → No changes detected
docker compose -f docker-compose.prod.yml exec backend python manage.py check
docker compose -f docker-compose.prod.yml exec backend python -m pytest tests/test_phase16k_action_workboard.py --tb=no -q
docker compose -f docker-compose.prod.yml restart nginx
curl -sS https://ai.nirogidhara.com/api/healthz/
```

**Validation checklist (Phase 16K — PASSED on the VPS; rerun after any redeploy):**
1. Hard refresh; safety shell unchanged (AI Paused / Sandbox OFF / Sync Live).
2. `/operations/ai-copilot` loads; both the Approved action queue and the Department action workboard sections render.
3. Use an existing applied internal action (or approve a suggestion → create an action), then **Assign** it to a department; it shows `assigned`.
4. **Start** → in progress; **Block** with a reason → blocked; **Unblock** → in progress; **Complete internal** → completed.
5. Summary cards + Director attention update; blocked / overdue / unassigned-high-priority items surface.
6. No live send/call/payment/courier button exists; `provider_action_taken` / `external_action_taken` stay false.
7. Phase 16J Approved action queue + Phase 16H Pilot Workbench still work.

**Rollback (Phase 16K):** additive (extends an existing app with new columns + 1 new table). For code rollback, `git revert` the Phase 16K commit + redeploy; the `ai_copilot.0003_phase16k_action_workboard` migration adds nullable columns + a new table that are unused after revert — leave them (no existing FK points into the new table) or prefer a forward-fix migration. Do NOT manually drop production columns/tables without a verified backup + Director approval.

### AI-Approved Internal Action Queue (Phase 16J — PRODUCTION VERIFIED on the VPS and CLOSED at `aa8cf13`)

Phase 16J is the **work execution bridge** on top of the Phase 16I copilot: it converts an **approved** AI suggestion into a tracked internal-only work item that a human applies. It **extends the existing `apps.ai_copilot` app** (2 additive models `AiApprovedAction` / `AiApprovedActionEvent`; migration `ai_copilot.0002_phase16j_ai_action_queue`) + 7 endpoints under `/api/v1/ai-copilot/` + an **Approved action queue** section on `/operations/ai-copilot`. Action types (all internal-only): `create_calling_followup_task` / `create_qa_review_task` / `create_pilot_task` / `create_customer_note` / `create_order_note` / `create_callback_item` / `create_rto_review_task` / `create_payment_followup_task` / `create_dispatch_review_task` / `create_director_review_item`. Status flow: `pending_internal_action → applied_internal | rejected | cancelled | failed`. **Only an approved suggestion can become an action** (a draft / pending / rejected suggestion → HTTP 409). **Applying is DB-only** — it may create an internal `PilotTask` via the Phase 16H safe service (when the source is a resolvable pilot plan) or record an internal `result_payload`; it NEVER sends WhatsApp/Meta Cloud, places a Vapi call, calls Razorpay/PayU live or creates a payment link, books a live Delhivery AWB, calls any AI/LLM provider, auto-confirms/cancels an order, auto-applies a discount, enqueues a business Celery job, or mutates `RuntimeKillSwitch` / `SandboxState`. Every action keeps `provider_action_attempted` / `provider_action_taken` / `external_action_allowed` / `external_action_taken` = false (asserted by a defensive test across the full create → apply flow over 9 safe action types).

**Open the Approved action queue — `/operations/ai-copilot` (Approved action queue section):**
- A no-side-effect safety banner: "Applying internal actions does not send WhatsApp, create payment links, book shipments, call customers, or invoke live AI providers."
- An **approved-suggestions list** with a per-suggestion **Create action** control (choose an action type).
- The **action queue** with per-action **Apply (internal)** / **Reject** / **Cancel** controls.

**How to create + apply an internal action:** on an approved suggestion card pick an action type then **Create action** → `POST /api/v1/ai-copilot/actions/from-suggestion/` (409 if the suggestion is not approved, 400 on an invalid action type). On a `pending_internal_action` card use **Apply (internal)** / **Reject** / **Cancel** → `POST /api/v1/ai-copilot/actions/<id>/{apply,reject,cancel}/`. Apply materialises an internal `PilotTask` (pilot-plan source) or records a `result_payload` and flips status to `applied_internal`; the locked contract holds after apply.

**Endpoints:** `GET actions/` (queue list), `GET actions/summary/`, `POST actions/from-suggestion/`, `GET actions/<id>/`, `POST actions/<id>/apply/`, `POST actions/<id>/reject/`, `POST actions/<id>/cancel/`. `actions/summary/` + `actions/from-suggestion/` are registered before the dynamic `actions/<id>/`.

**Permissions:** create / apply / reject / cancel require director / admin / superuser; reads require auth.

**Deploy (Phase 16J — ADDS migration `ai_copilot.0002_phase16j_ai_action_queue`, so `migrate` is required):**
```bash
cd /opt/nirogidhara-command
...
docker compose -f docker-compose.prod.yml exec backend python manage.py check
docker compose -f docker-compose.prod.yml exec backend python -m pytest tests/test_phase16j_ai_action_queue.py --tb=no -q
docker compose -f docker-compose.prod.yml restart nginx
curl -sS https://ai.nirogidhara.com/api/healthz/
```

**Validation checklist (Phase 16J — PASSED on the VPS; rerun after any redeploy):**
1. Hard refresh; safety shell unchanged (AI Paused / Sandbox OFF / Sync Live).
2. `/operations/ai-copilot` loads; the Approved action queue section + safety copy render.
3. Approve a suggestion, then create an action from it; it appears in the queue as `pending_internal_action` with `external action allowed/taken: false`.
4. Apply the action; status → `applied_internal`; an internal task/result is recorded; no provider button exists.
5. Reject / cancel another action; status updates internally.
6. No WhatsApp / payment / courier / Vapi / AI-provider side effect observed.
7. Phase 16I / 16H / 16G / 16E pages still work.

**Rollback (Phase 16J):** additive (extends an existing app with 2 new tables). For code rollback, `git revert` the Phase 16J commit + redeploy; the `ai_copilot.0002_phase16j_ai_action_queue` migration creates two new tables that are unused after revert — leave them (no FK from existing tables points into them) or drop manually; no existing table is altered. Prefer a forward-fix migration over manually dropping production tables.

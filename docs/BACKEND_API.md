...
> **Operational baseline (Phase 16O — implemented and pushed; VPS production verification pending).** Current operational baseline is **Phase 16O — Director Briefing Snapshot History + Acknowledgement Trail** (internal-only history / acknowledgement layer over the Phase 16N briefing; extends `apps.ai_copilot` with **2 additive models** `AiDirectorBriefingSnapshot` + `AiDirectorBriefingSnapshotEvent` + **migration `ai_copilot.0005_phase16o_director_briefing_snapshot`** [pure `CreateModel` + `AddIndex`, additive]; new `apps.ai_copilot.briefing_snapshots` service reuses the Phase 16N briefing service; **8 new endpoints** under `/api/v1/ai-copilot/director-briefing/snapshots/` [`GET`+`POST`, `GET summary/`, `GET <id>/`, `POST <id>/{acknowledge,needs-follow-up,archive,notes}/`, `GET <id>/safe-text/`]; read = any authenticated user, save + acknowledge + needs-follow-up + archive + notes = Director/Admin only, unsupported methods → 405; never calls a provider, mutates a business / workboard row, enqueues a Celery job, or touches `RuntimeKillSwitch` / `SandboxState`; every snapshot preserves the locked flags; backend 19 Phase 16O + 114 regression = 133 passed targeted, frontend 437/437, lint 0, build green, `makemigrations --check` → "No changes detected" [migration `0005` present]; **ADDS migration `ai_copilot.0005_phase16o_director_briefing_snapshot` — a fresh VPS deploy requires `migrate`; implemented + pushed, VPS production verification pending; Phase 16P is NOT started; do NOT mark production verified until a VPS deploy + Director browser validation pass**), on top of **Phase 16N — Director AI Daily Briefing Real Data Wiring + Safe Recommendation Pack** (PRODUCTION VERIFIED + CLOSED at `8c168db`; read-only / internal-only; extends `apps.ai_copilot` with **NO new model and NO migration** — a new `apps.ai_copilot.briefing` module + **3 read-only endpoints** **`GET /api/v1/ai-copilot/director-briefing/`** + `.../summary/` + `.../recommendations/` [`AuthenticatedReadAdminWrite`, GET-only, `?windowDays=N` clamped 1–30; POST/PATCH/DELETE → **405**]; composes the Phase 16M `get_workboard_analytics` + the Phase 16K `get_director_attention_queue` + pending suggestion/action counts into a deterministic Director AI briefing [briefingStatus / executiveSummary / attentionItems / departmentSummary / memberSummary / safeRecommendations with `permittedAction` ∈ {internal_review, assign_internal, create_internal_action, review_blocker, no_external_action} / blockedLiveActions / safetySnapshot]; every response carries `readonly:true` / `internalOnly:true` / `providerCallMade:false` / `externalActionTaken:false` / `liveAutonomousLocked:true`; never calls a provider, mutates a business / workboard row, enqueues a business Celery job, or touches `RuntimeKillSwitch` / `SandboxState`; backend 17 Phase 16N + 97 regression = 114 passed targeted, frontend 423/423, lint 0, build green, `makemigrations --check` → "No changes detected"; **NO migration; Phase 16N is PRODUCTION VERIFIED on the VPS and CLOSED at commit `8c168db`; Phase 16O is implemented and pushed to origin/main (VPS verification pending); Phase 16P is NOT started**), on top of **Phase 16M — Workboard Analytics + SLA Throughput Dashboard** (PRODUCTION VERIFIED + CLOSED at `a992207`; read-only / internal-only; extends `apps.ai_copilot` with **NO new model and NO migration** — 1 read-only service `get_workboard_analytics(window_days=14)` + 1 read-only endpoint **`GET /api/v1/ai-copilot/workboard/analytics/`** [`AuthenticatedReadAdminWrite`, GET-only, `?windowDays=N` clamped 1–90; POST/PATCH/DELETE → **405**]; derives summary / per-department / per-member / SLA / blocker / daily-throughput-trend analytics purely from existing `AiApprovedAction` + `AiActionWorkEvent` fields; SLA reuses Phase 16K `compute_sla_status` at read time; every response carries `readonly:true` / `internalOnly:true` / `providerActionAttempted:false` / `providerActionTaken:false` / `externalActionAllowed:false` / `externalActionTaken:false`; never calls a provider, mutates a business row, enqueues a business Celery job, or touches `RuntimeKillSwitch` / `SandboxState`; backend 15 Phase 16M + 82 regression = 97 passed targeted, frontend 410/410, lint 0, build green, `makemigrations --check` → "No changes detected" + `manage.py check` clean; **NO migration — a fresh VPS deploy needs no `migrate` for Phase 16M; Phase 16N is PRODUCTION VERIFIED on the VPS and CLOSED at commit `8c168db`; Phase 16O is implemented and pushed to origin/main (VPS verification pending); Phase 16P is NOT started**), on top of **Phase 16L — Scoped Team Member Work Permissions + My Work Queue** (PRODUCTION VERIFIED + CLOSED at `9d144f5`; internal/DB-only; extends `apps.ai_copilot` with 1 additive model `AiWorkboardDepartmentMember` [`user` / `department` (9 Phase 16K teams) / `is_active` / `can_claim` / `can_work` / `can_complete` / `created_by` / timestamps]; 7 new endpoints — `workboard/my/`, `workboard/my/summary/`, `workboard/my-permissions/`, `workboard/department-members/` GET+POST, `.../<id>/activate/` + `.../deactivate/`; the 6 Phase 16K scoped transitions [`actions/<id>/{claim,start,block,unblock,complete-internal,notes}/`] now run `IsAuthenticated` + service-level scoped checks; Director/Admin keep full control, a non-admin claims an unclaimed action only with an active dept membership and works only its own assigned actions, assign/reassign + membership-management stay Director/Admin-only, viewers read-only; every workboard response carries per-action `permissions` booleans; every path is DB-only and never calls a provider / never broadens operations users to admin, `providerAction*` / `externalAction*` stay false; backend 24 Phase 16L + 58 regression = 82 passed targeted, frontend 398/398, lint 0, build green, `makemigrations --check` + `manage.py check` clean; **migration `ai_copilot.0004_phase16l_workboard_department_member` applied on the VPS; VPS browser-validated at `/operations/ai-copilot` [My work queue + workboard] + `/operations/pilot-workbench`; healthz 200 OK; PRODUCTION VERIFIED + CLOSED at `9d144f5`**), on top of **Phase 16K — Department Action Workboard + Ownership / SLA Execution Layer** (PRODUCTION VERIFIED at `efea751`). See the Phase 16L + Phase 16K sections below.
>
> **Phase 16K baseline (PRODUCTION VERIFIED on the VPS and CLOSED at commit `efea751`).** **Phase 16K — Department Action Workboard + Ownership / SLA Execution Layer** (internal/DB-only; extends `apps.ai_copilot` — `AiApprovedAction` gains 8 additive workboard fields [`department`, `assignee_user`, `work_status`, `due_at`, `blocker_reason`, `completed_by`, `completed_at`, `last_activity_at`] + a new `AiActionWorkEvent` model; 12 new endpoints — `workboard/`, `workboard/summary/`, `workboard/director-attention/`, `actions/<id>/{assign,claim,start,block,unblock,complete-internal,reassign,notes}/`; `sla_status` computed at read time; every transition is DB-only and never calls a provider, `providerAction*` / `externalAction*` stay false; all workboard mutations require director/admin/superuser, reads = auth; backend 23 Phase 16K + 102 regression = 125 passed targeted, frontend 385/385, lint 0, build green, `makemigrations --check` + `manage.py check` clean; VPS browser-validated at `/operations/ai-copilot` — Department action workboard + summary cards; Assign / Claim / Block [Director attention updated] / Unblock / Complete internal / Add note all PASSED; external/provider action flags false; `pytest tests/test_phase16k_action_workboard.py` → 23 passed; healthz OK; ADDS migration `ai_copilot.0003_phase16k_action_workboard` → a fresh VPS deploy requires `migrate`, already applied on the VPS), on top of **Phase 16J — AI-Approved Internal Action Queue + Work Execution Bridge** (PRODUCTION VERIFIED at `aa8cf13`). (Phase 16L has since shipped on top — PRODUCTION VERIFIED + CLOSED at `9d144f5`; the current next-planned work is Phase 16P.) See the Phase 16L + Phase 16K + Phase 16J sections below.
>
> **Phase 16J baseline (PRODUCTION VERIFIED on the VPS and CLOSED at commit `aa8cf13`).** **Phase 16J — AI-Approved Internal Action Queue + Work Execution Bridge** (internal/DB-only; extends `apps.ai_copilot` with `AiApprovedAction` / `AiApprovedActionEvent` + 7 endpoints under `/api/v1/ai-copilot/actions...`; converts an approved Phase 16I suggestion into an internal-only work item a human applies — only an approved suggestion can become an action [else `409`]; applying is DB-only [may create an internal `PilotTask` via the Phase 16H safe service, else records a `result_payload`] and never calls a provider; locked `providerActionAttempted` / `providerActionTaken` / `externalActionAllowed` / `externalActionTaken` = false; backend 16 new + regression 102 passed targeted, frontend 374/374, lint 0, build green; `makemigrations --check` + `manage.py check` clean; VPS browser-validated at `/operations/ai-copilot` — an approved suggestion created an internal action [PENDING INTERNAL ACTION → Apply → APPLIED INTERNAL; Reject → REJECTED], safety flags stayed false, no live provider action, `pytest tests/test_phase16j_ai_action_queue.py` → 16 passed, healthz OK), on top of **Phase 16I — AI Copilot Enablement + Human Approval Workflow** (PRODUCTION VERIFIED at `0f91f6b`). **It ADDS migration `ai_copilot.0002_phase16j_ai_action_queue`, so a fresh VPS deploy requires `migrate` (already applied on the VPS). Phase 16J is CLOSED (Phase 16K has since shipped on top — PRODUCTION VERIFIED + CLOSED at `efea751`; the current next-planned work is Phase 16N).** See the Phase 16K + Phase 16J + Phase 16I sections below.
>
...
## Phase 16O — Director Briefing Snapshot History (`/api/v1/ai-copilot/director-briefing/snapshots/`) — implemented and pushed; VPS production verification pending

Internal-only **history / acknowledgement trail** over the Phase 16N Director AI briefing. **ADDS migration `ai_copilot.0005_phase16o_director_briefing_snapshot`** (2 additive models `AiDirectorBriefingSnapshot` + `AiDirectorBriefingSnapshotEvent`). Backed by `apps.ai_copilot.briefing_snapshots`, which reuses `briefing.get_director_ai_briefing` to create snapshots. Calls no external provider, mutates no business / workboard row, enqueues no Celery job, never touches `RuntimeKillSwitch` / `SandboxState`. Every saved snapshot preserves the locked flags (`providerCallMade=false` / `externalActionTaken=false` / `internalOnly=true` / `readonly=true` / `liveAutonomousLocked=true` / `phase15ShellFrozenCommit=eefd8b3`). Response shapes are camelCase.

| Method | Path | Purpose | Auth |
| --- | --- | --- | --- |
| `GET` | `/api/v1/ai-copilot/director-briefing/snapshots/` | List snapshots (`?status=&createdBy=&limit=N`). | any authenticated |
| `POST` | `/api/v1/ai-copilot/director-briefing/snapshots/` | Save the current Phase 16N briefing as a snapshot (`{windowDays?, title?}`). | **Director/Admin** |
| `GET` | `/api/v1/ai-copilot/director-briefing/snapshots/summary/` | Counts (`total`, `unreviewed`, `acknowledged`, `needsFollowUp`, `archived`, `lastSnapshotAt`). | any authenticated |
| `GET` | `/api/v1/ai-copilot/director-briefing/snapshots/<id>/` | Snapshot detail + event trail. | any authenticated |
| `POST` | `/api/v1/ai-copilot/director-briefing/snapshots/<id>/acknowledge/` | Mark acknowledged (`{note?}`) → status `acknowledged` + `acknowledgedBy`/`acknowledgedAt`. | **Director/Admin** |
| `POST` | `/api/v1/ai-copilot/director-briefing/snapshots/<id>/needs-follow-up/` | Mark `needs_follow_up` (`{note?}`). | **Director/Admin** |
| `POST` | `/api/v1/ai-copilot/director-briefing/snapshots/<id>/archive/` | Archive (`{note?}`). | **Director/Admin** |
| `POST` | `/api/v1/ai-copilot/director-briefing/snapshots/<id>/notes/` | Append an internal note (`{note}`). | **Director/Admin** |
| `GET` | `/api/v1/ai-copilot/director-briefing/snapshots/<id>/safe-text/` | Sanitized internal-only plain-text summary (never sent externally). | any authenticated |

`status` lifecycle: `unreviewed → acknowledged / needs_follow_up / archived` (an archived snapshot refuses further transitions with `409`). Every transition writes an `AiDirectorBriefingSnapshotEvent` (created / acknowledged / marked_needs_follow_up / archived / note_added). Unsupported methods (e.g. `PATCH`/`DELETE` on the list, `GET` on a transition) return **405**. The page `/operations/ai-copilot` renders a **Director Briefing History** section consuming these endpoints — read-only, no live-action button.

## Phase 16N — Director AI Daily Briefing (`/api/v1/ai-copilot/director-briefing/`) — PRODUCTION VERIFIED on the VPS and CLOSED at commit `8c168db`

Read-only / internal-only **Director AI briefing** — a decision layer over the existing Phase 16I/16J/16K/16L/16M workboard data. **NO new model, NO migration.** Backed by `apps.ai_copilot.briefing.get_director_ai_briefing(window_days=7)` (composing the Phase 16M analytics + Phase 16K director-attention queue + pending suggestion/action counts). Read = any authenticated user (`AuthenticatedReadAdminWrite`). **GET-only — POST/PATCH/DELETE return `405`.** Calls no external provider, mutates no business / workboard row, enqueues no Celery job, and never touches `RuntimeKillSwitch` / `SandboxState`. Briefing text is deterministic ("mock") — no live AI/LLM call, no medical claim, no customer-facing instruction, no PII beyond the existing username convention. Response shapes are camelCase.

| Method | Path | Purpose |
| --- | --- | --- |
| `GET` | `/api/v1/ai-copilot/director-briefing/` | Full briefing (`?windowDays=N`, clamped 1–30, default 7). Returns `{ briefingStatus, executiveSummary[], attentionItems, departmentSummary[], memberSummary[], safeRecommendations[], slaSummary, blockedLiveActions[], safetySnapshot, generatedAt, windowDays, readonly:true, internalOnly:true, providerCallMade:false, externalActionTaken:false, liveAutonomousLocked:true, phase:"16N" }`. |
| `GET` | `/api/v1/ai-copilot/director-briefing/summary/` | `briefingStatus` + `executiveSummary` + a `headline` counts block + `safetySnapshot` + the locked flags. |
| `GET` | `/api/v1/ai-copilot/director-briefing/recommendations/` | `briefingStatus` + `safeRecommendations[]` + `blockedLiveActions[]` + `permittedActions[]` + the locked flags. |

- **`executiveSummary`** — 3–8 deterministic internal-only bullets (no medical claim, no customer-facing instruction).
- **`attentionItems`** — `total` / `blockedCount` / `overdueCount` / `dueSoonCount` / `unassignedHighPriority` / `pendingSuggestions` / `pendingInternalActions` / `slaRiskCount` + `blocked[]` / `overdue[]` / `unassignedHigh[]` / `items[]` (each item: `id`, `title`, `department`, `workStatus`, `priority`, `slaStatus`, `assigneeUser`, `reason` — internal task title only, no customer PII).
- **`safeRecommendations[]`** — each `{ recommendationType, priority (low|medium|high), reason, linkedMetric, permittedAction }`, where `permittedAction` ∈ `{ internal_review, assign_internal, create_internal_action, review_blocker, no_external_action }` (never an external/customer-facing verb).
- **`blockedLiveActions[]`** — `whatsapp` / `payment` / `courier` / `vapi` / `live_ai`, all `locked:true`.
- **`safetySnapshot`** — AI Paused / Sandbox OFF / Sync Live / AI Mode mock / Live Autonomous Locked / `providerCallMade:false` / `externalActionTaken:false` / `phase15ShellFrozenCommit:"eefd8b3"`.

The page `/operations/ai-copilot` renders a **Director AI Briefing** section consuming this endpoint — read-only, no live-action button.

## Phase 16M — Workboard Analytics + SLA Throughput (`/api/v1/ai-copilot/workboard/analytics/`) — PRODUCTION VERIFIED on the VPS and CLOSED at commit `a992207`

Read-only / internal-only **analytics** over the existing Phase 16J/16K/16L workboard. **NO new model, NO migration.** Backed by `services.get_workboard_analytics(window_days=14)`. Read = any authenticated user (`AuthenticatedReadAdminWrite`). **GET-only — POST/PATCH/DELETE return `405`.** Calls no external provider, mutates no business row, enqueues no Celery job, and never touches `RuntimeKillSwitch` / `SandboxState`. SLA classification reuses the Phase 16K `compute_sla_status` at read time (no DB column). Response shapes are camelCase.

| Method | Path | Purpose |
| --- | --- | --- |
| `GET` | `/api/v1/ai-copilot/workboard/analytics/` | Workboard analytics (`?windowDays=N`, clamped 1–90, default 14). Returns `{ summary, departments[], members[], sla, blockers, trend, generatedAt, windowDays, readonly:true, internalOnly:true, providerActionAttempted:false, providerActionTaken:false, externalActionAllowed:false, externalActionTaken:false, phase:"16M" }`. |

Derived fields, all from existing `AiApprovedAction` (`work_status` / `department` / `priority` / `due_at` / `completed_at` / `blocker_reason` / `assignee_user`) + `AiActionWorkEvent` / `AiApprovedActionEvent`:
- **`summary`** — `total`, `openActions`, `unassigned`, `assigned`, `inProgress`, `blocked`, `completedInternal`, `overdue`, `dueSoon`, `noDueDate`, `directorAttention`, `closed`, `avgCompletionHours`.
- **`departments[]`** — per dept: `department`, `label`, `total`, `open`, `assigned`, `inProgress`, `blocked`, `completedInternal`, `overdue`, `dueSoon`, `noDueDate`, `completionRate`, `avgCompletionHours`, `oldestOpenAgeHours`.
- **`members[]`** — per assignee: `userId`, `username`, `departments[]`, `assignedOpen`, `inProgress`, `blocked`, `overdue`, `completedInternalRecent` (within the window), `avgCompletionHours`.
- **`sla`** — `overdue`, `dueSoon`, `onTrack`, `noDueDate`, `overdueByDepartment`, `dueSoonByDepartment`, `highestRiskDepartment`.
- **`blockers`** — `blockedCount`, `topBlockerReasons[]` (`{reason,count}`, reason truncated to 80 chars), `blockedByDepartment`, `oldestBlockedAgeHours`.
- **`trend`** — `windowDays`, `hasData`, `reason`, `days[]` (`{date, created, assigned, started, blocked, completedInternal}` per local day). Empty/insufficient data → `hasData:false` + `reason:"insufficient_event_data"`.

The page `/operations/ai-copilot` renders a **Workboard Analytics + SLA Throughput** section consuming this endpoint — no mutation/provider button anywhere.

## Phase 16L — Scoped Team Member Work Permissions + My Work (`/api/v1/ai-copilot/workboard/my...` + `workboard/department-members...`) — PRODUCTION VERIFIED at `9d144f5`

Internal/DB-only **scoped permission layer** over the Phase 16K workboard. Extends `apps.ai_copilot` with `AiWorkboardDepartmentMember` (migration `ai_copilot.0004_phase16l_workboard_department_member`). **Calls no external provider; never broadens operations users to admin.** Authorization for the scoped transitions lives in `services.can_user_work_action` / `can_user_claim_action`. Reads require authentication; **membership-management endpoints require director / admin / superuser (even on GET)**; the scoped transition endpoints require authentication and enforce scope in the service (403 `work_permission_denied` on denial). Every workboard / My Work response attaches safe per-action `permissions` booleans + a `myPermissions` summary. Response shapes are camelCase.

| Method | Path | Purpose |
| --- | --- | --- |
| `GET` | `/api/v1/ai-copilot/workboard/my/` | The caller's own assigned actions (`{ items, total, myPermissions }`); each item carries `permissions`. Any authenticated user. |
| `GET` | `/api/v1/ai-copilot/workboard/my/summary/` | Counts over the caller's assigned work: `total`, `assigned`, `inProgress`, `blocked`, `completedInternal`, `dueSoon`, `overdue`, `byWorkStatus`, `phase:"16L"`. |
| `GET` | `/api/v1/ai-copilot/workboard/my-permissions/` | `{ isAdmin, canViewWorkboard, canAssign, canReassign, canManageMembership, departments[], ... }`. |
| `GET` | `/api/v1/ai-copilot/workboard/department-members/` | List scoped memberships (`?department=&active=`). **Director/Admin only.** |
| `POST` | `/api/v1/ai-copilot/workboard/department-members/` | Create/reactivate a membership. Body: `{ userId, department, canClaim?, canWork?, canComplete? }`. Invalid department → `400`; unknown user → `400`. Writes `ai_copilot.workboard.member_added`. **Director/Admin only.** |
| `POST` | `/api/v1/ai-copilot/workboard/department-members/{id}/activate/` | Reactivate a membership. **Director/Admin only.** |
| `POST` | `/api/v1/ai-copilot/workboard/department-members/{id}/deactivate/` | Deactivate a membership. **Director/Admin only.** |

**Scoped transitions (updated in Phase 16L — now `IsAuthenticated` + service scope check):** `POST actions/<id>/{claim,start,block,unblock,complete-internal,notes}/`. Director/Admin pass through; a non-admin must be the action's assignee (or hold an active department membership with `can_claim=true` for `claim`). `block` still requires a `reason`. Denied → `403 work_permission_denied` with a `reason` (`not_assignee` / `no_active_membership` / `membership_cannot_claim` / `membership_cannot_complete` / `action_terminal_or_closed` / `already_assigned` / …). **`assign` / `reassign` remain Director/Admin-only.** No endpoint calls a live provider, mutates a real `Order` / `Payment` / `Shipment` / `Customer` / `Lead`, or changes the Phase 15 safety shell.

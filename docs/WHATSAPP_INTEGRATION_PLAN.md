...
> **Operational baseline note (Phase 16O — implemented and pushed; VPS production verification pending).** Current overall project baseline is **Phase 16O — Director Briefing Snapshot History + Acknowledgement Trail** (internal-only history / acknowledgement layer over the Phase 16N briefing; extends `apps.ai_copilot` with 2 additive models + migration `ai_copilot.0005_phase16o_director_briefing_snapshot` [pure additive] + 8 new `/api/v1/ai-copilot/director-briefing/snapshots/*` endpoints; **Phase 16O does NOT activate WhatsApp in any form** — saving / acknowledging / archiving / annotating a briefing snapshot reuses ONLY the Phase 16N briefing service and never sends WhatsApp, creates a payment link, books a shipment, calls a customer, invokes Vapi, or calls a live AI provider; every snapshot preserves the locked `providerCallMade` / `externalActionTaken` / `liveAutonomousLocked` flags; **ADDS migration `0005`; implemented + pushed, VPS production verification pending; Phase 16P is NOT started**), on top of **Phase 16N — Director AI Daily Briefing Real Data Wiring + Safe Recommendation Pack** (PRODUCTION VERIFIED + CLOSED at `8c168db`; read-only / internal-only decision layer over the existing workboard data; extends `apps.ai_copilot` with **NO new model and NO migration** — a new `apps.ai_copilot.briefing` module + 3 read-only `GET /api/v1/ai-copilot/director-briefing/` + `.../summary/` + `.../recommendations/` endpoints; **Phase 16N does NOT activate WhatsApp in any form** — generating/rendering the briefing never sends WhatsApp, creates a payment link, books a shipment, calls a customer, invokes Vapi, or calls a live AI provider, and a defensive test asserts `queue_template_message` / `send_freeform_text_message` / `trigger_call_for_lead` / `razorpay_client.create_payment_link` / `delhivery_client.create_awb` stay `assert_not_called`; the briefing's `blockedLiveActions` list explicitly marks WhatsApp/payment/courier/Vapi/live-AI as `locked:true`; locked `providerCallMade` / `externalActionTaken` / `liveAutonomousLocked`; **PRODUCTION VERIFIED on the VPS and CLOSED at commit `8c168db`; Phase 16O is implemented and pushed to origin/main (VPS verification pending); Phase 16P is NOT started**), on top of **Phase 16M — Workboard Analytics + SLA Throughput Dashboard** (PRODUCTION VERIFIED + CLOSED at `a992207`; read-only / internal-only analytics over the existing workboard; extends `apps.ai_copilot` with **NO new model and NO migration** — 1 read-only `GET /api/v1/ai-copilot/workboard/analytics/` endpoint; **Phase 16M does NOT activate WhatsApp in any form** — rendering analytics never sends WhatsApp, creates a payment link, books a shipment, calls a customer, invokes Vapi, or calls a live AI provider, and a defensive test asserts `queue_template_message` / `send_freeform_text_message` / `trigger_call_for_lead` / `razorpay_client.create_payment_link` / `delhivery_client.create_awb` stay `assert_not_called`; locked `providerActionAttempted` / `providerActionTaken` / `externalActionAllowed` / `externalActionTaken` = false; Phase 16N is PRODUCTION VERIFIED on the VPS and CLOSED at commit `8c168db`), on top of **Phase 16L — Scoped Team Member Work Permissions + My Work Queue** (PRODUCTION VERIFIED + CLOSED at `9d144f5`; extends `apps.ai_copilot` with a scoped department-membership layer; lets internal team members work their own assigned internal actions without broad Director/Admin power; every transition is DB-only and never calls a provider — locked `provider_action_*` / `external_action_*` false; **Phase 16L does NOT activate WhatsApp in any form**: claiming / starting / blocking / completing a workboard action never sends WhatsApp, creates a payment link, books a shipment, calls a customer, invokes Vapi, or calls a live AI provider, and a defensive test asserts `queue_template_message` / `send_freeform_text_message` stay `assert_not_called` across the full scoped lifecycle), on top of **Phase 16K — Department Action Workboard + Ownership / SLA Execution Layer** (PRODUCTION VERIFIED at `efea751`; also never activates WhatsApp), on top of **Phase 16J — AI-Approved Internal Action Queue + Work Execution Bridge** (PRODUCTION VERIFIED at `aa8cf13`; converts an approved Phase 16I suggestion into an internal-only work item — also never activates WhatsApp), on top of **Phase 16I — AI Copilot Enablement + Human Approval Workflow** (PRODUCTION VERIFIED at `0f91f6b`; Phase 16H — Internal Pilot Execution Workbench verified at `d733cf0`; Phase 16G — Internal Pilot Control Center verified at `38e8dc8`; Phase 16F — Controlled Internal Pilot Readiness verified at `967ed3d`; Phase 16E — Payment / Logistics Integration Hardening verified at `36395f6`; Phase 16D — Uploaded Customer Data Campaigns verified at `c0be74a`; Phase 16C — Director Daily Briefing + Team Roles UI at `687ef41`; Phase 16B — Customer Lifecycle UI Backbone at `00c3295`; the Phase 15 safety shell remains frozen at `eefd8b3`). **Phase 16I did NOT activate WhatsApp in any form** — its AI copilot can produce an internal `whatsapp_draft` suggestion, but that draft is NEVER sent: it is stored sanitized for human review with `external_action_allowed=false` / `external_action_taken=false` / `provider_call_made=false`, and a defensive test asserts `queue_template_message` / `send_freeform_text_message` stay `assert_not_called`. No live LLM/AI provider is called (deterministic generation only). **Carried-forward Phase 16F observation (risk, NOT a blocker):** during VPS validation the pilot gate matrix surfaced **"WhatsApp live automation blocked — WARNING"** with detail **"WhatsApp automation appears enabled — review before pilot."** — this WhatsApp-automation flag posture must be reviewed before any future pilot; it is NOT a blocker because live provider actions remain locked behind a future Director live gate, and **WhatsApp live automation is NOT approved**. All WhatsApp live / broadcast / campaign / lifecycle / handoff / rescue-discount / Day-20 reorder flows **remain locked / OFF** unless the Director explicitly approves a specific phase. This doc is **historical** for WhatsApp integration unless updated later; the snapshot below is the Phase 12D-era WhatsApp status; safety flags, approval requirements, and Claim Vault gating are unchanged. For current truth read [`../nd.md`](../nd.md) head-of-file. Phase 16O is implemented and pushed to origin/main (VPS verification pending); next planned work: Phase 16P (NOT started; separate Director directive required); Phase 16N is PRODUCTION VERIFIED on the VPS and CLOSED at commit `8c168db`.
...
**FULL PASS**. Phase 6I adds a simulation-only layer for internal
live-gate rehearsal. `whatsapp.send_text` is one of the three allowed
simulation operations, but approving/running the simulation does not send
WhatsApp messages or call Meta. The default global runtime kill switch is
enabled, live execution remains blocked, and every simulation keeps
`dryRun=true`, `liveExecutionAllowed=false`, `externalCallWillBeMade=false`,
`externalCallWasMade=false`, and `providerCallAttempted=false`. WhatsApp
runtime still uses env/config; no raw Meta tokens or full phone numbers are
exposed. Auto-reply stays OFF, limited Meta test mode stays ON, and
...
## MM. Phase 5F-Gate Controlled Reply Confidence Fix (post-ship)

After the Claim Vault Grounding Fix landed, the live `--send`
against the allowed test number still failed: `WAM-100007` came
back with `action=handoff`, `confidence=0.7`,
`blockedReason=low_confidence` even though the Weight Management
Claim row was correctly mapped and surfaced in the prompt.

### Root cause

The prompt did not tell the LLM that a normal grounded inquiry must
use `action=send_reply` with `confidence ≥ 0.85`. The LLM defaulted
to handoff out of caution.

### Fix (three pieces)

**1. Business facts injected.** New `BUSINESS FACTS YOU MAY STATE
FREELY` section in the system prompt:

```
- Standard price: ₹3000 for one bottle of 30 capsules.
- Fixed advance amount on order booking: ₹499.
- Conservative usage guidance: "use as directed on the label or as
  advised by a qualified Ayurvedic practitioner". No invented dosage.
- Lifestyle support (diet + activity) only when the Claim Vault for
  the product carries a lifestyle phrase. Do not invent any.
- Doctor escalation for pregnancy / serious illness / allergies /
  existing medication / adverse reaction.
```

The `settings` block in the user JSON gains
`standardCapsuleCount=30`, `currency="INR"`, a
`discountDiscipline` description, and a `businessFactsAllowed`
whitelist.

**Business facts are routing labels and price/quantity facts** —
they are NOT medical claims. Medical / product-benefit content
still comes only from `Claim.approved`.

**2. Action discipline.** New `ACTION SELECTION DECISION TREE`
section in the system prompt with cases A–E:

- A: grounded inquiry → `action='send_reply'`, `confidence ≥ 0.85`,
  `safety.claimVaultUsed=true`, replyText literally includes one
  APPROVED phrase.
- B: open question with no approved Claim Vault entry → handoff.
- C: unknown category → ask_question.
- D: confirmed booking with complete address → book_order.
- E: safety vocabulary in inbound → handoff with safety flag set.

A new `FINAL CHECK` paragraph at the end of the schema instructions
makes "Defaulting to action='handoff' on a grounded inquiry is a
defect" a literal instruction.

**Confidence threshold env unchanged.** The fix is in the prompt —
never lower `WHATSAPP_AI_AUTO_REPLY_CONFIDENCE_THRESHOLD` globally.

**3. Diagnostics cleanup.** The ambiguous `claim_count` (which
sometimes meant rows and sometimes phrases) is split into three
explicit fields across all four orchestrator audit kinds AND the
controlled-test JSON:

- `claim_row_count` — number of `Claim` rows for the category.
- `approved_claim_count` — sum of `Claim.approved` phrase counts.
- `disallowed_phrase_count` — sum of `Claim.disallowed` phrase
  counts.

`claim_count` is preserved as a backward-compat alias for
`approved_claim_count`. Affected audit kinds:
`whatsapp.ai.{category_detected, reply_blocked, handoff_required,
controlled_test.blocked}`. Controlled-test JSON also gains
`confidenceThreshold`, `actionReason`, `sendEligibilitySummary`,
and `groundingStatus.{claimRowCount, businessFactsInjected}`.

### Hard rules preserved

- `claimVaultUsed=false` still blocks the send.

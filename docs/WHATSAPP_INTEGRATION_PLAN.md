...
> **Operational baseline note (Phase 16F — PRODUCTION VERIFIED on the VPS and CLOSED at `967ed3d`).** Current overall project baseline is **Phase 16F — Controlled Internal Pilot Readiness + End-to-End Dry Run** (Phase 16E — Payment / Logistics Integration Hardening verified at `36395f6`; Phase 16D — Uploaded Customer Data Campaigns verified at `c0be74a`; Phase 16C — Director Daily Briefing + Team Roles UI at `687ef41`; Phase 16B — Customer Lifecycle UI Backbone at `00c3295`; the Phase 15 safety shell remains frozen at `eefd8b3`). **Phase 16F did NOT activate WhatsApp in any form** — its pilot readiness + dry-run surface is internal/DB-only and triggers no WhatsApp/Meta Cloud send (a defensive test asserts `queue_template_message` / `send_freeform_text_message` stay `assert_not_called`). **Phase 16F observation (risk, NOT a blocker):** during VPS validation the pilot gate matrix surfaced **"WhatsApp live automation blocked — WARNING"** with detail **"WhatsApp automation appears enabled — review before pilot."** — this WhatsApp-automation flag posture must be reviewed before any future pilot; it is NOT a blocker because live provider actions remain locked and blocked behind a future Director live gate, and **WhatsApp live automation is NOT approved**. All WhatsApp live / broadcast / campaign / lifecycle / handoff / rescue-discount / Day-20 reorder flows **remain locked / OFF** unless the Director explicitly approves a specific phase. This doc is **historical** for WhatsApp integration unless updated later; the snapshot below is the Phase 12D-era WhatsApp status; safety flags, approval requirements, and Claim Vault gating are unchanged. For current truth read [`../nd.md`](../nd.md) head-of-file. Next planned work: Phase 16G (NOT started; separate Director directive required).

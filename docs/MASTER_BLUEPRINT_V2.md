...
> **Operational supersession note (Phase 16E — implemented + pushed; VPS verification pending).** This blueprint v2 reflects the project's strategic framing as of its original revision date below and contains historical Phase 12D / Phase 8F-era wording. **Current operational truth is [`../nd.md`](../nd.md) head-of-file.** The current operational baseline is **Phase 16E — Payment / Logistics Integration Hardening** (internal/read-only payment + logistics readiness layer + shipment-create mode-dispatch hardening; no live provider action); **Phase 16D — Uploaded Customer Data Campaigns is verified at `c0be74a`**, **Phase 16C — Director Daily Briefing + Team Roles UI at `687ef41`**, and **Phase 16B — Customer Lifecycle UI Backbone at `00c3295`** (the Phase 15 safety shell remains frozen at commit `eefd8b3`). Where this blueprint and `nd.md` disagree on any operational detail, **`nd.md` wins** and the blueprint is the historical strategic mirror. Next planned work is **Phase 16F** (NOT started; separate Director directive required). This blueprint is not edited section-by-section as new phases ship; it is preserved as the strategic v2 snapshot.

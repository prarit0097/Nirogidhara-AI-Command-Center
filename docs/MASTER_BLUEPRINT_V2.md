...
> **Operational supersession note (Phase 16B — production verified).** This blueprint v2 reflects the project's strategic framing as of its original revision date below and contains historical Phase 12D / Phase 8F-era wording. **Current operational truth is [`../nd.md`](../nd.md) head-of-file.** The current operational baseline is **Phase 16B — Customer Lifecycle UI Backbone, PRODUCTION VERIFIED + CLOSED at commit `00c3295`** (after Hotfix-2; the Phase 15 safety shell remains frozen at commit `eefd8b3`). Where this blueprint and `nd.md` disagree on any operational detail, **`nd.md` wins** and the blueprint is the historical strategic mirror. Next planned work is **Phase 16C — Director Daily Briefing + Team Roles UI** (separate Director directive required). This blueprint is not edited section-by-section as new phases ship; it is preserved as the strategic v2 snapshot.
...
| Completed phase range (historical — as of original revision) | Phase 1 → **Phase 12D** (Tier-2 AI agent layer 9A-9F complete; Phase 10A-10C payment-recovery workflow complete). **Current operational baseline is Phase 16B (Customer Lifecycle UI Backbone, production verified at `00c3295`) — see `nd.md` head-of-file.** |

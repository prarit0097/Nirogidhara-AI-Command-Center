...
> **Operational supersession note (Phase 16J — IMPLEMENTED + PUSHED to `origin/main`; VPS production verification pending).** This blueprint v2 reflects the project's strategic framing as of its original revision date below and contains historical Phase 12D / Phase 8F-era wording. **Current operational truth is [`../nd.md`](../nd.md) head-of-file.** The current operational baseline is **Phase 16J — AI-Approved Internal Action Queue + Work Execution Bridge** (internal/DB-only; extends `apps.ai_copilot` with `AiApprovedAction` / `AiApprovedActionEvent` + 7 `/api/v1/ai-copilot/actions...` endpoints; converts an approved Phase 16I suggestion into an internal-only work item a human applies — only an approved suggestion can become an action; applying is DB-only [may create an internal `PilotTask` via the Phase 16H safe service, else records a `result_payload`] and never calls a provider; locked `provider_action_attempted` / `provider_action_taken` / `external_action_allowed` / `external_action_taken` = false; IMPLEMENTED + PUSHED, VPS verification pending), on top of **Phase 16I — AI Copilot Enablement + Human Approval Workflow** (internal/DB-only AI copilot / decision-support layer in `apps.ai_copilot`; deterministic "mock"/"sandbox" generation, no live AI/LLM provider call, human approval before any business action; locked `provider_call_made` / `external_action_allowed` / `external_action_taken` = false; PRODUCTION VERIFIED at `0f91f6b`), on top of **Phase 16H — Internal Pilot Execution Workbench + Role-Based Task Queues** (PRODUCTION VERIFIED at `d733cf0`), **Phase 16G — Internal Pilot Control Center / Pilot Execution Dashboard** (PRODUCTION VERIFIED at `38e8dc8`), **Phase 16F — Controlled Internal Pilot Readiness + End-to-End Dry Run** (PRODUCTION VERIFIED at `967ed3d`) and **Phase 16E — Payment / Logistics Integration Hardening** (PRODUCTION VERIFIED at `36395f6`); **Phase 16D — Uploaded Customer Data Campaigns is verified at `c0be74a`**, **Phase 16C — Director Daily Briefing + Team Roles UI at `687ef41`**, and **Phase 16B — Customer Lifecycle UI Backbone at `00c3295`** (the Phase 15 safety shell remains frozen at commit `eefd8b3`). Where this blueprint and `nd.md` disagree on any operational detail, **`nd.md` wins** and the blueprint is the historical strategic mirror. Next planned work is **Phase 16K** (NOT started; separate Director directive required). This blueprint is not edited section-by-section as new phases ship; it is preserved as the strategic v2 snapshot.

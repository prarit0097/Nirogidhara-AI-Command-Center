...
> **Operational supersession note (Phase 16L — PRODUCTION VERIFIED on the VPS and CLOSED at commit `9d144f5`).** This blueprint v2 reflects the project's strategic framing as of its original revision date below and contains historical Phase 12D / Phase 8F-era wording. **Current operational truth is [`../nd.md`](../nd.md) head-of-file.** The current operational baseline is **Phase 16O — Director Briefing Snapshot History + Acknowledgement Trail** (internal-only history / acknowledgement layer over the Phase 16N briefing; extends `apps.ai_copilot` with **2 additive models** `AiDirectorBriefingSnapshot` + `AiDirectorBriefingSnapshotEvent` + migration `ai_copilot.0005_phase16o_director_briefing_snapshot` [pure additive]; new `apps.ai_copilot.briefing_snapshots` service reuses the Phase 16N briefing service; 8 new `/api/v1/ai-copilot/director-briefing/snapshots/*` endpoints — save / list / detail / summary / acknowledge / needs-follow-up / archive / notes / safe-text; read = any authenticated user, save + state changes = Director/Admin only; never calls a provider, mutates a row, enqueues a Celery job, or touches `RuntimeKillSwitch` / `SandboxState`; every snapshot preserves the locked flags; **ADDS migration `0005` — a fresh deploy requires `migrate`; implemented + pushed, VPS production verification pending; Phase 16P is NOT started**), on top of **Phase 16N — Director AI Daily Briefing Real Data Wiring + Safe Recommendation Pack** (read-only / internal-only; extends `apps.ai_copilot` with **NO new model and NO migration** — a new `apps.ai_copilot.briefing` module + 3 read-only `GET /api/v1/ai-copilot/director-briefing/` + `.../summary/` + `.../recommendations/` endpoints composing the Phase 16M analytics + Phase 16K director-attention queue + pending suggestion/action counts into a deterministic Director AI briefing with safe internal-only recommendations [`permittedAction` ∈ {internal_review, assign_internal, create_internal_action, review_blocker, no_external_action}]; never calls a provider, mutates a row, enqueues a Celery job, or touches `RuntimeKillSwitch` / `SandboxState`; locked `providerCallMade` / `externalActionTaken` / `liveAutonomousLocked`; **PRODUCTION VERIFIED on the VPS and CLOSED at commit `8c168db`**), on top of **Phase 16M — Workboard Analytics + SLA Throughput Dashboard** (read-only / internal-only; extends `apps.ai_copilot` with **NO new model and NO migration** — 1 read-only `GET /api/v1/ai-copilot/workboard/analytics/` endpoint backed by `get_workboard_analytics()`; derives summary / per-department / per-member / SLA / blocker / throughput-trend analytics purely from existing workboard fields; never calls a provider, mutates a business row, enqueues a business Celery job, or touches `RuntimeKillSwitch` / `SandboxState`; locked `providerActionAttempted` / `providerActionTaken` / `externalActionAllowed` / `externalActionTaken` = false; **PRODUCTION VERIFIED + CLOSED at `a992207`**), on top of **Phase 16L — Scoped Team Member Work Permissions + My Work Queue** (internal/DB-only; extends `apps.ai_copilot` with 1 additive model `AiWorkboardDepartmentMember` + 7 new `/api/v1/ai-copilot/workboard/my...` & `workboard/department-members...` endpoints; lets internal team members safely work their own assigned internal actions [+ claim eligible department work] without broad Director/Admin power — assign/reassign + membership-management stay Director/Admin-only; every path is DB-only and never calls a provider / never broadens operations users to admin; locked `provider_action_*` / `external_action_*` = false; PRODUCTION VERIFIED + CLOSED at `9d144f5`), on top of **Phase 16K — Department Action Workboard + Ownership / SLA Execution Layer** (internal/DB-only; PRODUCTION VERIFIED + CLOSED at `efea751`), on top of **Phase 16J — AI-Approved Internal Action Queue + Work Execution Bridge** (internal/DB-only; converts an approved Phase 16I suggestion into an internal-only work item a human applies; PRODUCTION VERIFIED + CLOSED at `aa8cf13`), on top of **Phase 16I — AI Copilot Enablement + Human Approval Workflow** (internal/DB-only AI copilot / decision-support layer in `apps.ai_copilot`; deterministic "mock"/"sandbox" generation, no live AI/LLM provider call, human approval before any business action; locked `provider_call_made` / `external_action_allowed` / `external_action_taken` = false; PRODUCTION VERIFIED at `0f91f6b`), on top of **Phase 16H — Internal Pilot Execution Workbench + Role-Based Task Queues** (PRODUCTION VERIFIED at `d733cf0`), **Phase 16G — Internal Pilot Control Center / Pilot Execution Dashboard** (PRODUCTION VERIFIED at `38e8dc8`), **Phase 16F — Controlled Internal Pilot Readiness + End-to-End Dry Run** (PRODUCTION VERIFIED at `967ed3d`) and **Phase 16E — Payment / Logistics Integration Hardening** (PRODUCTION VERIFIED at `36395f6`); **Phase 16D — Uploaded Customer Data Campaigns is verified at `c0be74a`**, **Phase 16C — Director Daily Briefing + Team Roles UI at `687ef41`**, and **Phase 16B — Customer Lifecycle UI Backbone at `00c3295`** (the Phase 15 safety shell remains frozen at commit `eefd8b3`). Where this blueprint and `nd.md` disagree on any operational detail, **`nd.md` wins** and the blueprint is the historical strategic mirror. Phase 16N is PRODUCTION VERIFIED on the VPS and CLOSED at commit `8c168db`; Phase 16O is implemented and pushed to origin/main (VPS verification pending); the next planned work is **Phase 16P** (NOT started; separate Director directive required). Phase 16M (Workboard Analytics + SLA Throughput Dashboard) is PRODUCTION VERIFIED + CLOSED at `a992207`. This blueprint is not edited section-by-section as new phases ship; it is preserved as the strategic v2 snapshot.
...
| Last revised (this blueprint v2) | 2026-05-14 |
...
| Completed phase range (historical — as of original revision) | Phase 1 → **Phase 12D** (Tier-2 AI agent layer 9A-9F complete; Phase 10A-10C payment-recovery workflow complete). **Current operational baseline is Phase 16M (Workboard Analytics + SLA Throughput Dashboard, read-only/internal-only, NO migration, PRODUCTION VERIFIED + CLOSED at `a992207`) on top of Phase 16L (Scoped Team Member Work Permissions + My Work Queue, PRODUCTION VERIFIED + CLOSED at `9d144f5`) on top of Phase 16K (Department Action Workboard + Ownership / SLA Execution Layer, PRODUCTION VERIFIED + CLOSED at `efea751`), Phase 16J (AI-Approved Internal Action Queue + Work Execution Bridge, PRODUCTION VERIFIED + CLOSED at `aa8cf13`), Phase 16I (AI Copilot Enablement + Human Approval Workflow, PRODUCTION VERIFIED + CLOSED at `0f91f6b`), Phase 16H (Internal Pilot Execution Workbench, PRODUCTION VERIFIED + CLOSED at `d733cf0`), Phase 16G (Internal Pilot Control Center, PRODUCTION VERIFIED + CLOSED at `38e8dc8`), Phase 16F (Controlled Internal Pilot Readiness, PRODUCTION VERIFIED + CLOSED at `967ed3d`) and Phase 16E (Payment / Logistics Integration Hardening, PRODUCTION VERIFIED at `36395f6`; Phase 16D `c0be74a`, Phase 16C `687ef41`, Phase 16B `00c3295` are earlier verified baselines) — see `nd.md` head-of-file.** |
| Snapshot commit at original revision (historical) | `dbe8a7b` on `origin/main` (Phase 12D — `nrg_payment_reminder` template schema fix). **Current HEAD lives in `nd.md` + [`PHASE_15M_DIRECTOR_SIGNOFF_PACK.md`](PHASE_15M_DIRECTOR_SIGNOFF_PACK.md);** this row is the v2 snapshot. |
| Test baseline at original revision (historical) | **2466 backend tests · 82 frontend tests** · `makemigrations --check` clean · `manage.py check` clean · frontend lint 0 errors / 8 pre-existing shadcn warnings · `npm test` 82/82 · `npm run build` green. **Current verification baseline lives in `nd.md` + Phase 15M sign-off pack** and is no longer reflected here. |
| Planned next phase at original revision (historical) | **Phase 8F live execute on the VPS — NOT approved.** Requires a separate dated Director directive, all three Phase 8F env flags true, and a 15-minute structured UTC window naming the actual Phase 8F gate id / attempt id / Phase 8E gate id 1 / target Order `NRG-20435` / target Payment `PAY-30125`. Phase 7E-Live-B and Phase 7G-Live remain NOT approved. **Current planned next phase is Phase 16E — Payment / Logistics Integration Hardening** (NOT started; separate Director directive required; Phase 16A audit + Phase 16B implementation + Phase 16C Director Briefing/Team Roles already shipped and verified — Phase 16C at `687ef41`). |
| Smoke + provider state (historical — as of original revision) | All earlier 5E / 5F gates green. Phase 6A → 6M shipped on top: SaaS multi-tenant scaffold (Phase 6A), default-org backfill (6B), org-scoped read APIs (6C), org-aware write paths (6D), SaaS Admin foundation + integration settings metadata (6E), per-org runtime routing preview (6F), controlled runtime routing dry run + AI provider routing preview (6G), controlled runtime live audit gate (6H), single internal live gate simulation (6I), single internal provider test plan (6J), single internal Razorpay test-mode execution gate (6K-A) + manual VPS one-shot pass (6K-B → `pex_8f309650e9644cfaae4418f9` → `order_Sks3KPf0vntKhf`, ₹1.00, rolled back), Razorpay test execution audit review + webhook readiness plan (6L), MCP Gateway foundation (6M-0, dormant), and Razorpay test-mode webhook handler (6M, dormant). Runtime providers still use env/config; `RuntimeKillSwitch` enabled (`enabled=True` — kill switch active, AI Paused); no MCP tools active; no broad-automation flags flipped. **For current production posture see `nd.md` + Phase 15M sign-off pack §9.** |
...
**Phase 7E update (2026-05-07):** Completed phase range is now Phase 1 → **Phase 7E — Controlled Internal WhatsApp Notification Readiness Gate**. Phase 7E is gate-only and CLI-only for review state changes, with `PHASE7E_WHATSAPP_INTERNAL_NOTIFICATION_GATE_ENABLED=false` by default. Phase 7E never sends WhatsApp, never queues an outbound, never calls Meta Cloud / Delhivery / Vapi, never creates a shipment / AWB / payment link, never captures, never refunds, never sends a customer notification, never mutates real business rows, never edits any `.env*` file. Approval flips status to `approved_for_future_phase7f_or_7e_send_review` only — it does **NOT** enable any send path. 14 audit kinds (each ≤ 64 chars). 8 CLI commands. 5 read-only auth-protected GET DRF endpoints. New shared utility `apps.saas.utc_window` ships `parse_director_signoff_window` + `validate_review_window`; Phase 7D-Hotfix-1 will reuse and extend it for execute-window guards. Last verified baseline: **1672 backend tests + 68 frontend tests**, `makemigrations --check` clean, `manage.py check` clean, frontend lint 0 errors, frontend build OK. **Phase 7F (Delhivery shipment) / Phase 7E-Live (real customer WhatsApp send) remain NOT approved. Phase 7D-Hotfix-1 (structured UTC window guard for execute commands) is mandatory before any future provider-touching command runs.**

**Phase 7D update (2026-05-07):** Phase 7D — Razorpay Controlled Pilot one-shot internal TEST execution — was **executed once on 2026-05-07** (provider order id `order_SmThqpK6sc6Dhs`, attempt id `1`, gate id `1`, `executed_at=2026-05-07T12:42:46Z`, status `rolled_back`, `rollback_status=completed`, `provider_call_attempted=True`, every business / WhatsApp / Delhivery / payment-link / capture / refund / shipment / customer-notification boolean stayed `False`, `safeToRunPhase7DExecution=false`, `nextPhase=7E_not_approved`). The execution fell at `2026-05-07T12:42:46Z` — ~134 seconds before the Director-approved UTC window of `2026-05-07T12:45:00Z → 2026-05-07T13:00:00Z` — but **no business or customer impact** (every locked-False boolean stayed False, the attempt was rolled back). Phase 7D-Hotfix-1 is mandatory before any future re-run.

**(Stale Phase 7D wording superseded by the two blocks above. Original block follows.)**
...
**Phase 6M baseline (2026-05-04, commit `bd38bee`):** Phase 6L Razorpay Test
Execution Audit Review + Webhook Readiness Plan was FULL PASS. Phase 6M-0
shipped the dormant MCP Gateway foundation (`apps.mcp_gateway` — 6 models,
admin-only readiness APIs, 13-name forbidden-tool list, `MCP_ENABLED=false`,
`MCP_READ_ONLY_MODE=true`). Phase 6M then implemented the Razorpay test-mode
webhook **handler** at `POST /api/webhooks/razorpay/test/` that stays
**dormant by default** (`RAZORPAY_WEBHOOK_TEST_MODE_ENABLED=false`). When
enabled, the handler verifies HMAC-SHA256 over the raw body in constant
time, validates a 300-second replay window, dedupes on
`X-Razorpay-Event-Id`, masks payloads against a 13-key scrub list, and
persists only a safe summary on `RazorpayWebhookEvent`; **business mutation
remains LOCKED OFF** (`RAZORPAY_WEBHOOK_BUSINESS_MUTATION_ENABLED=false`,
`RAZORPAY_WEBHOOK_NOTIFY_CUSTOMER_ENABLED=false`). The handler asserts
`assert_no_business_mutation` in tests — Order / Payment / Shipment /
DiscountOfferLog / Customer rows are never touched.

**Phase 6K-B real artefact (immutable):** the only real provider-side write
this platform has ever made is `pex_8f309650e9644cfaae4418f9` →
`order_Sks3KPf0vntKhf`, amount `100 paise INR` (₹1.00), Razorpay TEST key,
no payment link, no capture, no customer notification, no business
mutation, `rollback_status=completed`,
`PHASE6K_RAZORPAY_TEST_EXECUTION_ENABLED` flipped back to `false`
immediately after the run.

**Locked safety state (do not flip):** `RuntimeKillSwitch` enabled,
`MCP_ENABLED=false`, every WhatsApp automation flag false,
`WHATSAPP_AI_AUTO_REPLY_ENABLED=false`,
`WHATSAPP_LIVE_META_LIMITED_TEST_MODE=true`, every Razorpay-webhook env
flag false, campaigns / broadcast / lifecycle automation / RTO rescue /
reorder cadence LOCKED.

Phase 6E–6I notes are preserved below for historical context. None of
those phases re-introduced live execution; every phase keeps `dryRun=true`,
`liveExecutionAllowed=false`, `externalCallWillBeMade=false`,
`externalCallWasMade=false`, and `providerCallAttempted=false` in every
read-only response.
...
`whatsapp.send_text`, and `ai.smoke_test`. Runtime providers still use
env/config, the global kill switch remains active, and approval/run in
Phase 6I does not execute external calls. No real
...
## 3. Completed Build Timeline — Phase 1 → Phase 14A

| Phase | Status | What shipped | Risk / Safety note |
| --- | --- | --- | --- |
| Phase 14B | ✅ Shipped (docs-only) | Doc Sync to Reality — six atomic commits bringing nd.md / CLAUDE.md / AGENTS.md / docs/MASTER_BLUEPRINT_V2.md / docs/BACKEND_API.md / docs/FUTURE_BACKEND_PLAN.md / docs/RUNBOOK.md / docs/DEPLOYMENT_VPS.md / ndmemory.txt up to actual VPS commit reality through Phase 14A. | No code, env, migration, or test files modified. |
| Phase 14A | ✅ Shipped (docs-only) | Founder Operating Model vision lock (commit `a7cee89`). Six canonical docs updated with solo-operator design constraint, ₹10,000 cr North Star, Net Delivered Profit per Director Hour metric. New nd.md §1.5 canonical specification. | No code, env, migration, or test files modified. |
| Phase 13D-1 | ✅ Shipped (DB-ops only) | Integration Readiness DB cleanup. `OrganizationIntegrationSetting` rows reset to reflect actual env reality: Delhivery + PayU `invalid`, Vapi `warning`, WhatsApp Meta + Razorpay + OpenAI `valid`. | No code commit; DB rows only. |
| Phase 13C | ✅ Shipped | ErrorBoundary wrapper + broad defensive pass (commits `cf48ddd`, `d72bfe1`). New `frontend/src/components/ErrorBoundary.tsx` around `/saas-admin` route + 148 array-like access defenses via reusable `scripts/phase13c_defensive_pass.py`. 3 new ErrorBoundary tests. | SaaS Admin page no longer blanks on field-shape mismatch. |
| Phase 13B | ✅ Shipped | SaasAdmin defensive optional chaining + WebSocket Mixed-Content fix (commits `98926ad`, `7c5d0ac`). 11 optional chaining edits across Phase 7 readiness nested chains + `wss://` scheme derivation on HTTPS pages. | Restores `/saas-admin` rendering after Phase 13A login; no provider call. |
| Phase 13A | ✅ Shipped — live in production | Director Login Flow (`001eaf0..a6eb0a0`). New `/login` route + JWT-backed login. SimpleJWT `TokenObtainPairView` at `/api/v1/auth/login/` alias (legacy `/api/auth/token/` preserved). RequireAuth route guard + 401 interceptor + safeFetch production fix + logout button. 4 backend tests + 3 frontend tests. | Director user `1995praritsidana@gmail.com` created via Django shell + `getpass()`; password never in code/env/git. |
| Phase 12D | ✅ Shipped (frontend-only) | Calling Performance Dashboard frontend Tier 4 complete (commit `dbe8a7b`). New `/operations/calling-dashboard` page with 4 sections (Campaign History / Call Outcomes / WhatsApp Follow-up Queue / CLI Reference). | Read-only end-to-end; all state changes via Phase 12A/B/C CLI commands. |
| Phase 12C | ✅ Shipped | Post-call WhatsApp followup queue v1 (commit `3fa8e6c`). Director-triggered. No automatic outbound without director-level enable flag. | Director-only CLIs (`list_post_call_followups`, `prepare_post_call_followup_gate`, `mark_followup_dispatched`, `skip_post_call_followup`). |
| Phase 12B | ✅ Shipped | Call outcome classifier v1 (commit `22ff52c`). Director-reviewed lead status SUGGESTIONS only. No auto-promotion. | Suggestions emitted on transcripts; no business mutation. |
| Phase 12A | ✅ Shipped | AI calling campaign gate v1 (commit `22535d6`). Director-approved Vapi outbound. Campaign cannot be approved without named candidate set + Claim Vault grounding + RuntimeKillSwitch precondition. | Live Vapi outbound execution status to be confirmed with operator before next planning round. |
| Phase 11D | ✅ Shipped | Learning loop gate v1 (commit `e01a52e`). Director-approved + human-reviewed pathway for any prompt / playbook / claim vault update. Auto-promotion forbidden. | New `apps.learning` Django app + frontend Learning Proposals page (commit `4105e1f`). |
| Phase 11C | ✅ Shipped | CAIO audit agent v1 (commit `9655472`). Governance recommendations-only daily audit. | CAIO never executes business actions (hard rule, double-guarded). New `apps.caio` Django app. |
| Phase 11B | ✅ Shipped | Call quality scorer v1 (commit `8a5d314`). Deterministic five-dimension scoring of ingested transcripts. | Pure-Python deterministic scorer; not LLM-only. |
| Phase 11A | ✅ Shipped | Transcript ingestion pipeline v1 (commit `828d2b5`). Vapi daily Celery pull for call transcripts. | Recommendations-only; no business mutation. |
| Phase 10C | ✅ Shipped | Razorpay payment link refresh gate (commits `553b1a9`, `5a0fee2`). Test default; live mode gated by env flag. Migration `0026_phase10c_payment_link_refresh_gate`. | Refreshed Razorpay link reference persisted. |
| Phase 10B + Hotfix-1 + Hotfix-2 | ✅ Shipped | Targeted payment reminder preparer (commits `5f1587c`, `cde213f` Hotfix-1 auto-create CRM customer, `47ac3ac` Hotfix-2 `nrg_payment_reminder` template params customer_name + context). | Stage-aware preparer creating Phase 7E-Live-B-ready draft gate for pending payments. |
| Phase 10A + Hotfix-1 | ✅ Shipped | Pending payments drilldown (commits `5b33c47`, `95f9b63` Hotfix-1 phone fallback). New `apps.diagnostics` Django app. Read-only by default. | No business mutation. |
| ✅ Phase 8F Live Execute + Rollback Reading 1 (2026-05-14) | ✅ EXECUTED + ROLLED BACK | First and only live Phase 8F real-customer Payment → Order mutation (commit `057b140` doc-only completion record). Target: Order `NRG-20435` + Payment `PAY-30125`. Result: `Partial → Paid → Partial` and `Pending → Paid → Pending`. All locked-False flags stayed False. No provider call, WhatsApp, customer notification, shipment/AWB, or `Order.state` mutation. | Three `PHASE8F_*` env flags via runtime env prefix only; `.env.production` NOT edited. Future runs require fresh Director directive. |
| Phase 8F-Hotfix-3 | ✅ Shipped (governance CLI only) | Attempt-level recovery command for blocked Phase 8F attempt (commit `2aca2fc`). `recover_phase8f_attempt_to_approved` used during pre-execute journey. | Does not execute Phase 8F. |
| Phase 7G-Live | ✅ Shipped (gate framework only) | Real customer Delhivery one-shot dispatch gate (commit `54f2c53`). Defaults LOCKED OFF. | Execution status against live customer order to be confirmed with operator. |
| Phase 7E-Live-B + Hotfix-1 | ✅ Shipped + executed | Real customer WhatsApp one-shot send gate (commits `10867c0`, `0d9c975` Hotfix-1 kill-switch precondition). Single internal one-shot executed under `WHATSAPP_LIVE_META_LIMITED_TEST_MODE=true`. | Broader live customer send still gated; allowlist-only mode preserved. |
...
| 5F-Gate Real Inbound Deterministic Fallback Fix | ✅ Live | Deterministic Claim-Vault-grounded reply fallback wired into the orchestrator's real-inbound webhook auto-reply path. `_attempt_deterministic_grounded_fallback` + `_dispatch_deterministic_fallback` cover every soft non-safety blocker (`claim_vault_not_used`, `low_confidence`, `ai_handoff_requested`, `no_action`); discount-objection inbounds use `build_objection_aware_reply`. Latest-inbound safety isolation: `safety_downgraded` audit now carries `latest_inbound_message_id` + `history_safety_ignored_for_current_safe_query`. `whatsapp.ai.auto_reply_flag_path_used` payload extended; `decision.safety["claimVaultUsed"]=True` flips after validator confirms an approved phrase is embedded. 13 new pytest cases. | Hard rules preserved end-to-end — zero mutation of Order/Payment/Shipment/DiscountOfferLog from any fallback path; final-send limited-mode guard still blocks non-allowed numbers. Phase 5F (campaigns) remains LOCKED. |
| 5F-Gate Auto-Reply Monitoring Dashboard | ✅ Live | Read-only command-center surface for the WhatsApp auto-reply safety gate. New `apps.whatsapp.dashboard` selectors (`get_auto_reply_gate_summary`, `get_recent_auto_reply_activity`, `get_internal_cohort_summary`, `get_recent_whatsapp_audit_events`, `get_whatsapp_mutation_safety_summary`, `get_unexpected_outbound_summary`, `get_whatsapp_monitoring_dashboard`). Seven new admin-only `/api/whatsapp/monitoring/...` endpoints (read-only — POST/PATCH/DELETE return 405). New `/whatsapp-monitoring` page renders status badge + gate cards + automation pills + activity metrics + mutation safety + cohort + audit timeline; auto-refresh 30s. Phones masked to last-4; tokens / verify token / app secret never returned. 22 new backend + 6 new frontend tests. | Dashboard is read-only end-to-end (asserted in tests). No send / enable / disable / flag-flip controls. Phase 5F (campaigns) remains LOCKED. |
| 5F-Gate Customer Pilot Readiness | ✅ Live | Approved-customer-pilot read-only readiness layer. New `WhatsAppPilotCohortMember` model (masked phone / consent verification / pending/approved/paused/removed lifecycle / source / approver / daily cap). Selectors in `apps.whatsapp.pilot`. CLI commands: `inspect_whatsapp_customer_pilot --json`, `prepare_whatsapp_customer_pilot_member`, `pause_whatsapp_customer_pilot_member`. Read-only `/api/v1/whatsapp/monitoring/pilot/` + `pilot` block in monitoring overview. Dashboard adds "Approved Customer Pilot Readiness" section. | Prepare/pause write audit rows only; no send path; no Order/Payment/Shipment/DiscountOfferLog mutation. Customer pilot soak was accelerated, not full-duration. Phase 5F (campaigns) remains LOCKED. |
| 6A | ✅ Live | SaaS multi-tenant scaffold. New `apps.saas` Django app — `Organization` (default `nirogidhara`), `Branch` (default `main`), `OrganizationMembership`, `OrganizationFeatureFlag`, `OrganizationSetting` (`is_sensitive=True` rows filtered out of every public-API selector). `python manage.py ensure_default_organization --json` is idempotent. Three read-only DRF endpoints under `/api/v1/saas/{current-organization,my-organizations,feature-flags}/` (POST/PATCH/DELETE return 405). Frontend topbar `OrgBadge`. 22 new backend + 3 new frontend pytest cases. | No `organization` FK on existing models yet; no request middleware filters yet; existing logins keep working via default-org fallback; sensitive settings never appear in any public response (asserted in tests). |
| 6B | ✅ Live | SaaS default-org data backfill (read-only by default). 14 business-state models gain nullable `organization` (and `branch` where applicable) FKs (`null=True, blank=True, on_delete=SET_NULL`). New `python manage.py backfill_default_organization_data [--dry-run\|--apply] [--json]` is idempotent + emits `saas.default_org_backfill.{started,completed,failed}` audit rows. New `inspect_default_organization_coverage` selector + CLI + read-only `GET /api/v1/saas/data-coverage/`. Frontend `SaasCoverageCard` on the dashboard. 17 new backend pytest cases. | `--dry-run` is the default. Backfill never overwrites existing org/branch (asserted with before/after counts). No `Order` / `Payment` / `Shipment` / `DiscountOfferLog` / `WhatsAppMessage` payload changes. No provider calls. Coverage card is read-only. |
| 6C | ✅ Live | Org-scoped read filtering foundation. New `apps.saas.context` ships seven resolvers (`get_default_organization`, `get_user_active_organization` with default-org fallback, `resolve_request_organization`, `user_has_org_access`) and four queryset helpers (`scoped_queryset_for_user`, `scoped_queryset_for_request`, `filter_queryset_by_organization` — no-op when model has no `organization` field). `apps.audit.signals.write_event` now auto-attaches the active organization (explicit > request > user > default-org fallback). New `inspect_org_scoped_api_readiness` selector + CLI + read-only `GET /api/v1/saas/org-scope-readiness/` + `OrgScopeReadinessCard`. 30 new backend + 2 new frontend pytest cases. | `globalTenantFilteringEnabled=false` (no middleware filters existing endpoints by org yet); per-org provider credentials still in env vars; superusers see across tenants for diagnostics; inactive memberships fall back to default org. Two-org leak proofs in tests assert `Customer` / `Order` / `WhatsAppConversation` scope correctly. |
| 6D | ✅ Live | Org-aware write path assignment. New `apps.saas.write_context` ships five helpers (`resolve_write_organization`, `apply_org_branch`, `assign_org_branch_from_parent`, `assign_org_branch_from_first_parent`). New `apps.saas.signals.auto_assign_org_branch` `pre_save` receiver wired to 13 business models (`Lead`, `Customer`, `Order`, `DiscountOfferLog`, `Payment`, `Shipment`, `Call`, `WhatsAppConsent`, `WhatsAppConversation`, `WhatsAppMessage`, `WhatsAppLifecycleEvent`, `WhatsAppHandoffToCall`, `WhatsAppPilotCohortMember`). New `inspect_org_write_path_readiness` selector + CLI + read-only `GET /api/v1/saas/write-path-readiness/` + `WritePathReadinessCard`. 28 new backend + 2 new frontend pytest cases. | Fires only on create + only when FK is `None`; never overwrites explicit assignment; never on bulk `update()`; silent on errors. Cross-tenant leak proof asserted. FKs remain nullable; no global queryset filter middleware. |
| 6E | ✅ Live | SaaS Admin foundation + integration settings metadata. New `OrganizationIntegrationSetting` table (per-org provider readiness only — non-sensitive config + `ENV:` / `VAULT:` secret refs). Selectors in `apps.saas.integration_settings` mask refs and never return raw values. Admin overview / organizations / integration-settings / integration-readiness APIs + read-only `/saas-admin` page. | Runtime providers still use env/config. Raw secrets neither stored nor exposed. Per-org runtime provider routing deferred to Phase 6F. |
| 6F | ✅ Live | Per-org runtime integration routing **preview**. New `apps.saas.integration_runtime` ships `mask_secret_ref`, `validate_secret_ref_format`, `get_secret_ref_status`, `resolve_secret_ref_preview`, `get_all_provider_runtime_previews` (composite + `safeToStartPhase6G` boolean). New `inspect_runtime_integration_routing` CLI + admin-only `GET /api/v1/saas/runtime-routing-readiness/`. Idempotent `seed_default_org_integration_refs` writes ENV: refs only (never raw values). `/saas-admin` adds "Runtime Integration Routing Preview" section. 24 new backend + 3 new frontend pytest cases. | `runtimeSource="env_config"` always; `perOrgRuntimeEnabled=False` always; `runtimeUsesPerOrgSettings=False` always. Live runtime is unchanged. |
| 6G | ✅ Live | Controlled Runtime Routing Dry Run + AI Provider Routing Preview. `apps.saas.runtime_operations` declares 14 operations (WhatsApp / Razorpay / PayU / Delhivery / Vapi / OpenAI / six AI tasks); each `dryRunAllowed=True` + `liveAllowedInPhase6G=False`. NVIDIA-primary AI routing table (reports → minimaxai/minimax-m2.7; ceo_planning → moonshotai/kimi-k2.6; caio_compliance / critical_fallback → mistralai/mistral-medium-3.5-128b; hinglish_chat / smoke_test → google/gemma-4-31b-it) + OpenAI fallback + Anthropic Claude secondary. Task-wise `max_tokens` resolves from env. New `apps.saas.runtime_dry_run` + `validate_dry_run_has_no_side_effects` invariant assertion. Three read-only management commands + one operator-only `smoke_test_ai_provider_routes` (the ONLY path that may issue a tiny live NVIDIA / OpenAI request — default prompt `"Reply only OK"`). Three admin-only DRF endpoints. `/saas-admin` adds "Controlled Runtime Routing Dry Run" + "AI Provider Routing Preview" sections. 56 new backend + 2 new frontend pytest cases. | `runtimeSource="env_config"`, `perOrgRuntimeEnabled=False`, `dryRun=True`, `liveExecutionAllowed=False`, `externalCallWillBeMade=False` — asserted on every dry-run output. PayU / Delhivery deferred-provider warnings (not blockers). Raw NVIDIA / OpenAI / Anthropic / Razorpay / Meta / Vapi keys never appear in any output. |
| 6H | ✅ Live | Controlled Runtime Routing Live Audit Gate (audit-only, no execution). Phase 6G is FULL PASS. New `apps.saas.live_gate_policy` declares the protected operations (each with `liveAllowedByDefault=false`, `approvalRequired=true`, `allowedInPhase6H=false`). New `RuntimeLiveGatePolicySnapshot`, `RuntimeLiveExecutionRequest`, `RuntimeKillSwitch` models — default global kill switch is enabled. `apps.saas.live_gate` evaluates / records sanitized decisions; approval in Phase 6H is audit-only. Management commands + `/api/v1/saas/runtime-live-gate/...` admin APIs. `/saas-admin` adds "Controlled Runtime Live Audit Gate" section. 16 new backend + 2 new frontend pytest cases. | No real WhatsApp / payment / shipment / call / provider side effect happens in Phase 6H. |
| 6I | ✅ Live | Single Internal Live Gate Simulation (simulation-only). `RuntimeLiveGateSimulation` + `apps.saas.live_gate_simulation` allow rehearsing one of `razorpay.create_order` (default), `whatsapp.send_text`, or `ai.smoke_test`. Six management commands (prepare / request-approval / approve / reject / run / rollback / inspect). `/api/v1/saas/runtime-live-gate/simulations/` admin APIs. `/saas-admin` adds "Single Internal Live Gate Simulation" section. | Hard invariant: `dryRun=true`, `liveExecutionAllowed=false`, `externalCallWillBeMade=false`, `externalCallWasMade=false`, `providerCallAttempted=false` in every simulation response. |
| 6J | ✅ Live | Single Internal Provider Test Plan. New `apps.saas.provider_test_plan_policy` registers seven plan policies (Razorpay create_order + create_payment_link, WhatsApp send_text, AI smoke_test, Vapi place_call, Delhivery create_shipment, PayU create_payment); only **target** is `razorpay.create_order`. New `RuntimeProviderTestPlan` model — every plan declares `dry_run=True`, `provider_call_allowed=False`, `external_call_will_be_made=False`, `real_money=False`, `real_customer_data_allowed=False`. Razorpay synthetic payload locked: `{amount: 100, currency: "INR", receipt: "phase6j_internal_test_plan_<plan_id>"}`. Six strictly-read-only / plan-only management commands + seven admin-only DRF endpoints under `/api/v1/saas/provider-test-plans/`. `/saas-admin` adds "Single Internal Provider Test Plan" section. 31 new backend + 2 new frontend pytest cases. | Approval in Phase 6J ONLY unlocks a future Phase 6K execution gate; never enables a provider call in Phase 6J. Asserted by `assert_provider_test_plan_has_no_side_effects`. |
| 6K-A | ✅ Live | Single Internal Razorpay Test-Mode Execution Gate (code / gate / readiness only — no execution yet). `apps.saas.provider_execution_policy` allows ONE Phase 6K target (`razorpay.create_order` against a Razorpay TEST key) and locks `amount_paise=100`, `currency="INR"`, `real_money=False`, `real_customer_data_allowed=False`, `api_execution_allowed=False` (CLI-only), `frontend_execution_allowed=False`, `max_executions_per_approved_plan=1`. New `RuntimeProviderExecutionAttempt` model + Phase-6K-only `apps.saas.razorpay_test_execution` adapter (separate from the Phase 2B production payment-link client). Five management commands; `execute_single_razorpay_test_order` is the ONLY path that may dispatch — refuses unless approved Phase 6J plan + `PHASE6K_RAZORPAY_TEST_EXECUTION_ENABLED=true` + `--confirm-test-execution` + `RAZORPAY_KEY_ID` starts with `rzp_test` + no prior successful execution + amount=100 paise + kill switch enabled. Five admin-only DRF endpoints (no POST execute — execution is CLI-only). 34 new backend + 2 new frontend pytest cases. | `/saas-admin` adds "Single Internal Razorpay Test-Mode Execution Gate" with readiness + invariants sub-cards. No "Execute" / "Capture" / "Go Live" / "Send" buttons on the UI. Never creates a payment link, never captures, never sends customer notification, never mutates business records. |
| 6K-B | ✅ Live (manual VPS one-shot) | First and only real Razorpay test-mode execution against the VPS. Result: `execution_id=pex_8f309650e9644cfaae4418f9` → `provider_object_id=order_Sks3KPf0vntKhf`, `amount_paise=100`, `currency=INR`. **No payment link, no capture, no notification, no business mutation.** `rollback_status=completed`. `PHASE6K_RAZORPAY_TEST_EXECUTION_ENABLED` flipped back to `false` immediately. This artefact is the immutable proof point that the Phase 6K gate works end-to-end on real Razorpay test infrastructure. | Recorded as the only real provider-side write the platform has ever made. Future Phase 6K-style runs require explicit Director sign-off + reset the env flag for one-shot use only. |
| 6L | ✅ Live | Razorpay Test Execution Audit Review + Webhook Readiness Plan. New `apps.saas.razorpay_audit_review` ships `review_razorpay_test_execution_audit(execution_id)` (replays the 10 Phase 6K invariants + scans every linked AuditEvent for raw-key leak; FAILs on flipped safety booleans / missing rollback / missing provider object id / leaked secret), `inspect_razorpay_webhook_readiness()` (env presence-only — masked id + webhook secret presence boolean — never raw value), `plan_razorpay_webhook_readiness()` (canonical policy doc with `POST /api/webhooks/razorpay/test/` endpoint design, HMAC-SHA256 + constant-time compare, `x_razorpay_event_id` idempotency, 300s replay window, 9-event allowlist + 9-event denylist, 8 future audit kinds, 13-key sensitive-payload scrub list, `businessMutationPolicy` all-False). Three read-only management commands + three auth-required DRF endpoints under `/api/v1/saas/razorpay/`. `/saas-admin` adds "Razorpay Test Execution Audit + Webhook Readiness" section. 33 new backend + 2 new frontend pytest cases. | Phase 6L never calls Razorpay, never creates a payment link, never captures, never sends notification, never mutates business records, never returns raw secrets, never returns the raw provider response (whitelisted summary only). |
| 6M-0 | ✅ Live | MCP Gateway Foundation + AI Connector Readiness (dormant). New `apps.mcp_gateway` Django app — six models (`McpClientApp`, `McpAccessPolicy`, `McpToolDefinition`, `McpResourceDefinition`, `McpPromptDefinition`, `McpToolInvocationLog`). 13-name forbidden-tool list. `MCP_ENABLED=false`, `MCP_READ_ONLY_MODE=true`, `MCP_WRITE_TOOLS_ENABLED=false`, `MCP_PROVIDER_TOOLS_ENABLED=false` defaults. Admin-only readiness APIs under `/api/v1/mcp/`. Idempotent registry seed command. `/saas-admin` adds "MCP Gateway Readiness" section. PII / raw-secret detection helpers (`detect_raw_secret`, `detect_full_pii` with `\b\d{10,}\b` word-boundary digit match to avoid timestamp false positives). 43 new backend + 2 new frontend pytest cases. | No tools active; no connectors active; no provider calls; no business mutation. The whole module is dormant scaffolding. |
| 6S | ✅ Shipped (planning-only, CLI-only) | Limited Internal Dispatch Pilot Plan. New `apps.payments.RazorpayPaymentDispatchPilotPlan` model + migration `payments.0009_phase6s_payment_dispatch_pilot_plan`. New `apps.payments.razorpay_payment_dispatch_pilot_plan` service ships 10 functions. Seven CLI commands. Four **read-only GET** admin/auth-protected DRF endpoints under `/api/v1/saas/razorpay/payment-dispatch-pilot-plan{,-plans,-plans/<id>,-preview}/` — **no POST endpoint** dispatches review state changes. Eight new audit kinds (`razorpay.payment_dispatch_pilot_plan.{readiness_inspected,previewed,prepared,approved_for_future_phase6t,rejected,archived,blocked,invariant_violation_blocked}`). New env flag `RAZORPAY_PAYMENT_DISPATCH_PILOT_PLAN_ENABLED` (default `False`). Approve requires non-empty manual review reason. Internal cohort only: `internal_only=True`, `max_pilot_orders=1`, `max_amount_paise=100`. Model declares **16 locked-False safety booleans** plus the defensive `assert_phase6s_no_live_execution_or_provider_call` guard. `/saas-admin` adds a "Razorpay Limited Internal Dispatch Pilot Plan" section (read-only — no buttons; CLI-only reminder; 9-event pilot contract table; recent pilot plans table; four readiness checklists — internal staff cohort / WhatsApp / courier / dispatch; abort criteria; verification checklist). 33 new backend tests + 2 new frontend tests; 1474 backend / 60 frontend, all green. | Phase 6S NEVER executes a pilot, NEVER sends a WhatsApp message, NEVER calls Meta Cloud, NEVER calls Delhivery, NEVER creates a shipment / AWB, NEVER mutates real `Order` / `Payment` / `Shipment` / `DiscountOfferLog` / `Customer` / `Lead` / `WhatsAppMessage` / `WhatsAppConversation` rows, NEVER calls Razorpay (asserted with before/after counts + mock-spied Razorpay / WhatsApp / Vapi / Delhivery clients). No POST API endpoint exists — review state changes are exclusively CLI. Production `.env.production` not edited. |
| 6R | ✅ Shipped (audit-only readiness, CLI-only) | Payment → WhatsApp / Courier Dispatch Readiness. New `apps.payments.RazorpayPaymentDispatchReadinessGate` model + migration `payments.0008_phase6r_payment_dispatch_readiness`. New `apps.payments.razorpay_payment_dispatch_readiness` service ships 10 functions. Seven CLI commands. Four **read-only GET** admin/auth-protected DRF endpoints under `/api/v1/saas/razorpay/payment-dispatch-readiness{,-gates,-gates/<id>,-preview}/` — **no POST endpoint** dispatches review state changes. Eight new audit kinds (`razorpay.payment_dispatch_readiness.{readiness_inspected,previewed,prepared,approved_for_future_phase6s,rejected,archived,blocked,invariant_violation_blocked}`). New env flag `RAZORPAY_PAYMENT_DISPATCH_READINESS_ENABLED` (default `False`). Approve requires non-empty manual review reason. The model declares **12 locked-False safety booleans** plus the defensive `assert_phase6r_no_live_send_or_courier_mutation` guard. `/saas-admin` adds a "Razorpay Payment → WhatsApp / Courier Dispatch Readiness" section (read-only — no buttons; CLI-only reminder; 9-event readiness contract table; recent gates table; three readiness checklists). 32 new backend tests + 2 new frontend tests; 1441 backend / 58 frontend, all green. | Phase 6R NEVER sends a WhatsApp message, NEVER calls Meta Cloud, NEVER calls Delhivery, NEVER creates a shipment / AWB, NEVER mutates real `Order` / `Payment` / `Shipment` / `DiscountOfferLog` / `Customer` / `Lead` / `WhatsAppMessage` / `WhatsAppConversation` rows, NEVER calls Razorpay (asserted with before/after counts + mock-spied Razorpay / WhatsApp / Vapi / Delhivery clients). No POST API endpoint exists — review state changes are exclusively CLI. Production `.env.production` not edited. |
| 6Q | ✅ Shipped (audit-gate-only, CLI-only) | Payment → Order Workflow Safety Gate. New `apps.payments.RazorpayPaymentOrderWorkflowGate` model + migration `payments.0007_phase6q_payment_order_workflow_gate`. New `apps.payments.razorpay_payment_order_workflow_gate` service ships 10 functions. Seven CLI commands. Four **read-only GET** admin/auth-protected DRF endpoints — no POST endpoint dispatches gate state changes. Eight new audit kinds (`razorpay.payment_order_gate.{readiness_inspected,previewed,prepared,approved_for_future_phase6r,rejected,archived,blocked,invariant_violation_blocked}`). New env flag `RAZORPAY_PAYMENT_ORDER_WORKFLOW_GATE_ENABLED` (default `False`). Approve requires non-empty manual review reason. `/saas-admin` adds a "Razorpay Payment → Order Workflow Safety Gate" section (read-only — no buttons; CLI-only reminder). 31 new backend tests + 2 new frontend tests; 1409 backend / 56 frontend, all green. | Phase 6Q NEVER mutates real `Order` / `Payment` / `Shipment` / `DiscountOfferLog` / `Customer` / `Lead` / `WhatsAppMessage` / `WhatsAppConversation` rows (asserted with before/after counts + mock-spied Razorpay / WhatsApp / Vapi clients). No POST API endpoint exists — gate state changes are exclusively CLI. Production `.env.production` not edited. |
| 6P | ✅ Shipped (sandbox-ledger-only, CLI-only) | Controlled Internal Paid-Status Mutation Test. New `apps.payments.RazorpaySandboxPaidStatusLedger` + `RazorpaySandboxPaidStatusMutationAttempt` models + migration `payments.0006_phase6p_sandbox_paid_status_models`. New `apps.payments.razorpay_sandbox_paid_status_mutation` service ships 10 functions (mapping + readiness + eligibility + preview + prepare + execute + rollback + archive + summary + invariant assertion). Seven CLI commands. Four **read-only** admin/auth-protected DRF endpoints under `/api/v1/saas/razorpay/sandbox-paid-status-...`. Eight new audit kinds (`razorpay.sandbox_paid_status.{readiness_inspected,previewed,attempt_prepared,execution_blocked,executed,rolled_back,archived,invariant_violation_blocked}`). New env flag `RAZORPAY_SANDBOX_PAID_STATUS_MUTATION_ENABLED` (default `False`) gates `execute`; even when `True`, execute requires `--confirm-sandbox-paid-status-mutation` + non-empty `--director-signoff`. `/saas-admin` adds a "Razorpay Sandbox Paid-Status Mutation Test" section (read-only — no execute/rollback buttons; CLI-only reminder). 31 new backend tests + 2 new frontend tests; 1378 backend / 54 frontend, all green. | Phase 6P NEVER mutates real `Order` / `Payment` / `Shipment` / `DiscountOfferLog` / `Customer` / `Lead` / `WhatsAppMessage` / `WhatsAppConversation` rows (asserted with before/after counts + mock-spied Razorpay / WhatsApp / Vapi clients). No POST execute/rollback API endpoint exists — execution is exclusively CLI. Production `.env.production` not edited. |
| 6O | ✅ Shipped (sandbox-review-only) | Razorpay Sandbox Status Mapping + Manual Review. New `apps.payments.RazorpaySandboxStatusReview` model + migration `payments.0005_phase6o_razorpay_sandbox_status_review`. New `apps.payments.razorpay_sandbox_status_mapping` service ships 10 functions (mapping + readiness + eligibility + preview + prepare + approve + reject + archive + invariant assert + summary). Six management commands (`inspect_razorpay_sandbox_status_mapping_readiness`, `preview_razorpay_sandbox_status_mapping`, `prepare_razorpay_sandbox_status_review`, `approve/reject/archive_razorpay_sandbox_status_review`). Seven admin/auth-protected DRF endpoints under `/api/v1/saas/razorpay/sandbox-status-...`. Eight new audit kinds (`razorpay.sandbox_status_mapping.readiness_inspected`, `razorpay.sandbox_status_review.{previewed,prepared,blocked,approved_for_future_phase6p,rejected,archived,invariant_violation_blocked}`). New env flag `RAZORPAY_SANDBOX_STATUS_MAPPING_ENABLED` (default `False`) gates `prepare`. `/saas-admin` adds a "Razorpay Sandbox Status Mapping + Manual Review" section with **only** "Approve Review Only" / "Reject Review" / "Archive Review" buttons (clearly labelled). 29 new backend tests + 2 new frontend tests; 1347 backend / 52 frontend, all green. | Approving a review only flips its `status` to `approved_for_future_phase6p`. Phase 6O NEVER mutates `Order` / `Payment` / `Shipment` / `DiscountOfferLog` / `Customer` (asserted with before/after counts + mock-spied Razorpay / WhatsApp / Vapi clients). Production `.env.production` not edited. |
| 6N | ✅ Shipped (planning-only) | Razorpay Webhook Business-Mutation Sandbox Plan. New `apps.saas.razorpay_business_mutation_plan` ships seven pure functions (`get_razorpay_business_mutation_sandbox_plan`, `inspect_razorpay_business_mutation_sandbox_readiness`, `build_razorpay_event_status_mapping_plan`, `build_synthetic_order_eligibility_policy`, `build_phase6n_manual_review_checklist`, `build_phase6n_rollback_plan`, `validate_phase6n_no_mutation_invariants`). 9-event mapping (`payment_link.paid`, `payment.captured`, `payment.failed`, `payment.authorized`, `order.paid`, `payment_link.cancelled`, `payment_link.expired`, `refund.created`, `refund.processed`); every row carries `mutationAllowedInPhase6N=False`. Synthetic-order eligibility policy + 8-item manual review checklist + 7-step rollback plan + locked safety invariants. Two read-only management commands. Two admin/auth-protected GET endpoints under `/api/v1/saas/razorpay/`. `/saas-admin` adds a "Razorpay Business Mutation Sandbox Plan" section. **No DB migration. No env flag flipped. No provider call. No business mutation. No customer notification.** 35 new backend tests + 2 new frontend tests; 1318 backend / 50 frontend, all green. | Phase 6N never mutates business state, never calls Razorpay, never sends a customer notification, never flips an env flag. Phase 6O remains future. |
| 6M | ✅ Live | Razorpay Webhook Handler Implementation (test-mode, dormant by default). `apps.payments.razorpay_webhooks.process_razorpay_webhook` ships HMAC-SHA256 signature verification over the raw body in constant time, 300-second replay window validation, idempotency on `X-Razorpay-Event-Id`, 9-event allowlist + 9-event denylist, 13-key sensitive-payload scrub list, `assert_no_business_mutation` invariant, and a safe-summary persistence path on `RazorpayWebhookEvent`. New `POST /api/webhooks/razorpay/test/` endpoint (separate from the Phase 2B production `/api/webhooks/razorpay/`). Four read-only / planning management commands (`inspect_razorpay_webhook_handler_readiness`, `simulate_razorpay_webhook_event`, `inspect_razorpay_webhook_events`, `purge_razorpay_webhook_test_events`). Four auth-required DRF readiness/list/detail/simulate APIs under `/api/v1/saas/razorpay/webhook-...`. `/saas-admin` adds "Razorpay Webhook Handler (Test Mode)" section. 9 new audit kinds (`razorpay.webhook.*`). 42 new backend + 2 new frontend pytest cases. | Dormant by default — `RAZORPAY_WEBHOOK_TEST_MODE_ENABLED=false`, `RAZORPAY_WEBHOOK_BUSINESS_MUTATION_ENABLED=false`, `RAZORPAY_WEBHOOK_NOTIFY_CUSTOMER_ENABLED=false`, `RAZORPAY_WEBHOOK_STORE_RAW_PAYLOAD=false`. Order / Payment / Shipment / DiscountOfferLog / Customer never mutated (asserted in tests). No customer notification ever sent. Production webhook secret is **never** used by this handler. |
...
- **Phase 6R — Payment → WhatsApp/courier readiness, no live send yet (planned, not started).** Audit-only readiness contract that maps an approved Phase 6Q workflow gate into a future WhatsApp / courier dispatch readiness check, behind a NEW env flag distinct from `RAZORPAY_PAYMENT_ORDER_WORKFLOW_GATE_ENABLED`. Phase 6R defines and audits readiness signals (Claim Vault coverage, consent state, allow-list cohort, courier service-area) without sending any real WhatsApp message or creating any real shipment. Phase 6R never mutates real business rows in its own scope.
- Phase 5F — approval-gated broadcast / sales campaigns (MARKETING tier templates, director sign-off).
- Phase 7+ — recording / QA / learning loop pipeline (speech-to-text → compliance → CAIO audit → sandbox → live promotion); Android / iOS apps hitting the same backend.

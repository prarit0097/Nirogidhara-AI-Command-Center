...
> **Operational supersession note (Phase 16H — PRODUCTION VERIFIED on the VPS and CLOSED at `d733cf0`).** This blueprint v2 reflects the project's strategic framing as of its original revision date below and contains historical Phase 12D / Phase 8F-era wording. **Current operational truth is [`../nd.md`](../nd.md) head-of-file.** The current operational baseline is **Phase 16H — Internal Pilot Execution Workbench + Role-Based Task Queues** (internal/DB-only role-based task-queue execution layer extending `apps.pilot`; no live provider action; provider actions locked at every task status including `in_progress`/`done`; VPS browser-validated at `/operations/pilot-workbench`), on top of **Phase 16G — Internal Pilot Control Center / Pilot Execution Dashboard** (PRODUCTION VERIFIED at `38e8dc8`), **Phase 16F — Controlled Internal Pilot Readiness + End-to-End Dry Run** (PRODUCTION VERIFIED at `967ed3d`) and **Phase 16E — Payment / Logistics Integration Hardening** (PRODUCTION VERIFIED at `36395f6`); **Phase 16D — Uploaded Customer Data Campaigns is verified at `c0be74a`**, **Phase 16C — Director Daily Briefing + Team Roles UI at `687ef41`**, and **Phase 16B — Customer Lifecycle UI Backbone at `00c3295`** (the Phase 15 safety shell remains frozen at commit `eefd8b3`). Where this blueprint and `nd.md` disagree on any operational detail, **`nd.md` wins** and the blueprint is the historical strategic mirror. Next planned work is **Phase 16I** (NOT started; separate Director directive required). This blueprint is not edited section-by-section as new phases ship; it is preserved as the strategic v2 snapshot.

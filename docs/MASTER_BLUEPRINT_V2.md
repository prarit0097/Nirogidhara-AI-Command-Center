...
| Completed phase range | Phase 1 → **Phase 6O — Razorpay Sandbox Status Mapping + Manual Review (sandbox-review-only)** |
| Latest pushed commit | see `git log -1` (Phase 6O commit on `origin/main`) |
| Last verified test baseline | **1347 backend tests · 52 frontend tests** · `makemigrations --check` clean · `manage.py check` clean · frontend lint 0 errors · build OK |
| Next planned phase | **Phase 6P — Controlled internal paid-status mutation test** (implementation-only synthetic `Order` / `Payment` status flip, behind a NEW env flag distinct from `RAZORPAY_SANDBOX_STATUS_MAPPING_ENABLED`, gated by an approved Phase 6O review + Director sign-off) |
...
## 3. Completed Build Timeline — Phase 1 → Phase 6O Razorpay Sandbox Status Mapping + Manual Review (sandbox-review-only)
...
- **Phase 6P — Controlled internal paid-status mutation test (planned, not started).** Implementation-only synthetic `Order.status` / `Payment.status` flip, behind a NEW env flag distinct from `RAZORPAY_SANDBOX_STATUS_MAPPING_ENABLED`, gated by: (a) the source `RazorpaySandboxStatusReview.status == approved_for_future_phase6p`, (b) the linked `RazorpayWebhookEvent` being synthetic-eligible per Phase 6O policy, (c) the new env flag being `True`, (d) a Director sign-off `AuditEvent` for the review id. New audit kinds `razorpay.sandbox_paid_status.{applied,blocked,rolled_back}`. Mandatory CLI rollback restores the synthetic row to pre-mutation state. Acceptance criteria: every Phase 6N + 6O safety invariant stays true on real (non-synthetic) rows; new flag stays `False` in `.env.production`.

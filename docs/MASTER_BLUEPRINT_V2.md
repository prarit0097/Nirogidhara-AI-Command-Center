...
| Completed phase range | Phase 1 → **Phase 6R — Payment → WhatsApp / Courier Dispatch Readiness (audit-only readiness contract; CLI-only review state changes)** |
| Latest pushed commit | see `git log -1` (Phase 6R commit on `origin/main`) |
| Last verified test baseline | **1441 backend tests · 58 frontend tests** · `makemigrations --check` clean · `manage.py check` clean · frontend lint 0 errors · build OK |
| Next planned phase | **Phase 6S — Limited Internal Live Payment → Dispatch Pilot Planning (planning-only, not started)** — behind a NEW env flag distinct from `RAZORPAY_PAYMENT_DISPATCH_READINESS_ENABLED`. |

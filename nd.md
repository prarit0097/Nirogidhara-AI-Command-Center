...
- **Test baseline (green):** **1540 backend tests + 64 frontend tests.** `python manage.py makemigrations --check --dry-run` → `No changes detected`. `python manage.py check` → 0 issues. `npm run lint` → 0 errors (8 pre-existing shadcn warnings). `npm test` → 64 passed. `npm run build` → OK.

...
- **What's next:** Live WhatsApp sender; learning loop pipeline; multi-tenant SaaS.
...
_End of `nd.md`. Last updated after Phase 4E Expanded Approved Execution Registry (`discount.up_to_10`, `discount.11_to_20`, `ai.sandbox.disable` handlers added; `apply_order_discount` service mutates only `Order.discount_pct` via Phase 3E policy; sandbox.disable Director-only + idempotent on already-off; `discount.above_20` + ad-budget + refund + WhatsApp + production-live-mode-switch all stay unmapped)._

...
- **Status today (Phase 1 + 2A + 2B + 2C + 2D + 2E + 3A + 3B + 3C + 3D + 3E + 4A + 4B + 4C + 4D + 4E done):** **15 Django apps** scaffolded, **25 read + 16 write endpoints + Razorpay/Delhivery/Vapi/Meta Lead Ads webhooks + AgentRun (3A) + 8 per-agent runtime endpoints (3B) + scheduler-status (3C) + sandbox + prompt-version + budget endpoints (3D) + product catalog read/write + approval matrix read endpoint (3E) + reward-penalty events / summary / sweep endpoints (4B) + approval list / approve / reject / evaluate / agent-run-request-approval endpoints (4C) + approval execute endpoint with allow-listed registry (4D) + Phase 4A real-time AuditEvent WebSocket at `/ws/audit/events/`** live, Master Event Ledger via signals + explicit service writes, JWT auth + role-based permissions, order state machine, **all four gateway integrations with three-mode (mock/test/live) adapters + HMAC-verified webhooks + idempotency**, **AgentRun foundation + 7 per-agent runtime services + Celery beat at 09:00 + 18:00 IST + provider fallback (OpenAI → Anthropic) + model-wise USD cost tracking + Phase 3D sandbox toggle + versioned prompts (rollback) + per-agent USD budget guards + Governance page + Phase 3E product catalog admin + discount policy (10/20% bands) + ₹499 fixed advance + reward/penalty scoring formula + approval matrix policy + WhatsApp sales/support design scaffold + Phase 4B reward/penalty engine (AI agents only, CEO AI net accountability) + Rewards page now shows agent-wise leaderboard and order-wise scoring events + Phase 4C approval-matrix middleware enforcement (`enforce_or_queue` + ApprovalRequest table + AgentRun → approval bridge + Governance page approval queue with Approve / Reject) + Phase 4D Approved Action Execution Layer (`POST /api/ai/approvals/{id}/execute/`, ApprovalExecutionLog table with one-executed-per-request constraint, allow-listed registry of 3 actions: payment.link.advance_499 + payment.link.custom_amount + ai.prompt_version.activate; everything else returns 400 + ai.approval.execution_skipped audit; CAIO blocked at engine + bridge + execute layer) + Phase 4A Django Channels live `/ws/audit/events/` WebSocket fan-out (initial snapshot + per-event push including the full stored AuditEvent.payload; Dashboard activity feed and Governance approval queue refresh live; existing polling endpoints remain as fallback) + Phase 4E expanded execution registry adds 3 more handlers (discount.up_to_10, discount.11_to_20, ai.sandbox.disable) — discount.above_20 + ad.budget_change + payment.refund + whatsapp.* + ai.production.live_mode_switch all stay unmapped → 400 + skipped audit; ai.sandbox.disable execution is Director-only via the matrix's director_override mode**, seed command, frontend wired with **automatic mock fallback** (19 pages). **351 backend tests + 13 frontend tests** all green.
- **What's next:** Phase 5A WhatsApp Live Sender Foundation (Meta Cloud production target; the reference repo's Meta Cloud provider is stubbed so Phase 5A builds from scratch; Baileys is dev/demo only). Phase 5A-1 (DONE, doc-only) widens the WhatsApp scope from "lifecycle reminder sender" to "**inbound-first AI Chat Sales Agent + lifecycle messaging**" mirroring the AI Calling Agent's business objective: greet → category detection → Claim-Vault-grounded explanation → objection handling → address collection in chat → order booking → payment-link handoff → confirmation / delivery / RTO / reorder → chat-to-call handoff when needed. **Locked discount discipline:** AI never offers a discount upfront — discount is a **rescue tool only** (eligible at order-booking refusal / confirmation refusal / delivery-RTO refusal); **total discount across all stages must NEVER exceed 50%** (worked examples in `docs/WHATSAPP_INTEGRATION_PLAN.md` §AA–§BB). Then 5B inbound inbox / Customer 360 timeline; 5C AI Chat Sales Agent with `learned_memory.py` ported wholesale + Claim-Vault-bound LLM + suggest/auto modes; 5D chat-to-call handoff + lifecycle automation; 5E confirmation/delivery/RTO/reorder automation with rescue-discount flow + 50% cap enforcement in code; 5F approval-gated campaigns; learning loop pipeline; multi-tenant SaaS.
...
_End of `nd.md`. Last updated after Phase 5A-1 WhatsApp AI Chat Agent + Discount Rescue Policy Addendum (sections S–GG appended to `docs/WHATSAPP_INTEGRATION_PLAN.md`; greeting rule locked, auto-reply with guardrails, address collection in chat, category detection, chat-to-call handoff triggers, discount discipline locked, **50% total-discount hard cap across all AI workflows**, future model + API planning notes, learning-loop scope; no runtime code changes in this phase)._

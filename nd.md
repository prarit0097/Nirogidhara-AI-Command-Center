...
- **Latest completed phase:** **Phase 6P — Controlled Internal Paid-Status Mutation Test (sandbox-ledger-only, CLI-only execution).**
...
- **Latest pushed commit:** see `git log -1` (Phase 6P commit on `origin/main`).
- **Test baseline (green):** **1378 backend tests + 54 frontend tests.** `python manage.py makemigrations --check --dry-run` → `No changes detected` (no Phase 6N migration required). `python manage.py check` → 0 issues. `npm run lint` → 0 errors (8 pre-existing shadcn warnings). `npm test` → 50 passed. `npm run build` → OK.
- **Next planned phase:** **Phase 6Q — Payment → order workflow safety gate.** Not started. Will introduce a controlled gate that converts Phase 6P sandbox ledger transitions into staged, audit-only payment-to-order workflow checks, behind a NEW env flag distinct from `RAZORPAY_SANDBOX_PAID_STATUS_MUTATION_ENABLED`. Phase 6Q never mutates real `Order` / `Payment` rows in its own scope either — it adds the workflow safety contract that future phases will use.
...
### Docs Sync Status (Phase 6P baseline)
...
**Phase 6Q — Payment → order workflow safety gate.** Implementation-only of the workflow-safety contract that wraps the Phase 6P sandbox ledger. Phase 6Q must:

1. Introduce a NEW env flag distinct from `RAZORPAY_SANDBOX_PAID_STATUS_MUTATION_ENABLED` (e.g. `RAZORPAY_PAYMENT_TO_ORDER_WORKFLOW_GATE_ENABLED`, default `False`).
2. Define the canonical "what would happen if Phase 6P ledger row X transitioned" contract — i.e. a sandbox-only audit-trail of which downstream business actions a future phase would queue for an approved Phase 6P attempt, **without any of those downstream actions actually firing**.
3. Add the missing pre-conditions: matrix entry / Director sign-off / cohort allowlist for the future production path. Audit-only.
4. Provide a CLI inspector that compares the sandbox ledger state against the workflow contract and reports `safeToStartPhase6R`-style readiness.
5. Phase 6Q must NOT mutate real `Order` / `Payment` rows. It documents and gates; it does not execute.

Acceptance criteria: every Phase 6N + 6O + 6P safety invariant stays true on real (non-synthetic) rows; the new env flag stays `False` in `.env.production`; `business_mutation_was_made` / `customer_notification_sent` stay `False` on every real `RazorpayWebhookEvent`, `RazorpaySandboxStatusReview`, `RazorpaySandboxPaidStatusLedger`, and `RazorpaySandboxPaidStatusMutationAttempt` row.
...
- Do **not** start writing Phase 6Q code, migrations, env flags, or models without first running the Phase 6P readiness inspector AND completing at least one CLI execute + rollback cycle on a synthetic VPS test event so `safeToStartPhase6Q=true` is observed.

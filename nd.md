...
- **Latest completed phase:** **Phase 6O — Razorpay Sandbox Status Mapping + Manual Review (sandbox-review-only).**
...
- **Latest pushed commit:** see `git log -1` (Phase 6O commit on `origin/main`).
- **Test baseline (green):** **1347 backend tests + 52 frontend tests.** `python manage.py makemigrations --check --dry-run` → `No changes detected` (no Phase 6N migration required). `python manage.py check` → 0 issues. `npm run lint` → 0 errors (8 pre-existing shadcn warnings). `npm test` → 50 passed. `npm run build` → OK.
- **Next planned phase:** **Phase 6P — Controlled internal paid-status mutation test.** Not started. Will introduce sandbox-only `Order.status` / `Payment.status` mutation against synthetic test orders behind a NEW env flag distinct from `RAZORPAY_SANDBOX_STATUS_MAPPING_ENABLED`, gated by an approved Phase 6O review + Director sign-off.
...
### Docs Sync Status (Phase 6O baseline)
...
**Phase 6P — Controlled internal paid-status mutation test.** Implementation-only of the next stage. Phase 6P must:

1. Introduce a NEW env flag distinct from `RAZORPAY_SANDBOX_STATUS_MAPPING_ENABLED` (e.g. `RAZORPAY_SANDBOX_PAID_STATUS_MUTATION_ENABLED`, default `False`).
2. Refuse to mutate unless: (a) the source `RazorpaySandboxStatusReview.status == approved_for_future_phase6p`, (b) the linked `RazorpayWebhookEvent` is synthetic-eligible per Phase 6O policy, (c) the new env flag is `True`, (d) a Director sign-off `AuditEvent` exists for the review id, (e) the Phase 6O rollback plan steps are implementable in code.
3. Permit sandbox-only `Order.status` / `Payment.status` flip on a SYNTHETIC test row (synthetic-order resolver gated on `phase6j_internal_test_plan_*` / `phase6n_sandbox_synthetic_*` prefixes only).
4. Emit `razorpay.sandbox_paid_status.{applied,blocked,rolled_back}` audit kinds.
5. Provide a CLI rollback command that restores the synthetic row to pre-mutation state.

Acceptance criteria for the Phase 6P implementation: every Phase 6N + 6O safety invariant remains true on real production rows; the new sandbox flag stays `False` in `.env.production`; `business_mutation_was_made` / `customer_notification_sent` stay `False` on every real (non-synthetic) `RazorpayWebhookEvent` and every `RazorpaySandboxStatusReview` row.
...
- Do **not** start writing Phase 6P code, migrations, env flags, or models without first running the Phase 6O readiness inspector and confirming `safeToStartPhase6P=true` on a real VPS run.

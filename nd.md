...
```bash
# Backend
cd backend && python -m pytest -q   # 322 tests (26 reads + 18 writes + 13 razorpay + 7 ai config + 13 delhivery + 16 vapi + 14 meta + 25 phase3a + 26 phase3b + 17 phase3c + 14 phase3d + 29 phase3e + 25 phase4b + 31 phase4c + 39 phase4d + 8 phase4a)
...
### Pending — live WhatsApp + expanded execution registry
...
- Expanded execution registry (discount + sandbox-disable + ad-budget execution) intentionally **not** in the Phase 4D first pass; expansion needs explicit Prarit sign-off + matching tests.
...
_End of `nd.md`. Last updated after Phase 4A Real-time AuditEvent WebSockets (Django Channels + channels_redis + daphne; `/ws/audit/events/` carries the full stored AuditEvent.payload; Dashboard activity feed and Governance approval queue refresh live; existing polling endpoints remain as fallback)._

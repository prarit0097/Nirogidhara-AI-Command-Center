...
- **What's next (locked sequence):** **OpenAI provider hotfix → OpenAI smoke retest → one-number Meta live test → Phase 5F.** Concretely: rebuild the backend image on the VPS so `openai>=1.0,<2.0` lands in the container (Phase 5E-Smoke-Fix); re-run `python manage.py run_controlled_ai_smoke_test --scenario ai-reply --use-openai --json` and verify `detail.openaiSucceeded=true` + `detail.providerPassed=true` + `overallPassed=true`; flip `WHATSAPP_PROVIDER=meta_cloud` with `WHATSAPP_LIVE_META_LIMITED_TEST_MODE=true` and exactly one number in `WHATSAPP_LIVE_META_ALLOWED_TEST_NUMBERS` for the one-number live test; then start **Phase 5F** (approval-gated broadcast campaigns, MARKETING template tier, director sign-off). Phase 6 (learning loop pipeline) and Phase 7 (multi-tenant SaaS) follow.

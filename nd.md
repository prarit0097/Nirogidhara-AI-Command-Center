...
- **Latest completed phase:** **Phase 6R — Payment → WhatsApp / Courier Dispatch Readiness (audit-only readiness contract; CLI-only review state changes; `RAZORPAY_PAYMENT_DISPATCH_READINESS_ENABLED=false` default; never sends WhatsApp, never calls Meta Cloud or Delhivery, never creates a shipment / AWB, never mutates real business tables, never calls Razorpay).**
...
- **Latest pushed commit:** see `git log -1` (Phase 6R commit on `origin/main`).
- **Test baseline (green):** **1441 backend tests + 58 frontend tests.** `python manage.py makemigrations --check --dry-run` → `No changes detected`. `python manage.py check` → 0 issues. `npm run lint` → 0 errors (8 pre-existing shadcn warnings). `npm test` → 58 passed. `npm run build` → OK.
- **Next planned phase:** **Phase 6S — Limited Internal Live Payment → Dispatch Pilot Planning (planning-only, not started).** Will define the audit-only contract that maps an approved Phase 6R dispatch readiness gate into a single-staff-cohort live pilot — ALL gates from Phase 6R remain in force; no broad rollout. Phase 6S must NOT send a real WhatsApp message, NOT create a real shipment, NOT call Razorpay capture, NOT mutate any real business row outside the cohort plan in its own scope.
...
### Docs Sync Status (Phase 6R baseline)
...
**Phase 6S — Limited Internal Live Payment → Dispatch Pilot Planning (planning-only).** Audit-only follow-up that maps an approved Phase 6R dispatch readiness gate into a single-staff-cohort live pilot plan. Phase 6S must:

1. Introduce a NEW env flag distinct from `RAZORPAY_PAYMENT_DISPATCH_READINESS_ENABLED` (suggested name `RAZORPAY_INTERNAL_LIVE_DISPATCH_PILOT_ENABLED`, default `False`).
2. Define and audit (no execution) the canonical pilot policy: cohort = the existing `WHATSAPP_LIVE_META_ALLOWED_TEST_NUMBERS` allow-list, ₹100 paise ceiling, single-Director-signoff, Phase 6Q + 6R proof on the source row, and the explicit kill-switch + rollback contract.
3. Provide CLI inspectors that walk Phase 6Q + 6R + readiness signals and report a `safeToStartPhase6T` readiness signal.
4. Phase 6S must NOT send a real WhatsApp message, NOT create a real shipment, NOT call Razorpay capture, NOT mutate any real business row in its own scope.

Acceptance criteria: every Phase 6N + 6O + 6P + 6Q + 6R safety invariant stays true on real rows; the new env flag stays `False` in `.env.production`; no real `Order` / `Payment` / `Shipment` / `DiscountOfferLog` / `Customer` / `Lead` / `WhatsAppMessage` / `WhatsAppConversation` mutation. No live WhatsApp send. No live courier API call. No live Razorpay capture.

...
- **What's next (locked sequence):** **VPS rebuild for Smoke-Fix-3 → OpenAI smoke retest → one-number Meta live test → Phase 5F.** Concretely: rebuild the backend image on the VPS so the new orchestrator (server-side safety corrector + tightened prompt) lands in the container; re-run `python manage.py run_controlled_ai_smoke_test --scenario ai-reply --use-openai --json` and verify `detail.openaiSucceeded=true` + `detail.providerPassed=true` + `overallPassed=true` (and look for `whatsapp.ai.safety_downgraded` audit rows when a normal inquiry's flag was over-set by the LLM); flip `WHATSAPP_PROVIDER=meta_cloud` with `WHATSAPP_LIVE_META_LIMITED_TEST_MODE=true` and exactly one number in `WHATSAPP_LIVE_META_ALLOWED_TEST_NUMBERS` for the one-number live test; then start **Phase 5F** (approval-gated broadcast campaigns, MARKETING template tier, director sign-off). Phase 6 (learning loop pipeline) and Phase 7 (multi-tenant SaaS) follow.
